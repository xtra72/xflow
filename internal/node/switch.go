...
//	메시지(Clone) 1건을 라우팅한다.
//
// all 모드: 매칭되는 모든 라우트의 포트로 각각 메시지 Clone을 팬아웃한다.
//
// 매칭이 하나도 없으면(두 모드 공통):
//   - defaultPort가 설정되어 있으면 기본 포트로 Clone 1건을 라우팅한다.
...
func (n *SwitchNode) Process(_ context.Context, msg message.Message) ([]message.Message, error) {
	...
	case matchAll:
		...
		// 매칭이 하나라도 있으면 default 미사용.
		if len(outs) > 0 {
			return outs, nil
		}
		// 매칭 0건 → default/드롭 규칙 적용.
		return n.routeDefault(msg, defPort), nil
...
// defPort가 비어 있지 않으면 Clone 1건을 기본 포트로 라우팅하고,
// 비어 있으면 빈 슬라이스를 반환한다 (드롭).
func (n *SwitchNode) routeDefault(msg message.Message, defPort string) []message.Message {
	if defPort != "" {
		out := msg.Clone()
