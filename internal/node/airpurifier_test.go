...
// ---------------------------------------------------------------------------
// 테스트 헬퍼 — 실제 port 모드 AirPurifier 에이전트 + resolver/transport 배선
// ---------------------------------------------------------------------------

// newPortAirPurifierAgent 는 port 모드 AirPurifier 에이전트를 생성하고 지정 디바이스를
// add_device 로 등록한다.
func newPortAirPurifierAgent(t *testing.T, deviceIDs ...string) *airpurifier.AirPurifierAgent {
	t.Helper()
	...
	a, err := airpurifier.NewAirPurifierAgent(cfg)
	require.NoError(t, err)
	ap, ok := a.(*airpurifier.AirPurifierAgent)
	require.True(t, ok)
...
func wireAPResolver(ap *airpurifier.AirPurifierAgent) AgentResolver {
...
func TestAirpurifierStatusNode_Type(t *testing.T) {
	t.Parallel()
	def := flow.NewNodeDef("ap-status", "airpurifier-status")
	n, err := NewAirpurifierStatusNode(def)
	require.NoError(t, err)
	require.NoError(t, n.Configure(map[string]any{"agent_ref": "ap-1"}))
	assert.Equal(t, "airpurifier-status", n.Type())
}

func TestAirpurifierControlNode_Type(t *testing.T) {
	t.Parallel()
	def := flow.NewNodeDef("ap-control", "airpurifier-control")
	n, err := NewAirpurifierControlNode(def)
	require.NoError(t, err)
	require.NoError(t, n.Configure(map[string]any{"agent_ref": "ap-1"}))
	assert.Equal(t, "airpurifier-control", n.Type())
}

func TestAirpurifierNode_AgentRefRequired(t *testing.T) {
	t.Parallel()
	sdef := flow.NewNodeDef("ap-status", "airpurifier-status")
	sn, err := NewAirpurifierStatusNode(sdef)
	require.NoError(t, err)
	assert.ErrorIs(t, sn.Configure(map[string]any{}), ErrAirPurifierMissingAgentRef)

	cdef := flow.NewNodeDef("ap-control", "airpurifier-control")
	cn, err := NewAirpurifierControlNode(cdef)
	require.NoError(t, err)
	assert.ErrorIs(t, cn.Configure(map[string]any{}), ErrAirPurifierMissingAgentRef)
}
...
func TestAirpurifierStatusNode_StateInputEmitsTelemetry(t *testing.T) {
	ap := newPortAirPurifierAgent(t, "ap-101")

	def := flow.NewNodeDef("ap-status", "airpurifier-status")
	node, err := NewAirpurifierStatusNode(def, WithAgentResolver(wireAPResolver(ap)))
	require.NoError(t, err)
	sn := node.(*AirpurifierStatusNode)
...
func TestAirpurifierControlNode_ControlOutputPort(t *testing.T) {
	ap := newPortAirPurifierAgent(t, "ap-101")

	def := flow.NewNodeDef("ap-control", "airpurifier-control")
	node, err := NewAirpurifierControlNode(def, WithAgentResolver(wireAPResolver(ap)))
	require.NoError(t, err)
	cn := node.(*AirpurifierControlNode)
...
func TestAirpurifierControlNode_InferSetPowerFromKey(t *testing.T) {
	ap := newPortAirPurifierAgent(t, "ap-201")
...
func TestAirpurifierNodes_StateInputAndControlOutputAreSeparate(t *testing.T) {
	ap := newPortAirPurifierAgent(t, "ap-101")

	sdef := flow.NewNodeDef("ap-status", "airpurifier-status")
	snode, err := NewAirpurifierStatusNode(sdef, WithAgentResolver(wireAPResolver(ap)))
	require.NoError(t, err)
	sn := snode.(*AirpurifierStatusNode)
	require.NoError(t, sn.Configure(map[string]any{"agent_ref": "ap-agent-1"}))
	require.NoError(t, sn.Init(context.Background()))
	defer func() { _ = sn.Shutdown(context.Background()) }()

	cdef := flow.NewNodeDef("ap-control", "airpurifier-control")
	cnode, err := NewAirpurifierControlNode(cdef, WithAgentResolver(wireAPResolver(ap)))
	require.NoError(t, err)
	cn := cnode.(*AirpurifierControlNode)
	require.NoError(t, cn.Configure(map[string]any{"agent_ref": "ap-agent-1"}))
	require.NoError(t, cn.Init(context.Background()))
	defer func() { _ = cn.Shutdown(context.Background()) }()
...
func TestAirpurifierNodes_AgentRefAndReinit(t *testing.T) {
	ap := newPortAirPurifierAgent(t, "ap-101")

	sdef := flow.NewNodeDef("ap-status", "airpurifier-status")
	sn0, err := NewAirpurifierStatusNode(sdef, WithAgentResolver(wireAPResolver(ap)))
	require.NoError(t, err)
	sn := sn0.(*AirpurifierStatusNode)
	require.NoError(t, sn.Configure(map[string]any{"agent_ref": "ap-agent-1"}))
	...
	cdef := flow.NewNodeDef("ap-control", "airpurifier-control")
	cn0, err := NewAirpurifierControlNode(cdef, WithAgentResolver(wireAPResolver(ap)))
	require.NoError(t, err)
	cn := cn0.(*AirpurifierControlNode)
	require.NoError(t, cn.Configure(map[string]any{"agent_ref": "ap-agent-1"}))
...
// resolve 실패(NoResolver/NotAirPurifier 이외)는 deferred connection 으로 Running 진행.
func TestAirpurifierNodes_InitDeferredOnResolveFailure(t *testing.T) {
	resolver := &mockNASAResolver{err: assert.AnError}

	sdef := flow.NewNodeDef("ap-status", "airpurifier-status")
	sn0, err := NewAirpurifierStatusNode(sdef, WithAgentResolver(resolver))
	require.NoError(t, err)
	sn := sn0.(*AirpurifierStatusNode)
	...
	cdef := flow.NewNodeDef("ap-control", "airpurifier-control")
	cn0, err := NewAirpurifierControlNode(cdef, WithAgentResolver(resolver))
	require.NoError(t, err)
	cn := cn0.(*AirpurifierControlNode)
...
// Init: resolver 미설정 시 NoResolver 에러; wrong 에이전트 타입 시 NotAirPurifier.
func TestAirpurifierNodes_InitErrors(t *testing.T) {
	// resolver 미설정.
	def := flow.NewNodeDef("ap-status", "airpurifier-status")
	sn0, err := NewAirpurifierStatusNode(def)
	require.NoError(t, err)
	sn := sn0.(*AirpurifierStatusNode)
	require.NoError(t, sn.Configure(map[string]any{"agent_ref": "ap-1"}))
	assert.ErrorIs(t, sn.Init(context.Background()), ErrAirPurifierNoResolver)

	// 잘못된 에이전트 타입 (AgentAccessor 미구현 transport).
	badResolver := &mockNASAResolver{transport: &mockNASATransportNoAccessor{}}
	def2 := flow.NewNodeDef("ap-control", "airpurifier-control")
	cn0, err := NewAirpurifierControlNode(def2, WithAgentResolver(badResolver))
	require.NoError(t, err)
	cn := cn0.(*AirpurifierControlNode)
	require.NoError(t, cn.Configure(map[string]any{"agent_ref": "ap-1"}))
	assert.ErrorIs(t, cn.Init(context.Background()), ErrAirPurifierAgentNotAirPurifier)
}

// 제어 키/command 가 없는 메시지는 명령을 구성할 수 없어 에러.
func TestAirpurifierControlNode_NoCommandError(t *testing.T) {
	ap := newPortAirPurifierAgent(t, "ap-101")
	def := flow.NewNodeDef("ap-control", "airpurifier-control")
	cn0, err := NewAirpurifierControlNode(def, WithAgentResolver(wireAPResolver(ap)))
	require.NoError(t, err)
	cn := cn0.(*AirpurifierControlNode)
	require.NoError(t, cn.Configure(map[string]any{"agent_ref": "ap-agent-1"}))
	require.NoError(t, cn.Init(context.Background()))
	...
	in.Payload().Set("device_id", "ap-101")
	_, err = cn.Process(context.Background(), in)
	assert.ErrorIs(t, err, ErrAirPurifierProcessFailed)
}

// 상태 입력 포트: device_id 없는 메시지는 no-op (주입하지 않음).
func TestAirpurifierStatusNode_ProcessNoDeviceID(t *testing.T) {
	ap := newPortAirPurifierAgent(t, "ap-101")
	def := flow.NewNodeDef("ap-status", "airpurifier-status")
	sn0, err := NewAirpurifierStatusNode(def, WithAgentResolver(wireAPResolver(ap)))
	require.NoError(t, err)
	sn := sn0.(*AirpurifierStatusNode)
	require.NoError(t, sn.Configure(map[string]any{"agent_ref": "ap-agent-1"}))
	require.NoError(t, sn.Init(context.Background()))
...
func TestAirpurifierNodes_Registered(t *testing.T) {
	t.Parallel()
	r := NewRegistry()

	for _, name := range []string{
		"airpurifier-status", "airpurifier-control",
		"airpurifier_status", "airpurifier_control", // `_` 별칭
	} {
		assert.True(t, r.Has(name), "레지스트리는 %q 를 포함해야 한다", name)
	}

	// canonical 이름으로 노드 생성 시 올바른 타입이 나온다.
	sn, err := r.Create(flow.NewNodeDef("s", "airpurifier-status"))
	require.NoError(t, err)
	_, ok := sn.(*AirpurifierStatusNode)
	assert.True(t, ok, "airpurifier-status 는 *AirpurifierStatusNode 를 생성해야 한다")

	cn, err := r.Create(flow.NewNodeDef("c", "airpurifier-control"))
	require.NoError(t, err)
	_, ok = cn.(*AirpurifierControlNode)
	assert.True(t, ok, "airpurifier-control 은 *AirpurifierControlNode 를 생성해야 한다")

	// `_` 별칭으로도 동일 타입이 나온다.
	snAlias, err := r.Create(flow.NewNodeDef("s2", "airpurifier_status"))
	require.NoError(t, err)
	_, ok = snAlias.(*AirpurifierStatusNode)
	assert.True(t, ok, "airpurifier_status 별칭은 *AirpurifierStatusNode 를 생성해야 한다")
}
