...
//   - mode / fan / setpoint_c / current_temp_c    : Confirmed
//   - live_13 / live_14 / live_15                 : Inferred
...
//
// 2026-05-29 current_temp byte 위치 정정 (실측 검증):
//
//	사용자 OFF↔ON 통제 캡처 (실측 ambient 26°C) 로 data[7..8] LE u16 ÷ 10 = 현재
//	실내 온도 임이 확정되었다 (0x0104 = 260 → 26.0°C, ambient 와 정확히 일치).
//	이전엔 data[7..8] 을 "cooling capacity ceiling / max compressor speed" 운전
//	파라미터로 추정했으나 (6-point 실험 시 240-250 관측), 실측에서 그 값들은
//	실험 중 AC 가 식히던 실내 ambient 였음이 확인되었다.
//	current_temp 의 원천이 Reg04 의 TempAC (data[10..11]) 에서 Reg02 의 CurrentTempC
//	(data[7..8]) 로 이전되었다 — Reg04 data[10..11] 은 ambient 와 무관한 운전 변수
//	(reg04_word_10 으로 노출, 실제 의미 TBD).
//
...
func DecodeReg02(f *Frame, tsMs int64, direction string) (*Reg02Decoded, error) {
	...
	// data[7..8] 현재 실내 온도 LE u16 ÷ 10 (실측 검증, 2026-05-29 — OFF↔ON 캡처로 ambient 와 일치 확인).
	currentTempRaw := binary.LittleEndian.Uint16(data[7:9])
	...
	return &Reg02Decoded{
		...
		CurrentTempC: FieldFloat32{Value: float32(currentTempRaw) / 10.0, Raw: currentTempRaw, ConfirmationStatus: Confirmed},
