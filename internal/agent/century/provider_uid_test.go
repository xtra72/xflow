...
//
// 본 테스트는 hvacr01DeviceAdapter.UID() 가 다음을 만족함을 검증한다:
//
//   - 저장소 설정 시 UUID v4 반환 (idempotent, unique per sub_dev_id).
//   - 저장소 미설정 시 빈 문자열 (graceful degradation).
//   - localID 형식이 "0x%02X" — Hvacr01Agent.emit 경로의 ResolveDeviceID
...
func TestCenturyAdapter_UID_WithRepository(t *testing.T) {
	...
	a := newHvacr01DeviceAdapter("century", snap)
	...
	snap2 := snap
	snap2.SubDevID = 0x82
	a2 := newHvacr01DeviceAdapter("century", snap2)
	uid2 := a2.UID()
...
func TestCenturyAdapter_UID_MatchesEmitPath(t *testing.T) {
	...
	assert.Equal(t, uidFromAdapter, uidFromEmit,
		"hvacr01DeviceAdapter.UID() must yield the same UUID as the emit path's "+
