...
// MQTTClient 는 브로커 I/O 경계 추상화이다 (REQ-XSFM-001-01-03).
//
...
type MQTTClient interface {
	...
	// Unsubscribe 는 토픽 구독을 해제한다 (remove_device 배선, REQ-XSFM-001-02-04).
...
// 페이로드를 담는다 (REQ-XSFM-001-01-11/12, M14 다중 필드). Fields/Attribute 는 blob 모드에서
...
// CommandSink 는 명령 출력(egress) 경계 추상화이다 (REQ-XSFM-001-01-11).
//
...
func (s *portCommandSink) SendCommand(cmd outboundCommand) error {
	...
	default:
		if s.logger != nil && s.logDrop {
			s.logger.Warn("xsfm: control port full, dropping command", "device_id", cmd.DeviceID)
		}
		return nil
	}
}
...
func (c *pahoMQTTClient) logLifecycle(msg string, args ...any) {
	...
	c.logger.Info("xsfm mqtt: "+msg, args...)
}

// newPahoMQTTClient 는 설정으로 pahoMQTTClient 를 생성한다 (아직 연결하지 않는다).
func newPahoMQTTClient(cfg XSFMConfig, logger *slog.Logger) *pahoMQTTClient {
...
func (c *pahoMQTTClient) Connect() error {
	...
	if err != nil {
		return fmt.Errorf("xsfm: tls config: %w", err)
	}
	...
	opts.SetConnectionLostHandler(func(_ mqtt.Client, err error) {
		c.logger.Warn("xsfm: mqtt connection lost", "broker", c.cfg.Broker, "error", err)
	})
	...
	if !connected {
		if c.cfg.AutoReconnect {
			c.logger.Warn("xsfm: initial mqtt connect failed, retrying in background",
				"broker", c.cfg.Broker, "error", token.Error())
			return nil
		}
		if token.Error() != nil {
			return fmt.Errorf("xsfm: mqtt connect: %w", token.Error())
		}
		return fmt.Errorf("xsfm: mqtt connect timeout (%s)", c.cfg.Broker)
	}
...
func (c *pahoMQTTClient) Publish(topic string, qos byte, payload []byte) error {
	...
	if err := token.Error(); err != nil {
		return fmt.Errorf("xsfm: mqtt publish %q: %w", topic, err)
	}
...
func (c *pahoMQTTClient) Unsubscribe(topic string) error {
	...
	token.Wait()
	if err := token.Error(); err != nil {
		return fmt.Errorf("xsfm: mqtt unsubscribe %q: %w", topic, err)
	}
	return nil
}
...
// onConnect 는 (재)연결 성공 시 기록된 구독을 모두 복원한다 (REQ-XSFM-001-01 재연결 복원).
func (c *pahoMQTTClient) onConnect(_ mqtt.Client) {
	...
	for topic, e := range restore {
		if err := c.doSubscribe(topic, e.qos, e.cb); err != nil {
			c.logger.Error("xsfm: subscription restore failed", "topic", topic, "error", err)
			continue
		}
	}
...
func (c *pahoMQTTClient) doSubscribe(topic string, qos byte, cb MessageHandler) error {
	...
	token.Wait()
	if err := token.Error(); err != nil {
		return fmt.Errorf("xsfm: mqtt subscribe %q: %w", topic, err)
	}
...
func (c *pahoMQTTClient) buildTLSConfig() (*tls.Config, error) {
	...
	if !strings.Contains(c.cfg.CACert, "-----BEGIN") {
		...
		if err != nil {
			return nil, fmt.Errorf("xsfm: read ca_cert: %w", err)
		}
		pemData = data
	}
	pool := x509.NewCertPool()
	if !pool.AppendCertsFromPEM(pemData) {
		return nil, fmt.Errorf("xsfm: parse ca_cert PEM failed")
	}
