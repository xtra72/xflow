...
func directAgentWithMock(t *testing.T, opts map[string]any) (*XSFMAgent, *mockMQTTClient) {
	t.Helper()
...
func readEvent(t *testing.T, ap *XSFMAgent) map[string]any {
	t.Helper()
	select {
	case b := <-ap.msgCh:
...
func TestAddDevice_BridgeSubscribeEvent(t *testing.T) {
	...
	assert.Contains(t, mock.subscribedTopics(), "xsfm/+/state", "와일드카드 state 토픽이 구독되어야 한다")
