...
type PayloadMapping struct {
	...
	// 바뀐다(스큐 주의 — XSFMConfig.LivenessSource 참조).
	TimeField string
}
...
// (REQ-XSFM-001-01-05, 다중 필드 일반화). {device_id} 단일 필드 템플릿에는
...
// 페이로드를 산출한다 (REQ-XSFM-001-01-11, Scenario 1B.8).
func encodeCommandPayload(m PayloadMapping, cmd commandPayload) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("xsfm: encode command payload: %w", err)
	}
	return b, nil
}
...
// 콜백)·port(입력 포트) 두 모드가 동일한 이 경로를 거친다 (REQ-XSFM-001-01-11).
func decodeStatePayload(m PayloadMapping, raw []byte) (decodedState, error) {
	var obj map[string]any
	if err := json.Unmarshal(raw, &obj); err != nil {
		return decodedState{}, fmt.Errorf("xsfm: decode state payload: %w", err)
	}
	var st decodedState
