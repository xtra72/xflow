...
// StationRegistryEntry 는 역사 레지스트리 항목 타입이다 (REQ-XSFM-001-02-09).
//
// 저장소(storage.StationRegistryEntry)와 동일 구조를 재사용하여 station→line 매핑을
// device_metadata 와 별개 저장소로 관리한다 (REQ-XSFM-001-02-12). 별칭이므로 B4/저장소
// 간 변환 없이 그대로 흐른다.
...
// (REQ-XSFM-001-02-11). 저장소(repo)를 감싸며 자체 RWMutex 로 보호된다.
//
...
// ---------------------------------------------------------------------------
// Module 2B — 역사 레지스트리 런타임 CRUD 명령 핸들러 (REQ-XSFM-001-02-11)
// ---------------------------------------------------------------------------

// handleAddStation 는 add_station 명령을 처리한다 (station→line, 표시명, 정렬 upsert).
// 저장소 경로가 설정된 경우 write-through 로 영속화된다.
func (a *XSFMAgent) handleAddStation(req processRequest) ([]byte, error) {
	if req.Station == "" {
...
func (a *XSFMAgent) handleRemoveStation(req processRequest) ([]byte, error) {
	if req.Station == "" {
...
func (a *XSFMAgent) handleListStations() ([]byte, error) {
...
func (a *XSFMAgent) handleAddPlace(req processRequest) ([]byte, error) {
	if req.Station == "" {
...
func (a *XSFMAgent) handleRemovePlace(req processRequest) ([]byte, error) {
	if req.Station == "" {
...
func (a *XSFMAgent) handleListPlaces(req processRequest) ([]byte, error) {
	if req.Station == "" {
...
// ---------------------------------------------------------------------------
// 셀렉터 타겟 도출 시임 (B4 소비, REQ-XSFM-001-02-13)
// ---------------------------------------------------------------------------
...
func (a *XSFMAgent) DevicesByStation(station string) []string {
	a.mu.RLock()
	defer a.mu.RUnlock()
	var ids []string
...
func (a *XSFMAgent) DevicesByLine(line string) (targets, excluded []string) {
