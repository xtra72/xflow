...
func drainEvents(t *testing.T, ap *XSFMAgent, deadline time.Duration) []map[string]any {
	t.Helper()
...
func newDirectAgentWithMock(t *testing.T, opts map[string]any) (*XSFMAgent, *mockMQTTClient) {
	t.Helper()
...
func TestMonitor_ObservedGatedEmit(t *testing.T) {
	...
	mock.deliver("xsfm/ap-101/state", []byte(`{"power":false,"fan_speed":3}`))
	events = drainEvents(t, ap, 100*time.Millisecond)
	evt = firstEventOfType(events, "device_state_changed")
	require.NotNil(t, evt)
...
func TestMonitor_WildcardSubscriptionModel(t *testing.T) {
	...
	assert.ElementsMatch(t, []string{"xsfm/+/state"}, mock.subscribedTopics())

	// 런타임 등록(add_device) 은 디바이스별 구독을 추가하지 않는다.
	_, err := ap.Process([]byte(`{"command":"add_device","device_id":"ap-201"}`))
	require.NoError(t, err)
	assert.NotContains(t, mock.subscribedTopics(), "xsfm/ap-201/state",
		"M14: 디바이스별 구독을 하지 않는다")

	// 그럼에도 ap-201 의 실제 토픽 유입은 와일드카드 콜백으로 라우팅되어 로스터를 갱신한다.
	mock.deliver("xsfm/ap-201/state", []byte(`{"power":true}`))
	...
	require.NoError(t, err)
	assert.NotContains(t, mock.unsubscribedTopics(), "xsfm/ap-201/state",
		"M14: 디바이스별 Unsubscribe 를 하지 않는다")
	assert.Contains(t, mock.subscribedTopics(), "xsfm/+/state", "와일드카드 구독은 유지된다")
}
