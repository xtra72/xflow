...
func TestLogMessages_RXGating(t *testing.T) {
	...
	aOn, err := NewAirPurifierAgent(agentConfigWithLogger(optsOn, logOn))
	require.NoError(t, err)
	apOn := asAP(t, aOn)
	bufOn.Reset() // 초기화 로그 제거 후 관측.
	apOn.FeedState("dev-1", payload)
	assert.Contains(t, bufOn.String(), "airpurifier: RX", "log_messages=true 이면 RX 프레임 로그가 남아야 한다")
	assert.Contains(t, bufOn.String(), "power=true")

	// OFF: RX 로그가 없어야 한다 (zero overhead).
	logOff, bufOff := captureLogger()
	aOff, err := NewAirPurifierAgent(agentConfigWithLogger(portOpts(), logOff))
	require.NoError(t, err)
	apOff := asAP(t, aOff)
	bufOff.Reset()
	apOff.FeedState("dev-1", payload)
	assert.NotContains(t, bufOff.String(), "airpurifier: RX", "log_messages=false 이면 RX 프레임 로그가 없어야 한다")
}
...
func TestLogMessages_TXGating(t *testing.T) {
	...
	newAgent := func(logMessages bool) (*AirPurifierAgent, *bytes.Buffer) {
		...
		a, err := NewAirPurifierAgent(agentConfigWithLogger(opts, logger))
		require.NoError(t, err)
		return asAP(t, a), buf
	}
	...
	require.NoError(t, err)
	assert.Contains(t, bufOn.String(), "airpurifier: TX", "log_messages=true 이면 TX 프레임 로그가 남아야 한다")
	...
	require.NoError(t, err)
	assert.NotContains(t, bufOff.String(), "airpurifier: TX", "log_messages=false 이면 TX 프레임 로그가 없어야 한다")
}
...
func TestLogMessages_TXAttributeMode(t *testing.T) {
	...
	got := buf.String()
	assert.Contains(t, got, "airpurifier: TX")
...
func TestLogMQTT_LifecycleGating(t *testing.T) {
	// OFF: 생명주기 로그 없음.
	logOff, bufOff := captureLogger()
	cOff := newPahoMQTTClient(AirPurifierConfig{LogMQTT: false}, logOff)
	...
	// ON: 생명주기 로그 출력.
	logOn, bufOn := captureLogger()
	cOn := newPahoMQTTClient(AirPurifierConfig{LogMQTT: true}, logOn)
	cOn.logLifecycle("connect succeeded", "broker", "tcp://x:1883")
	assert.Contains(t, bufOn.String(), "airpurifier mqtt: connect succeeded")
}
