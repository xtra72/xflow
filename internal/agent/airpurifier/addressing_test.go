...
func attrAgentWithMock(t *testing.T, opts map[string]any) (*AirPurifierAgent, *mockMQTTClient) {
	t.Helper()
...
func TestConfig_MultiFieldTemplateAcceptedZeroPlaceholderRejected(t *testing.T) {
	...
	cfg, err := parseAirPurifierConfig(attrOpts())
	require.NoError(t, err)
...
func TestAttr_BackwardCompatBlobMode(t *testing.T) {
	opts := directOpts() // airpurifier/{device_id}/state, JSON blob.
