...
// (REQ-AIRPUR-001-05-03, 양 모드 공통). offline_timeout<=0 이면 감지가 비활성이므로 고루틴을
...
func (a *AirPurifierAgent) startOfflineMonitor() {
...
func (a *AirPurifierAgent) checkOfflineDevices() {
...
// device_offline 이벤트를 방출한다 (REQ-AIRPUR-001-05-03 path 1, direct 모드 전용).
//
...
func (a *AirPurifierAgent) handleLWTOffline(deviceID string) {
	a.mu.Lock()
