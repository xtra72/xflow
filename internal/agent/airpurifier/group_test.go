...
func TestGroup_FanOutAllMembers(t *testing.T) {
	...
	assert.ElementsMatch(t,
		[]string{"airpurifier/ap-101/cmd", "airpurifier/ap-102/cmd", "airpurifier/ap-103/cmd"},
...
func TestGroup_PartialFailureBestEffort(t *testing.T) {
	...
	ap, mock := directAgentWithMock(t, opts)
	mock.failPublish("airpurifier/ap-102/cmd") // ap-102 발행 실패.
	...
	assert.ElementsMatch(t,
		[]string{"airpurifier/ap-101/cmd", "airpurifier/ap-103/cmd"},
		mock.publishedTopics(),
	)
...
func TestGroup_StationSelector(t *testing.T) {
	...
	assert.ElementsMatch(t,
		[]string{"airpurifier/ap-101/cmd", "airpurifier/ap-102/cmd"},
		mock.publishedTopics(),
	)
	out := decodeFanOut(t, resp)
...
func TestGroup_SelectorResponseWaitAggregation(t *testing.T) {
	...
	ap, mock := directAgentWithMock(t, opts)
	mock.failPublish("airpurifier/ap-103/cmd")

	// ap-101 발행 관측 후 매칭 에코 주입 (register→emit 순서상 pending 이미 등록됨).
	go func() {
		for i := 0; i < 3000; i++ {
			if mock.hasPublished("airpurifier/ap-101/cmd") {
