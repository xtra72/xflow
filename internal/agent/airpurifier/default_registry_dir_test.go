...
// ---------------------------------------------------------------------------
// 기본 영속 경로 자동화 (SPEC-AIRPURIFIER-001)
//
// SetDefaultRegistryDir 로 서버 데이터 디렉터리를 주입하면, registry_path/
// station_registry_path 설정이 비어 있어도 <dataDir>/airpurifier/<agentID>/ 아래로
...
func perAgentDir(base string) string {
	return filepath.Join(base, "airpurifier", "ap-agent-1")
}
...
func TestDefaultRegistryDir_StationPersistAndRestore(t *testing.T) {
	...
	a1, err := NewAirPurifierAgent(baseAgentConfig(portOpts()))
	...
	a2, err := NewAirPurifierAgent(baseAgentConfig(portOpts()))
	require.NoError(t, err)
	ap2 := asAP(t, a2)
...
func mustProcessOK(t *testing.T, ap *AirPurifierAgent, cmd string) error {
	t.Helper()
