...
// ---------------------------------------------------------------------------
// B7 — 제어 감사 배선 (REQ-AIRPUR-001-06-03)
// ---------------------------------------------------------------------------
//
...
func (a *AirPurifierAgent) auditActor() string {
	a.mu.RLock()
	defer a.mu.RUnlock()
...
// recordControlAudit 는 단일 디바이스 제어 감사 레코드를 기록한다(REQ-AIRPUR-001-06-03).
//
...
func (a *AirPurifierAgent) recordControlAudit(res memberResult) {
	...
	if err := repo.Append(context.Background(), rec); err != nil {
		a.logger.Warn("airpurifier: control audit append failed", "device_id", res.DeviceID, "error", err)
	}
}

// recordGroupAudit 는 셀렉터 fan-out 의 그룹 감사 레코드를 기록한다(REQ-AIRPUR-001-06-03 그룹 절).
//
...
func (a *AirPurifierAgent) recordGroupAudit(sel selectorRef, results []groupResult) {
	repo := getAuditRepository()
	if repo == nil {
		return
	}
	...
	if err := repo.Append(context.Background(), rec); err != nil {
		a.logger.Warn("airpurifier: group audit append failed", "selector", sel.Value, "error", err)
	}
}
