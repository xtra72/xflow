...
// ---------------------------------------------------------------------------
// B4 — 셀렉터 fan-out (그룹/역사/호선 일괄 제어, REQ-AIRPUR-001-04-01..09)
// ---------------------------------------------------------------------------
//
...
// groupResult 는 fan-out 집계 응답의 멤버별 결과이다 (REQ-AIRPUR-001-04-07).
...
// fanOutResponse 는 셀렉터 fan-out 집계 응답이다 (REQ-AIRPUR-001-04-07).
//
...
// device_id 목록이다 (REQ-AIRPUR-001-02-13/04-09).
...
// 라우팅한다 (REQ-AIRPUR-001-04-04 우선순위).
//
...
func (a *AirPurifierAgent) dispatchControl(req processRequest) ([]byte, error) {
	if req.DeviceID != "" {
		switch req.Command {
		case "set_power":
...
//
// 셀렉터 우선순위(REQ-AIRPUR-001-04-04): device_id > station > line > group_id. device_id 는
...
func (a *AirPurifierAgent) handleSelectorControl(req processRequest) ([]byte, error) {
	var sel selectorRef
...
// 되므로 동시 쓰기 race 가 없다.
func (a *AirPurifierAgent) fanOutControl(sel selectorRef, targets, excluded []string, plan []controlStep) ([]byte, error) {
	if len(targets) == 0 {
...
func (a *AirPurifierAgent) executeMemberPlan(deviceID string, plan []controlStep) groupResult {
...
// REQ-AIRPUR-001-06-03 그룹 절). 구현은 audit.go 에 있다 — 셀렉터 종류·값 + fan-out 멤버
