...
// asAP 는 agent.Agent 를 *AirPurifierAgent 로 캐스팅한다 (in-package 테스트).
func asAP(t *testing.T, a agent.Agent) *AirPurifierAgent {
	t.Helper()
	ap, ok := a.(*AirPurifierAgent)
	require.True(t, ok, "expected *AirPurifierAgent")
	return ap
}
...
func TestParseConfig_MissingPlaceholder(t *testing.T) {
	opts := directOpts()
	opts["state_topic_template"] = "airpurifier/state" // {device_id} 없음
...
func TestParseConfig_BrokerRequiredDirect(t *testing.T) {
	...
	opts2 := directOpts()
	delete(opts2, "broker")
	_, err = parseAirPurifierConfig(opts2)
	assert.ErrorIs(t, err, ErrBrokerRequired)
}
...
func TestAccessors(t *testing.T) {
	...
	assert.Equal(t, "airpurifier-1", ap.Name())
	assert.Equal(t, "airpurifier", ap.Type())
