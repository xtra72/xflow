...
// ---------------------------------------------------------------------------
// B7 — 로스터 영속화 라운드트립 (REQ-AIRPUR-001-02-05 / 02-08)
// ---------------------------------------------------------------------------
//
...
//
// 이는 airpurifier 로스터 전용 저장소이며, 설정-디바이스 메커니즘(parseConfigDevices)과 별개다.
...
// 주소(UnitID)를 글로벌 device_id(UUID)로 매핑하기 위해 device_id 저장소를 쓴다. airpurifier 의
// device_id 는 사용자가 부여한 글로벌 식별자이자 로스터 키 자체이며 프로토콜 주소에서 도출되지
// 않으므로, 그 매핑 라운드트립은 적용되지 않는다(device_id 를 그대로 저장/복원한다).

// persistedDevice 는 registry_path 저장소의 디바이스 항목이다(REQ-AIRPUR-001-02-05/08).
//
...
// (REQ-AIRPUR-001-02-05/08). Init 에서 호출한다. 설정 기반 디바이스(NewAirPurifierAgent 에서
// 선등록)와 device_id 가 충돌하면 설정이 우선하여 덮어쓰지 않는다(config precedence). 복원된
// 디바이스는 저장된 Source 를 유지한다(재시작 후에도 "bridge" → 삭제 가능성 보존).
func (a *AirPurifierAgent) loadPersistedRoster() error {
	if a.registry == nil {
		return nil
	}
...
func (a *AirPurifierAgent) persistRoster() {
	if a.registry == nil {
		return
	}
	...
	if err := a.registry.save(snapshot); err != nil {
		a.logger.Warn("airpurifier: persist roster failed", "error", err)
	}
}
...
//
// airpurifier 의 device_id 는 정체성(주소 = 안정 식별자)이므로 Address/Name 슬롯에 device_id 를,
...
func (a *AirPurifierAgent) GetPersistableDevices() []agent.DeviceEntry {
	a.mu.RLock()
	defer a.mu.RUnlock()
	...
	for id, dev := range a.devices {
		...
		result = append(result, agent.DeviceEntry{
			Address:     id,       // device_id = airpurifier 정체성(주소 = 식별자)
			Name:        id,       // 안정 식별자 슬롯
