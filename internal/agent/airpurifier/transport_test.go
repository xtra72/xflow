...
func TestBuildTLSConfig(t *testing.T) {
	...
	// TLS 미사용 → nil.
	c := newPahoMQTTClient(AirPurifierConfig{TLS: false}, log)
	tc, err := c.buildTLSConfig()
	require.NoError(t, err)
	assert.Nil(t, tc)

	// TLS 사용, CA 없음 → 기본 config.
	c = newPahoMQTTClient(AirPurifierConfig{TLS: true}, log)
	tc, err = c.buildTLSConfig()
	require.NoError(t, err)
	require.NotNil(t, tc)
	...
	c = newPahoMQTTClient(AirPurifierConfig{TLS: true, CACert: genCertPEM(t)}, log)
	tc, err = c.buildTLSConfig()
	require.NoError(t, err)
	require.NotNil(t, tc)
	...
	c = newPahoMQTTClient(AirPurifierConfig{TLS: true, CACert: brokenPEM}, log)
	_, err = c.buildTLSConfig()
	assert.Error(t, err)

	// TLS 사용, 파일 경로 (없는 파일) → 에러.
	c = newPahoMQTTClient(AirPurifierConfig{TLS: true, CACert: "/nonexistent/ca.pem"}, log)
	_, err = c.buildTLSConfig()
	assert.Error(t, err)
}
...
func TestPahoClient_Unconnected(t *testing.T) {
	c := newPahoMQTTClient(AirPurifierConfig{Broker: "tcp://127.0.0.1:1"}, slog.Default())
	assert.False(t, c.IsConnected())
	assert.ErrorIs(t, c.Publish("t", 1, []byte("x")), ErrNotConnected)

	// 미연결 Subscribe 는 재연결 복원용으로 기록만 하고 nil 반환.
	called := false
	require.NoError(t, c.Subscribe("airpurifier/ap-1/state", 1, func(_ string, _ []byte) { called = true }))
	assert.False(t, called)
	c.mu.Lock()
	_, ok := c.subs["airpurifier/ap-1/state"]
	c.mu.Unlock()
