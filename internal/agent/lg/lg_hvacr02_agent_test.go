...
// ---------------------------------------------------------------------------
// hvacr02MockTransport: LGAPTransport 인터페이스 목 구현
// ---------------------------------------------------------------------------

// hvacr02MockTransport 는 테스트용 LGAPTransport 목 구현이다.
type hvacr02MockTransport struct {
	mu        sync.Mutex
	buf       *bytes.Buffer
	opened    atomic.Bool
	openErr   error
...
func newHvacr02MockTransport() *hvacr02MockTransport {
	return &hvacr02MockTransport{
		buf: bytes.NewBuffer(nil),
	}
}

func (m *hvacr02MockTransport) Open() error {
	m.openCount.Add(1)
	if m.openErr != nil {
		return m.openErr
	}
...
func (m *hvacr02MockTransport) Close() error {
	m.opened.Store(false)
	return m.closeErr
}

func (m *hvacr02MockTransport) Send(_ []byte) error {
	// LG HVACR-02 에이전트는 절대 Send 를 호출하지 않는다
	return nil
}

func (m *hvacr02MockTransport) Receive(buf []byte) (int, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	if m.buf.Len() == 0 {
		return 0, io.EOF
...
func (m *hvacr02MockTransport) Available() bool {
	return m.opened.Load()
}

func (m *hvacr02MockTransport) Write(data []byte) (int, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *hvacr02MockTransport) feedData(data []byte) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.buf.Write(data)
}
...
// newTestHvacr02Agent 는 테스트용 LG HVACR-02 에이전트를 생성한다.
func newTestHvacr02Agent(t *testing.T, transport LGAPTransport) *Hvacr02Agent {
	t.Helper()

	config := agent.AgentConfig{
		ID:   "test-lg_hvacr02-001",
		Name: "test-lg_hvacr02-capture",
		Type: "lg_hvacr02",
...
func runCaptureLoopAndWait(a *Hvacr02Agent, captureFor time.Duration) {
...
// buildIcp02Frame 은 테스트용 최소 LG ICP-02 프레임을 생성한다.
// STX(0x56) + LEN + DLEN + DA + SLEN + SA + CMD(2) + SEQ0 + PLEN + SEQ1 + CRC(2)
func buildIcp02Frame(da, sa []byte, cmd [2]byte, payload []byte) []byte {
...
// TestNewHvacr02Agent_Factory 는 팩토리 함수가 올바르게 에이전트를 생성하는지 검증한다.
func TestNewHvacr02Agent_Factory(t *testing.T) {
	...
	a, err := NewHvacr02Agent(config)
	if err != nil {
		t.Fatalf("NewHvacr02Agent() error = %v", err)
	}
	if a == nil {
		t.Fatal("NewHvacr02Agent() returned nil agent")
	}
	if a.ID() != "lg_hvacr02-001" {
		t.Errorf("ID() = %q, want %q", a.ID(), "lg_hvacr02-001")
	}
	if a.Name() != "lg_hvacr02-capture" {
		t.Errorf("Name() = %q, want %q", a.Name(), "lg_hvacr02-capture")
	}
	if a.Type() != "lg_hvacr02" {
		t.Errorf("Type() = %q, want %q", a.Type(), "lg_hvacr02")
	}
}

// TestNewHvacr02Agent_MissingSerialPort 는 필수 설정 누락 시 에러를 반환하는지 검증한다.
func TestNewHvacr02Agent_MissingSerialPort(t *testing.T) {
	config := agent.AgentConfig{
		ID:   "lg_hvacr02-001",
		Name: "lg_hvacr02-capture",
		Type: "lg_hvacr02",
	...
	_, err := NewHvacr02Agent(config)
	if err == nil {
		t.Fatal("NewHvacr02Agent() expected error for missing serial_port, got nil")
	}
}

// TestHvacr02Agent_CaptureLoop 는 캡처 루프가 프레임을 올바르게 캡처하는지 검증한다.
func TestHvacr02Agent_CaptureLoop(t *testing.T) {
	...
	frame := buildIcp02Frame(da, sa, cmd, payload)
	...
	if snap.Extra == nil {
		t.Error("Stats.Extra should contain LG HVACR-02 specific stats")
	}
}

// TestHvacr02Agent_CaptureLoop_MultipleFrames 는 연속 프레임 캡처를 검증한다.
func TestHvacr02Agent_CaptureLoop_MultipleFrames(t *testing.T) {
	mock := newHvacr02MockTransport()
	...
	// 3개 프레임 주입
	for i := 0; i < 3; i++ {
		frame := buildIcp02Frame(da, sa, cmd, []byte{byte(i)})
		mock.feedData(frame)
	}
...
// TestHvacr02Agent_MsgChDrop 는 msgCh 가 가득 찼을 때 드롭하는지 검증한다.
func TestHvacr02Agent_MsgChDrop(t *testing.T) {
	mock := newHvacr02MockTransport()
...
// TestHvacr02Agent_Process_GetStats 는 get_stats 명령이 올바른 통계를 반환하는지 검증한다.
func TestHvacr02Agent_Process_GetStats(t *testing.T) {
	mock := newHvacr02MockTransport()
	mock.opened.Store(true)
...
// TestHvacr02Agent_Process_GetRecent 는 get_recent 명령이 최근 프레임을 반환하는지 검증한다.
func TestHvacr02Agent_Process_GetRecent(t *testing.T) {
	mock := newHvacr02MockTransport()
	mock.opened.Store(true)
...
// TestHvacr02Agent_Process_GetRecent_CountZeroDrains 는 v0.7.1 의미를 검증한다:
//
//	count 미지정 (JSON zero-value = 0) → drain (전체 반환 + 버퍼 비움).
//
// 이전 v0.7.0: count 미지정 시 default 10 개. v0.7.1 부터 drain 의미로 통일.
func TestHvacr02Agent_Process_GetRecent_CountZeroDrains(t *testing.T) {
	mock := newHvacr02MockTransport()
	mock.opened.Store(true)
	a := newTestHvacr02Agent(t, mock)
...
// TestHvacr02Agent_Process_GetRecent_ExplicitCount 는 명시적 count > 0 일 때
// 그 수만큼만 반환하는지 검증한다 (비파괴).
func TestHvacr02Agent_Process_GetRecent_ExplicitCount(t *testing.T) {
	mock := newHvacr02MockTransport()
	mock.opened.Store(true)
	a := newTestHvacr02Agent(t, mock)
...
// TestHvacr02Agent_Process_Drain 은 drain 커맨드가 프레임을 반환하고 버퍼를 비우는지 검증한다.
func TestHvacr02Agent_Process_Drain(t *testing.T) {
	mock := newHvacr02MockTransport()
	mock.opened.Store(true)
	a := newTestHvacr02Agent(t, mock)
...
// TestHvacr02Agent_Process_Drain_Default 는 drain에 count 미지정 시 전체 버퍼를 반환하는지 검증한다.
func TestHvacr02Agent_Process_Drain_Default(t *testing.T) {
	mock := newHvacr02MockTransport()
	mock.opened.Store(true)
	a := newTestHvacr02Agent(t, mock)
...
// TestHvacr02Agent_Process_UnsupportedCommand 는 지원되지 않는 명령에 대해 에러를 반환하는지 검증한다.
func TestHvacr02Agent_Process_UnsupportedCommand(t *testing.T) {
	tests := []struct {
		name    string
		command string
	}{
...
// TestHvacr02Agent_Process_InvalidJSON 은 잘못된 JSON 요청에 대해 에러를 반환하는지 검증한다.
func TestHvacr02Agent_Process_InvalidJSON(t *testing.T) {
	mock := newHvacr02MockTransport()
	a := newTestHvacr02Agent(t, mock)
...
// TestHvacr02Agent_ReceiveMessage 은 ReceiveMessage 가 msgCh 에서 메시지를 수신하는지 검증한다.
func TestHvacr02Agent_ReceiveMessage(t *testing.T) {
	mock := newHvacr02MockTransport()
	a := newTestHvacr02Agent(t, mock)
...
// TestHvacr02Agent_ReceiveMessage_ContextCancel 은 컨텍스트 취소 시 에러를 반환하는지 검증한다.
func TestHvacr02Agent_ReceiveMessage_ContextCancel(t *testing.T) {
	mock := newHvacr02MockTransport()
	a := newTestHvacr02Agent(t, mock)
...
// TestHvacr02Agent_State 는 State() 가 올바른 캡처 상태를 반환하는지 검증한다.
func TestHvacr02Agent_State(t *testing.T) {
	mock := newHvacr02MockTransport()
	mock.opened.Store(true)
	a := newTestHvacr02Agent(t, mock)
...
// TestHvacr02Agent_BufferInfo 는 BufferInfo() 가 올바른 버퍼 정보를 반환하는지 검증한다.
func TestHvacr02Agent_BufferInfo(t *testing.T) {
	mock := newHvacr02MockTransport()
	a := newTestHvacr02Agent(t, mock)
...
// TestHvacr02Agent_TransportConnected 는 TransportConnected() 가 트랜스포트 상태를 반환하는지 검증한다.
func TestHvacr02Agent_TransportConnected(t *testing.T) {
	mock := newHvacr02MockTransport()
	a := newTestHvacr02Agent(t, mock)
...
// TestHvacr02Agent_PassiveOnly 는 에이전트가 절대로 Send() 를 호출하지 않는지 검증한다.
// Send 호출 시 panic 을 발생시키는 트랜스포트를 사용하여 검증한다.
func TestHvacr02Agent_PassiveOnly(t *testing.T) {
	...
	sa := []byte{0x10}
	cmd := [2]byte{0x40, 0x80}
	frame := buildIcp02Frame(da, sa, cmd, nil)
	mock.feedData(frame)
...
// TestHvacr02Agent_RecentBuffer_Overflow 는 링 버퍼가 가득 찼을 때 올바르게 순환하는지 검증한다.
func TestHvacr02Agent_RecentBuffer_Overflow(t *testing.T) {
	mock := newHvacr02MockTransport()
	a := newTestHvacr02Agent(t, mock)

	// hvacr02RecentBufferSize (64) 보다 많은 프레임 주입
	...
	// 최대 hvacr02RecentBufferSize 개까지만 반환
	count := int(result["count"].(float64))
	if count != hvacr02RecentBufferSize {
		t.Errorf("count = %d, want %d", count, hvacr02RecentBufferSize)
	}
...
// TestRegisterLGHvacr02Types 는 LG HVACR-02 에이전트 타입 등록을 검증한다.
func TestRegisterLGHvacr02Types(t *testing.T) {
	mgr := agent.NewManager()

	err := RegisterLGHvacr02Types(mgr)
	if err != nil {
		t.Fatalf("RegisterLGHvacr02Types() error = %v", err)
	}
}

// TestRegisterLGHvacr02Types_Duplicate 는 중복 등록 시 에러를 반환하는지 검증한다.
func TestRegisterLGHvacr02Types_Duplicate(t *testing.T) {
	mgr := agent.NewManager()
...
// TestNewHvacr02Transport_Serial 은 serial 타입으로 시리얼 트랜스포트가 생성되는지 검증한다.
func TestNewHvacr02Transport_Serial(t *testing.T) {
	cfg := Hvacr02Config{
	...
	tr, err := newHvacr02Transport(cfg)
	if err != nil {
		t.Fatalf("newHvacr02Transport() error: %v", err)
	}
	if _, ok := tr.(*lgapSerialTransport); !ok {
		t.Errorf("newHvacr02Transport(serial) type = %T, want *lgapSerialTransport", tr)
	}
}

// TestNewHvacr02Transport_TCPClient 는 tcp-client 타입으로 TCP 클라이언트 트랜스포트가 생성되는지 검증한다.
func TestNewHvacr02Transport_TCPClient(t *testing.T) {
	cfg := Hvacr02Config{
	...
	if _, ok := tr.(*lgapTCPClientTransport); !ok {
		t.Errorf("newHvacr02Transport(tcp-client) type = %T, want *lgapTCPClientTransport", tr)
	}
}

// TestNewHvacr02Transport_TCPServer 는 tcp-server 타입으로 TCP 서버 트랜스포트가 생성되는지 검증한다.
func TestNewHvacr02Transport_TCPServer(t *testing.T) {
	cfg := Hvacr02Config{
	...
	if _, ok := tr.(*lgapTCPServerTransport); !ok {
		t.Errorf("newHvacr02Transport(tcp-server) type = %T, want *lgapTCPServerTransport", tr)
	}
}

// TestNewHvacr02Transport_Unknown 은 알 수 없는 타입에 대한 에러를 검증한다.
func TestNewHvacr02Transport_Unknown(t *testing.T) {
	cfg := Hvacr02Config{
		TransportType: "websocket",
	}

	_, err := newHvacr02Transport(cfg)
	if err == nil {
		t.Fatal("newHvacr02Transport() expected error for unknown type, got nil")
	}
...
// TestNewHvacr02Agent_TCPClient 는 tcp-client 설정으로 에이전트 생성을 검증한다.
func TestNewHvacr02Agent_TCPClient(t *testing.T) {
	config := agent.AgentConfig{
		ID:   "lg_hvacr02-tcp-001",
		Name: "lg_hvacr02-tcp-client",
		Type: "lg_hvacr02",
		Transport: agent.TransportConfig{
			Type: "tcp",
			Options: map[string]any{
	...
	if a.ID() != "lg_hvacr02-tcp-001" {
		t.Errorf("ID() = %q, want %q", a.ID(), "lg_hvacr02-tcp-001")
	}
}

// TestNewHvacr02Agent_TCPServer 는 tcp-server 설정으로 에이전트 생성을 검증한다.
func TestNewHvacr02Agent_TCPServer(t *testing.T) {
	config := agent.AgentConfig{
		ID:   "lg_hvacr02-tcp-002",
		Name: "lg_hvacr02-tcp-server",
		Type: "lg_hvacr02",
		Transport: agent.TransportConfig{
			Type: "tcp",
			Options: map[string]any{
...
// TestNewHvacr02Agent_UnknownTransportType 은 알 수 없는 transport_type 에 대한 에러를 검증한다.
func TestNewHvacr02Agent_UnknownTransportType(t *testing.T) {
	config := agent.AgentConfig{
		ID:   "lg_hvacr02-bad",
		Name: "lg_hvacr02-bad",
		Type: "lg_hvacr02",
	...
	_, err := NewHvacr02Agent(config)
	if err == nil {
		t.Fatal("NewHvacr02Agent() expected error for unknown transport_type, got nil")
	}
}
