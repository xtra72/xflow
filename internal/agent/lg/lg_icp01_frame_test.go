...
var icp01TestODU_SEQ01, _ = hex.DecodeString("58010000000000000057660000002e00190143" + "1d")

// TYPE-A SEQ=02 체크섬 없음, 실외 온도 포함
// 58 02 e0 06 02 00 81 4d 9f 00 31 cd e5 00 6b 5c 0f 00 f8 ee
var icp01TestODU_SEQ02, _ = hex.DecodeString("580200060200814d9f0031cde5006b5c0f00f8ee")

// TYPE-A SEQ=04 SUM 체크섬 테스트 벡터 (수정됨)
// 58 04 00 00 00 00 64 fe fe 00 84 00 00 00 00 00 00 00 15 55
var icp01TestODU_SEQ04, _ = hex.DecodeString("5804000000006464fe008400000000000000" + "1555")
...
func TestIcp01_ODUChecksum_SEQ01_XOR(t *testing.T) {
	t.Parallel()

	var raw [20]byte
	copy(raw[:], icp01TestODU_SEQ01)

	valid := icp01VerifyODUChecksum(raw, 0x01)
	assert.True(t, valid, "SEQ=01 XOR 체크섬이 유효해야 함")
}

func TestIcp01_ODUChecksum_SEQ02_NoChecksum(t *testing.T) {
	t.Parallel()

	var raw [20]byte
	copy(raw[:], icp01TestODU_SEQ02)

	valid := icp01VerifyODUChecksum(raw, 0x02)
	assert.True(t, valid, "SEQ=02는 체크섬 없음, 항상 유효")
}

func TestIcp01_ODUChecksum_SEQ03_NoChecksum(t *testing.T) {
	t.Parallel()
	...
	valid := icp01VerifyODUChecksum(raw, 0x03)
	assert.True(t, valid, "SEQ=03은 체크섬 없음, 항상 유효")
}

func TestIcp01_ODUChecksum_SEQ04_SUM(t *testing.T) {
	t.Parallel()
	...
	valid := icp01VerifyODUChecksum(raw, 0x04)
...
func TestIcp01_ODUChecksum_SEQ04_Fixed0x55Marker(t *testing.T) {
	t.Parallel()
...
func TestIcp01_ODUChecksum_SEQ01_Invalid(t *testing.T) {
	t.Parallel()
...
func TestIcp01_ODUChecksum_SEQ01_B13MarkerVariant(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			valid := icp01VerifyODUChecksum(tc.raw, 0x01)
			assert.True(t, valid, "SEQ=01 b[13]^0x1D marker variant 는 유효 처리")
		})
	}
}

func TestIcp01_ODUChecksum_SEQ05_XOR(t *testing.T) {
	t.Parallel()
	...
	valid := icp01VerifyODUChecksum(raw, 0x05)
...
func TestIcp01_IDURedundancy_Valid(t *testing.T) {
	t.Parallel()
	...
	valid := icp01VerifyIDURedundancy(raw)
	assert.True(t, valid, "이중 기록이 일치하면 유효해야 함")
}

func TestIcp01_IDURedundancy_Invalid_SlotNum(t *testing.T) {
	t.Parallel()
...
func TestIcp01_IDURedundancy_Invalid_RoomTemp(t *testing.T) {
	t.Parallel()
...
func TestIcp01_IDUStructure_Valid(t *testing.T) {
	t.Parallel()
	...
	valid := icp01VerifyIDUStructure(raw)
	assert.True(t, valid, "구조 검증이 통과해야 함")
}

func TestIcp01_IDUStructure_InvalidCMD(t *testing.T) {
	t.Parallel()
...
func TestIcp01_IDUStructure_InvalidB20(t *testing.T) {
	t.Parallel()
...
func TestIcp01_DecodeSensorTemp(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			result := icp01DecodeSensorTemp(tc.b)
			assert.Equal(t, tc.expected, result)
		})
	}
}

func TestIcp01_DecodeODUOutdoorTemp(t *testing.T) {
	t.Parallel()

	// SEQ=02 바이트[14]=0x6B → (0x6B-0x40)/2 = 21.5°C
	result := icp01DecodeODUOutdoorTemp(0x6B)
	assert.Equal(t, 21.5, result)

	// SEQ=02 바이트[15]=0x5C → (0x5C-0x40)/2 = 14.0°C
	result2 := icp01DecodeODUOutdoorTemp(0x5C)
...
func TestIcp01_IDURange_Valid(t *testing.T) {
	t.Parallel()
	...
	assert.True(t, icp01VerifyIDURange(f))
}

func TestIcp01_IDURange_RoomTempTooHigh(t *testing.T) {
	t.Parallel()
	...
	assert.False(t, icp01VerifyIDURange(f))
}

func TestIcp01_IDURange_InletTempTooHigh(t *testing.T) {
	t.Parallel()
...
func TestIcp01_FrameParser_ReadODUFrame(t *testing.T) {
	t.Parallel()

	reader := bytes.NewReader(icp01TestODU_SEQ01)
...
func TestIcp01_FrameParser_ReadIDUFrame(t *testing.T) {
	t.Parallel()
...
func TestIcp01_FrameParser_MixedStream(t *testing.T) {
	t.Parallel()

	// ODU 프레임 + IDU 프레임을 연결한 스트림
	iduRaw := buildTestIDUFrame()
	stream := append(icp01TestODU_SEQ01, iduRaw...)
...
func TestIcp01_FrameParser_SkipGarbage(t *testing.T) {
	t.Parallel()

	// 가비지 바이트 + ODU 프레임
	garbage := []byte{0x00, 0xFF, 0x12, 0x34}
	stream := append(garbage, icp01TestODU_SEQ01...)
	reader := bytes.NewReader(stream)
	parser := NewIcp01FrameParser(reader)
...
func TestIcp01_FrameParser_EOF(t *testing.T) {
	t.Parallel()
...
func TestIcp01_FrameParser_TruncatedODU(t *testing.T) {
	t.Parallel()
...
func TestIcp01_FrameParser_CMD_0x43(t *testing.T) {
	t.Parallel()
...
func TestIcp01_FrameParser_IDU_AllAddresses(t *testing.T) {
	t.Parallel()
...
func TestIcp01_ODUFrame_String(t *testing.T) {
	t.Parallel()

	var raw [20]byte
	copy(raw[:], icp01TestODU_SEQ01)

	f := &Icp01ODUFrame{Raw: raw, SEQ: 0x01, ChecksumValid: true}
	s := f.String()
	assert.Contains(t, s, "SEQ=1")
	assert.Contains(t, s, "checksum=true")
}

func TestIcp01_ODUFrame_String_Error(t *testing.T) {
	t.Parallel()

	f := &Icp01ODUFrame{ParseErr: io.ErrUnexpectedEOF}
	s := f.String()
	assert.Contains(t, s, "err=")
}

func TestIcp01_IDUFrame_String(t *testing.T) {
	t.Parallel()
...
func TestIcp01_IsValidCMD(t *testing.T) {
	t.Parallel()

	valid := []byte{0x00, 0x01, 0x02, 0x03, 0x06, 0x08, 0x09, 0x41, 0x43, 0x47, 0x49}
	for _, cmd := range valid {
		assert.True(t, icp01IsValidCMD(cmd), "CMD 0x%02X는 유효해야 함", cmd)
	}

	invalid := []byte{0xFF, 0x80, 0x10, 0x20, 0x30, 0x50, 0x60, 0x90, 0xA0}
	for _, cmd := range invalid {
		assert.False(t, icp01IsValidCMD(cmd), "CMD 0x%02X는 무효해야 함", cmd)
	}
}

func TestIcp01_IDUStructure_ExtendedCMD(t *testing.T) {
	t.Parallel()
...
func TestIcp01_IDUFrame_CMDBits(t *testing.T) {
	t.Parallel()
...
func TestIcp01_IDUFrame_SetTempReliable(t *testing.T) {
	t.Parallel()
...
func TestIcp01_IDUFrame_ActiveFlag(t *testing.T) {
	t.Parallel()
...
func TestIcp01_IDUFrame_SetTempFormula(t *testing.T) {
	t.Parallel()
...
// TestIcp01_FrameParser_ShortIDU_FollowedByIDU 는 IDU#1 의 20바이트 short
...
func TestIcp01_FrameParser_ShortIDU_FollowedByIDU(t *testing.T) {
	t.Parallel()
	...
	require.NoError(t, err)
	parser := NewIcp01FrameParser(bytes.NewReader(stream))
...
// TestIcp01_FrameParser_ShortIDU_FollowedByODU 는 IDU#5 의 20바이트 short
// 프레임 직후 ODU SEQ=01 (STX=0x58) 이 도착하는 시나리오를 검증한다.
func TestIcp01_FrameParser_ShortIDU_FollowedByODU(t *testing.T) {
	t.Parallel()
...
// TestIcp01_FrameParser_LongIDU_NotMisidentifiedAsShort 는 표준 40바이트
// IDU 프레임이 short 로 잘못 인식되지 않는지 검증한다. b[20] 의 값이 IDU_INDEX
// (0x01~0x05) 이므로 LG ICP-01 STX 범위 밖이라 Peek 으로 short 라 판단되지 않아야
// 한다. 단 IDU_INDEX 가 0x05 인 IDU#5 의 경우 b[20]=0x05 는 STX 범위 (0x81~0x85)
// 밖이므로 무관.
func TestIcp01_FrameParser_LongIDU_NotMisidentifiedAsShort(t *testing.T) {
	t.Parallel()
	...
	assert.False(t, idu.IsShort, "b[20]=0x01 은 LG ICP-01 STX 가 아니므로 long 으로 인식")
