...
// CalcIcp02CRC16 은 CRC-16/XMODEM 알고리즘으로 CRC 를 계산하여 반환한다.
...
//   - RefOut: false
//   - XorOut: 0x0000
func CalcIcp02CRC16(data []byte) uint16 {
	reg := uint16(0x0000)
	for _, b := range data {
		reg ^= uint16(b) << 8
		for i := 0; i < 8; i++ {
