...
// LGCPFrameBuilder 는 LGCP 제어 프레임을 구성한다.
//
// 프레임 구조:
//
...
// NewLGCPFrameBuilder 는 프레임 빌더를 생성한다.
func NewLGCPFrameBuilder() *LGCPFrameBuilder {
	return &LGCPFrameBuilder{}
}

// Build 는 완전한 LGCP 프레임을 구성하여 반환한다.
...
func (b *LGCPFrameBuilder) BuildControl(da, sa []byte, seq0 byte, payload []byte, seq1 byte) []byte {
...
func ParseHexAddress(s string) ([]byte, error) {
	if len(s) != 8 {
		return nil, fmt.Errorf("lgcp: address must be 8 hex chars, got %d", len(s))
	}
	b, err := hex.DecodeString(s)
	if err != nil {
		return nil, fmt.Errorf("lgcp: invalid hex address %q: %w", s, err)
	}
	return b, nil
}
