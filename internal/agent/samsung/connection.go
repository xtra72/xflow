...
// ---------------------------------------------------------------------------
// SPEC-HVACR-CONNSTATE-001: Device Connection-State Reporting
//
// 신규 message_type "device_connection.<trigger>" 를 device 당 개별 메시지로
// 방출한다. trigger ∈ {initial, change, report}, connection_state ∈ {online, offline}.
// 기존 device_state 봉투 규약(top-level unit_id/device_id/trigger, metadata.message_type,
// epoch-ms int64)을 재사용하며 최상위 type 필드는 두지 않는다.
// ---------------------------------------------------------------------------
...
const (
	...
	// startup_probe_timeout 보다 크면 timeout 값으로 축소된다.
...
func (a *Hvacr01Agent) buildConnectionPayloadLocked(addr NasaAddress, dev *NasaDevice, trigger string, eventMs int64) map[string]any {
	unitID := effectiveDeviceID(addr, dev.UnitID)
...
// 호출 전제: a.mu 보유(payload 구성이 device 필드를 읽으므로). 채널 송신 자체는
// a.mu 와 무관하다. 기존 sendEvent 관행과 동일하게 채널 포화 시 non-blocking drop.
func (a *Hvacr01Agent) emitConnectionLocked(addr NasaAddress, dev *NasaDevice, trigger string, eventMs int64) {
	payload := a.buildConnectionPayloadLocked(addr, dev, trigger, eventMs)
	...
	default:
		if a.hvacr01Config.LogDrops {
			a.logger.Warn("samsung_hvacr01: msgCh full, dropping connection event", "trigger", trigger)
		} else {
			a.logger.Debug("samsung_hvacr01: msgCh full, dropping connection event", "trigger", trigger)
		}
	}
}

// resolveStartupProbes 는 아직 initial 을 방출하지 않은 device 를 처리한다.
// force=false: online 확정은 handleMessage 경로가 담당하므로 여기서는 미방출 device
...
func (a *Hvacr01Agent) resolveStartupProbes(force bool) bool {
	a.mu.Lock()
	defer a.mu.Unlock()
	...
	for addr, dev := range a.devices {
		...
		if force {
			...
			a.emitConnectionLocked(addr, dev, connTriggerInitial, now)
			continue
		}
		allDone = false
	}
...
// handleMessage 에서 initial=online 을, startup_probe_timeout 내 미확정 시 여기서
// initial=offline 을 방출한다. a.wg 로 join 되어 Stop 시 leak 이 없다(§7.2/N10).
func (a *Hvacr01Agent) startupProbeLoop() {
	defer a.wg.Done()
	...
	timeout := a.hvacr01Config.StartupProbeTimeout
	if timeout <= 0 {
...
// Start 에서 기동된다. a.wg 로 join 된다.
func (a *Hvacr01Agent) connectionReportLoop() {
	defer a.wg.Done()

	ticker := time.NewTicker(a.hvacr01Config.ConnectionReportInterval)
	defer ticker.Stop()
...
// offline device 도 포함하며(S2/N5), AllCoreObserved() 게이트를 적용하지 않는다.
// 아직 initial 이 방출되지 않은 device 는 skip 한다(S5) — per-device 순서(E9) 보장.
// event_ms 는 tick 시점 time.Now()(S3).
func (a *Hvacr01Agent) emitConnectionReports() {
	a.mu.Lock()
	defer a.mu.Unlock()
	...
	for addr, dev := range a.devices {
		...
		a.emitConnectionLocked(addr, dev, connTriggerReport, now)
	}
}
