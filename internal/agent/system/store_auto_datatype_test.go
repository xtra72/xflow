...
//
// store_auto_datatype_test.go 는 storage-write 의 data_type: "auto" (값 기반 타입 추론)
// 경로를 검증한다. 단일 storage-write 노드가 측정별로 서로 다른 값 타입(boolean/int/float)을
...
func TestSetWithMeta_Auto_Boolean(t *testing.T) {
	...
	err := adapter.SetWithMeta(ctx, "dev-power", false, StoreWriteMeta{DataType: DataTypeAuto, Field: "power"})
	require.NoError(t, err)

	seriesKey := EncodeSeriesKey(SeriesID{Measurement: "dev-power", Field: "power"})
...
func TestSetWithMeta_Auto_Int(t *testing.T) {
	...
	err := adapter.SetWithMeta(ctx, "dev-mode", 1, StoreWriteMeta{DataType: DataTypeAuto, Field: "mode"})
	require.NoError(t, err)

	seriesKey := EncodeSeriesKey(SeriesID{Measurement: "dev-mode", Field: "mode"})
...
func TestSetWithMeta_Auto_MixedMeasurements_NoMismatch(t *testing.T) {
	...
	for _, w := range writes {
		err := adapter.SetWithMeta(ctx, w.key, w.value, StoreWriteMeta{DataType: DataTypeAuto, Field: w.field})
		require.NoErrorf(t, err, "measurement %q 쓰기는 성공해야 한다(ErrTypeMismatch 없음)", w.field)

		seriesKey := EncodeSeriesKey(SeriesID{Measurement: w.key, Field: w.field})
		assert.Equalf(t, w.wantType, a.StaticKeysSnapshot()[seriesKey].DataType,
			"measurement %q 는 %s 로 고정되어야 한다", w.field, w.wantType)
	}

	// 같은 power 키에 대한 후속 boolean 쓰기도 계속 성공한다(키 단위 타입 일정).
	require.NoError(t, adapter.SetWithMeta(ctx, "집중회의실-power", true,
		StoreWriteMeta{DataType: DataTypeAuto, Field: "power"}))
...
func TestSetWithMeta_Auto_Nil_RejectedLikeNormalWrite(t *testing.T) {
	...
	err := adapter.SetWithMeta(ctx, "dev-x", nil, StoreWriteMeta{DataType: DataTypeAuto, Field: "x"})
	require.Error(t, err, "nil 은 auto 여도 저장되지 않는다(일반 쓰기와 동일)")

	// 고정도 되지 않았고 값도 없으므로 키가 등록되지 않는다.
	seriesKey := EncodeSeriesKey(SeriesID{Measurement: "dev-x", Field: "x"})
