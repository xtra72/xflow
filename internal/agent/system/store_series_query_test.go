...
// findSeries 는 결과에서 (metric, tags) 시리즈를 찾는다.
func findSeries(results []SeriesResult, metric string, tags map[string]string) (SeriesResult, bool) {
	want := EncodeSeriesKey(SeriesID{MetricType: metric, Tags: tags})
	for _, r := range results {
		if EncodeSeriesKey(SeriesID{MetricType: r.Series.MetricType, Tags: r.Series.Tags}) == want {
			return r, true
		}
	}
...
// AC-1: 같은 key + 다른 metric_type → 별개 시리즈, 서로 덮어쓰지 않음 (E2/N1).
func TestQuerySeries_DiffMetricType_IndependentSeries(t *testing.T) {
	...
	require.NoError(t, err)
	require.Len(t, results, 2, "두 metric 시리즈가 독립 존재해야 한다")
...
func TestQuerySeries_DataTypeIrrelevant_SameSeries(t *testing.T) {
	...
	require.NoError(t, adapter.SetWithMeta(ctx, "sensor", float64(22), StoreWriteMeta{
		DataType: "float", MetricType: "temperature",
	}))
	// 같은 (key, metric, tags) — data_type 만 다른 값. 동일 시리즈 갱신.
...
func TestQuerySeries_MetricAndTagsFilter_SingleSeries(t *testing.T) {
	...
	assert.Equal(t, "temperature", results[0].Series.MetricType)
	assert.Equal(t, "a", results[0].Series.Tags["area"])
}

// AC-7: metric_type 만 필터(tags 생략) → 해당 metric 의 모든 tags 시리즈 (S3).
func TestQuerySeries_MetricOnlyFilter_AllTagsOfMetric(t *testing.T) {
	...
	for _, r := range results {
		assert.Equal(t, "temperature", r.Series.MetricType)
	}
}
...
func TestQuerySeries_IndependentHistory(t *testing.T) {
	...
	for _, v := range []int{20, 21, 22} {
		require.NoError(t, adapter.SetWithMeta(ctx, "room", v, StoreWriteMeta{MetricType: "temperature"}))
	}
	// humidity 시리즈: 50, 51.
	for _, v := range []int{50, 51} {
		require.NoError(t, adapter.SetWithMeta(ctx, "room", v, StoreWriteMeta{MetricType: "humidity"}))
	}
...
func TestQuerySeries_DefaultSeries_NoMetaWrite(t *testing.T) {
	...
	assert.Equal(t, MetricTypeUnknown, results[0].Series.MetricType)
...
func TestQuerySeries_AutoRegistersNewSeries(t *testing.T) {
	...
	seriesKey := EncodeSeriesKey(SeriesID{Key: "new", MetricType: "temperature", Tags: map[string]string{"room": "1"}})
...
// AC-17: 같은 key 에 서로 다른 metric/tags 로 동시 다수 goroutine 쓰기 → race/panic 없음,
...
func TestQuerySeries_ConcurrentDifferentSeries_NoRace(t *testing.T) {
	...
	require.NoError(t, err)
	// metric 2종 × tags 4종 = 최대 8개 시리즈 (실제 조합 수에 따라).
	assert.NotEmpty(t, results, "동시 쓰기 후 시리즈가 존재해야 한다")
	for _, r := range results {
		assert.Equal(t, "shared", r.Series.Key)
