...
// SeriesID 정규화(tags 정렬, field 기본값)와 EncodeSeriesKey 의 결정성·단사성을 검증한다.
...
// -----------------------------------------------------------------------------
// 정규화: field 빈 값 → unknown (A3)
// -----------------------------------------------------------------------------

// TestEncodeSeriesKey_EmptyMetricDefaultsUnknown 은 field 빈 값이
// FieldUnknown 으로 보정되어 명시적 "unknown" 과 동일하게 인코딩됨을 검증한다.
func TestEncodeSeriesKey_EmptyMetricDefaultsUnknown(t *testing.T) {
	empty := SeriesID{Measurement: "k", Field: "", Tags: nil}
	explicit := SeriesID{Measurement: "k", Field: FieldUnknown, Tags: nil}

	if got, want := EncodeSeriesKey(empty), EncodeSeriesKey(explicit); got != want {
		t.Fatalf("빈 field 가 unknown 으로 보정되지 않음: %q vs %q", got, want)
	}
}
...
// TestSeriesID_Normalize 는 Normalize 가 field 기본값과 tags 깊은 복사를
// 적용하고, 원본을 변형하지 않음을 검증한다.
func TestSeriesID_Normalize(t *testing.T) {
	orig := SeriesID{Measurement: "dev", Field: "", Tags: map[string]string{"a": "1"}}
	norm := orig.Normalize()

	if norm.Field != FieldUnknown {
		t.Fatalf("빈 field 가 %q 로 보정되지 않음: %q", FieldUnknown, norm.Field)
	}
	if norm.Measurement != "dev" {
		t.Fatalf("key 가 보존되지 않음: %q", norm.Measurement)
	}
	if norm.Tags["a"] != "1" {
		t.Fatalf("tags 가 보존되지 않음: %v", norm.Tags)
	}

	// 원본 field 은 변형되지 않아야 한다(값 복사 시맨틱).
	if orig.Field != "" {
		t.Fatalf("Normalize 가 원본 field 을 변형함: %q", orig.Field)
	}
}
...
// TestEncodeSeriesKey_Determinism 은 같은 (key, field, tags) 가 항상 같은 인코딩을
// 생성함을 검증한다 (U4 결정성).
func TestEncodeSeriesKey_Determinism(t *testing.T) {
	s := SeriesID{Measurement: "room", Field: "temperature", Tags: map[string]string{"area": "a"}}
...
// TestEncodeSeriesKey_Injectivity 는 key/field/tags 중 하나라도 다르면 인코딩이
// 달라짐을 검증한다 (N5 충돌 회피, 단사성).
func TestEncodeSeriesKey_Injectivity(t *testing.T) {
	cases := []SeriesID{
		{Measurement: "room", Field: "temperature", Tags: nil},
		{Measurement: "room", Field: "humidity", Tags: nil},                               // field 만 다름
		{Measurement: "room", Field: "temperature", Tags: map[string]string{"area": "a"}}, // tags 만 다름
		{Measurement: "room", Field: "temperature", Tags: map[string]string{"area": "b"}}, // tag value 만 다름
		{Measurement: "room", Field: "temperature", Tags: map[string]string{"zone": "a"}}, // tag key 만 다름
		{Measurement: "other", Field: "temperature", Tags: nil},                           // key 만 다름
		{Measurement: "room", Field: "temperature", Tags: map[string]string{"a": "1", "b": "2"}},
		{Measurement: "room", Field: "temperature", Tags: map[string]string{"a": "1"}},
	}
...
// -----------------------------------------------------------------------------
// 기본 시리즈: SeriesID{Measurement:k} == SeriesID{Measurement:k, Field:"unknown"} (AC-16)
// -----------------------------------------------------------------------------
...
func TestEncodeSeriesKey_DefaultSeriesEquivalence(t *testing.T) {
	bare := SeriesID{Measurement: "simple"}
	explicit := SeriesID{Measurement: "simple", Field: FieldUnknown, Tags: map[string]string{}}
	nilTags := SeriesID{Measurement: "simple", Field: "", Tags: nil}
...
func TestEncodeSeriesKey_ArbitraryKeyNoBoundaryAmbiguity(t *testing.T) {
	...
	cases := []SeriesID{
		{Measurement: "dev|a", Field: "temperature", Tags: nil},
		{Measurement: "dev", Field: "temperature", Tags: map[string]string{"x": "a"}}, // "temperature||dev" 와 충돌 시도
		{Measurement: "a|b|c", Field: "humidity", Tags: map[string]string{"r": "1"}},
		{Measurement: "uuid-1234.5678", Field: "temperature", Tags: nil},
		{Measurement: "k,m=v", Field: "temperature", Tags: nil},
		{Measurement: "", Field: "temperature", Tags: nil}, // 빈 key 도 다른 식별자로 취급
	}
...
func TestEncodeSeriesKey_TagCardinality(t *testing.T) {
	none := SeriesID{Measurement: "k", Field: "m", Tags: map[string]string{}}
	one := SeriesID{Measurement: "k", Field: "m", Tags: map[string]string{"a": "1"}}
	many := SeriesID{Measurement: "k", Field: "m", Tags: map[string]string{"a": "1", "b": "2", "c": "3"}}
...
func TestDecodeSeriesKey_RoundTrip(t *testing.T) {
	cases := []SeriesID{
		{Measurement: "room", Field: "temperature", Tags: nil},
		{Measurement: "a|b|c", Field: "humidity", Tags: map[string]string{"r": "1", "z": "2"}},
		{Measurement: "uuid-1234.5678", Field: "", Tags: map[string]string{"area": "a"}},
		{Measurement: "", Field: "temperature", Tags: nil},
	}
	...
	for _, s := range cases {
		...
		want := s.Normalize()
		if dec.Measurement != want.Measurement || dec.Field != want.Field {
			t.Fatalf("디코딩 key/field 불일치: got=%+v want=%+v", dec, want)
		}
...
func TestEncodeSeriesKey_ConcurrentSafe(t *testing.T) {
	s := SeriesID{Measurement: "room", Field: "temperature", Tags: map[string]string{"a": "1", "b": "2"}}
	...
	for i := 0; i < 64; i++ {
		...
		go func() {
			defer wg.Done()
			local := SeriesID{Measurement: "room", Field: "temperature", Tags: map[string]string{"a": "1", "b": "2"}}
