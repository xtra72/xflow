...
//
// 한 key 아래 여러 (metric_type, tags) 시리즈가 독립 저장되므로(M2), 조회는 key →
...
// SeriesID{Key: raw}(metric=unknown, tags 없음) 기본 시리즈로 해석하여 흡수한다.
...
// 기본 시리즈 SeriesID{Key: raw} 로 해석한다(레거시/plain Set 흡수).
func decodeStorageKeyToSeries(rawKey string) SeriesID {
	if sid, err := DecodeSeriesKey(rawKey); err == nil {
		return sid
	}
	// 비-시리즈 키: 기본 시리즈로 정규화(metric=unknown, tags={}).
	return SeriesID{Key: rawKey}.Normalize()
}

// seriesMatchesFilter 는 시리즈가 (key, metricFilter, tagsFilter) 조건을 만족하는지 검사한다.
//
// 규칙 (E5/S3):
//   - SeriesID.Key 는 조회 key 와 정확히 일치해야 한다.
//   - metricFilter 가 빈 문자열이면 metric 축은 통과(no-op), 아니면 정확히 일치해야 한다.
//   - tagsFilter 의 각 (k,v) 는 시리즈 tags 에 정확히 존재해야 한다(부분집합 AND 매칭).
//     tagsFilter 가 비어있으면 tags 축은 통과(no-op) → 해당 metric 의 모든 tags 조합 시리즈.
func seriesMatchesFilter(sid SeriesID, key, metricFilter string, tagsFilter map[string]string) bool {
	if sid.Key != key {
		return false
	}
	if metricFilter != "" && sid.MetricType != metricFilter {
		return false
	}
...
//
// 동작:
//   - metricFilter/tagsFilter 가 모두 비어있으면 해당 key 의 **모든 시리즈**를 반환한다(E4).
//   - metricFilter 만 주어지면 그 metric 의 **모든 tags 조합 시리즈**를 반환한다(S3).
//   - metric+tags 가 주어지면 일치하는 **단일/부분집합 시리즈**만 반환한다(E5).
