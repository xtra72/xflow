...
// newMetaAdapterWithAgent 는 SetWithMeta 의 field 적용 검증을 위해
...
// TestSetWithMeta_AppliesField 는 Field 이 지정되면 기록되는 키의
// field 으로 반영되는지 검증한다.
func TestSetWithMeta_AppliesField(t *testing.T) {
	...
	seriesKey := EncodeSeriesKey(SeriesID{Measurement: "k", Field: "temperature"})
	meta, ok := sa.StaticKeyMetaFor(seriesKey)
	require.True(t, ok)
	assert.Equal(t, "temperature", meta.Field)
}

// TestSetWithMeta_FieldWithTags 는 Field 과 Tags 가 함께 지정되면
// 둘 다 한 번에 적용되는지 검증한다.
func TestSetWithMeta_FieldWithTags(t *testing.T) {
	ctx := context.Background()
	adapter, sa := newMetaAdapterWithAgent(t, "default")
	...
	seriesKey := EncodeSeriesKey(SeriesID{Measurement: "k", Field: "humidity", Tags: map[string]string{"unit": "percent"}})
	meta, ok := sa.StaticKeyMetaFor(seriesKey)
	require.True(t, ok)
	assert.Equal(t, "humidity", meta.Field)
	assert.Equal(t, map[string]string{"unit": "percent"}, meta.Tags)
}

// @spec SPEC-STORE-004
// TestSetWithMeta_DiffMetricTags_IndependentSeries 는 같은 key 에 (field only) 쓰기와
...
func TestSetWithMeta_DiffMetricTags_IndependentSeries(t *testing.T) {
	...
	// 2) field 없이 tags 만 → (k, unknown, {unit:celsius}) 별개 시리즈.
	...
	tempKey := EncodeSeriesKey(SeriesID{Measurement: "k", Field: "temperature"})
	tagsKey := EncodeSeriesKey(SeriesID{Measurement: "k", Tags: map[string]string{"unit": "celsius"}})

	tempMeta, ok := sa.StaticKeyMetaFor(tempKey)
	require.True(t, ok, "temperature 시리즈가 보존되어야 한다")
	assert.Equal(t, "temperature", tempMeta.Field)
	...
	assert.Equal(t, FieldUnknown, tagsMeta.Field)
	assert.Equal(t, map[string]string{"unit": "celsius"}, tagsMeta.Tags)
}

// @spec SPEC-STORE-004
// TestSetWithMeta_SameSeries_DataTypeIrrelevant 는 같은 (key, field, tags) 에 대해
...
func TestSetWithMeta_SameSeries_DataTypeIrrelevant(t *testing.T) {
	...
	// 같은 (key, field, tags) — data_type 만 명시(float) → 동일 시리즈.
	...
	seriesKey := EncodeSeriesKey(SeriesID{Measurement: "k", Field: "temperature", Tags: map[string]string{"unit": "celsius"}})
...
// TestSetWithMeta_DataTypeMetricTags_Combined 는 data_type + field + tags 가
...
// field 은 unknown(자동 등록) 으로 남는지 확인한다 (기존 동작 보존).
func TestSetWithMeta_NoMeta_Fallback(t *testing.T) {
	...
	seriesKey := EncodeSeriesKey(SeriesID{Measurement: "k"})
	meta, ok := sa.StaticKeyMetaFor(seriesKey)
	require.True(t, ok)
	assert.Equal(t, FieldUnknown, meta.Field, "메타 미지정 시 기본 시리즈 unknown")
}
