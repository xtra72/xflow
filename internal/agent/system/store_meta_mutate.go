...
//
// store_meta_mutate.go 는 임의 엔트리(정적 + 동적)의 field / tags 를 런타임에
...
// SetKeyMeta 는 지정된 key 의 field 과 tags 를 설정/갱신한다.
//
// 검증:
//   - key 는 비어있지 않아야 한다.
//   - field 은 빈 문자열(→ "unknown" normalize) 이거나 ^[a-zA-Z0-9_-]+$ 를 만족해야 한다.
...
//
// 반환:
//   - ErrInvalidField: field 정규식 위반
//   - ErrInvalidTagKey:     tag key 정규식 위반
//   - 그 외 입력 오류는 일반 error 로 반환
func (a *UserStoreAgent) SetKeyMeta(key string, field string, tags map[string]string) error {
	if key == "" {
		return fmt.Errorf("store set key meta: key must be non-empty")
	}

	// field 검증/normalize (빈 문자열 → "unknown").
	normalized, err := validateField(field)
	if err != nil {
		return fmt.Errorf("%w: field=%q", ErrInvalidField, field)
	}
