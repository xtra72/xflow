...
// (float64)을 쓸 때 타입 불일치로 거부되어 field 이 유실되던 문제를 검증한다.
//
// 서브플로우 브리지가 메시지를 JSON 직렬화하면 모든 숫자가 float64 가 되므로,
// data_type=int(mode/fan_speed) 키 쓰기가 거부되어 field=unknown 으로 남았다.
// 수정 후: 정수값 float64 는 int 로 허용·정규화되어 쓰기 성공 + field 적용.
func TestRepro_DataTypeInt_AcceptsIntegralFloat(t *testing.T) {
	...
	if meta.Field != "mode" {
		t.Errorf("field=%q, want %q", meta.Field, "mode")
	}
