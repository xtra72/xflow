...
// 저장 아이템을 단일 string key 가 아니라 (key, metric_type, sorted(tags)) 복합
...
//
// 시리즈 식별자 = (Key, MetricType, sorted(Tags)) (namespace 스코프 내).
// 같은 Key 라도 MetricType 또는 (정규화된) Tags 가 하나라도 다르면 다른 시리즈이다.
//
// data_type 은 식별 차원이 아니므로 본 타입에 포함하지 않는다 (U3). data_type 은
// 시리즈가 보관하는 값의 타입 계약일 뿐이며, 같은 (Key, MetricType, Tags) 이면
// data_type 이 달라도 동일 시리즈로 취급된다.
//
// 필드:
//   - Key: 사용자 관점의 논리 key. 디바이스 UUID 등 임의 문자열일 수 있다
//     (`.`, `-`, 심지어 인코딩 구분자 `|`/`,`/`=` 를 포함할 수도 있다).
//   - MetricType: 측정 종류. ^[a-zA-Z0-9_-]+$ 정규식을 따르며 빈 값은 정규화 시
//     MetricTypeUnknown("unknown") 으로 보정된다 (A3).
...
//  1. MetricType 빈 값 → MetricTypeUnknown ("unknown").
...
// 시리즈 키 인코딩 구분자.
//
// 인코딩 안전성 근거 (N5): MetricType 과 tag key/value 는 모두 ^[a-zA-Z0-9_-]+$
// 정규식을 따르므로(A3) 아래 구분자 `|`, `,`, `=` 중 어느 것도 포함할 수 없다.
// 반면 Key 는 임의 문자열이라 구분자를 포함할 수 있다. 따라서 인코딩은 "제약된
// 문자집합을 가진 필드(metric, tags)를 앞쪽"에 배치하여, 두 개의 `|` 구분자 위치가
...
const (
	seriesFieldSep = "|" // metric | tags | key 프레임 구분자
...
//
// 인코딩 형식: `metric_type | tagsEncoded | key`
//   - tagsEncoded = tag-key 사전순 정렬된 "k1=v1,k2=v2" (빈 tags 면 빈 문자열).
//   - metric_type 빈 값은 "unknown" 으로 보정된다(Normalize).
//
// 성질:
//   - 결정성 (U4): 동일 식별자는 항상 동일 문자열을 생성한다. tag 순서가 달라도
//     정렬 덕분에 동일하다 (U2).
//   - 단사성 (N5): key/metric/tags 중 하나라도 다르면 다른 문자열을 생성한다.
//     metric/tags 는 구분자를 포함할 수 없으므로(A3) 두 `|` 위치가 결정적이고,
//     임의 key 가 구분자를 포함해도 충돌이 없다.
//   - 기본 시리즈: SeriesID{Key:k} 는 SeriesID{Key:k, MetricType:"unknown", Tags:{}}
...
//	동일한 문법이다. 바깥 프레임(metric|tags|key)만 store 고유로 둔다. 이로써 공유
//	하위 문법은 유지하면서 임의 key 안전성을 확보한다.
func EncodeSeriesKey(s SeriesID) string {
	n := s.Normalize()

	var b strings.Builder
	b.WriteString(n.MetricType)
...
// 첫 번째 `|` 이전 = metric_type, 두 번째 `|` 이전 = tagsEncoded, 그 이후 = key.
// metric/tags 가 `|` 를 포함하지 않으므로(A3) SplitN(.., 3) 분해는 안전하다.
//
...
func DecodeSeriesKey(encoded string) (SeriesID, error) {
	...
	metric, tagsEncoded, key := parts[0], parts[1], parts[2]
