...
// recordingProcessor 는 AirPurifierProcessor 를 구현하며 수신한 요청 바이트를 기록한다.
...
func TestAirPurifierCommandSpecs(t *testing.T) {
	t.Parallel()
	dev := NewControllableAirPurifierDevice("ap-1", AirPurifierDeviceInfo{DeviceID: "ap-101"}, nil)
	specs := dev.Commands()
...
// 어댑터 인터페이스 준수 + Protocol 키("airpurifier").
func TestAirPurifierAdapter_ProtocolKeyAndInterfaces(t *testing.T) {
	t.Parallel()
	power := true
	fan := 2
	...
	dev := NewControllableAirPurifierDevice("ap-agent", info, nil)

	// 어댑터 레지스트리 키.
	assert.Equal(t, "airpurifier", dev.Protocol())
...
func TestAirPurifierAdapter_State_FanOmittedWhenOff(t *testing.T) {
	t.Parallel()
	power := false
	fan := 3
	dev := NewControllableAirPurifierDevice("ap-agent", AirPurifierDeviceInfo{
...
func TestAirPurifierExecutor_BridgesToProcess(t *testing.T) {
	t.Parallel()
	proc := &recordingProcessor{response: []byte(`{"status":"ok","device_id":"ap-101","command":"set_power"}`)}
	exec := NewAirPurifierExecutor(proc, "ap-101")
...
func TestAirPurifierAdapter_Accessors(t *testing.T) {
	t.Parallel()
	...
	dev2 := NewAirPurifierDevice("ap-agent", AirPurifierDeviceInfo{DeviceID: "ap-202"})
	assert.Equal(t, "auto", dev2.Source())
}

// executor 오류는 그대로 전파된다.
func TestAirPurifierExecutor_ProcessError(t *testing.T) {
	t.Parallel()
...
func TestAirPurifierAdapter_NonControllable(t *testing.T) {
	t.Parallel()
	dev := NewAirPurifierDevice("ap-agent", AirPurifierDeviceInfo{DeviceID: "ap-101"})
