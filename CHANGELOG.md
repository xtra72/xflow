...
- 설정 → 테마 탭에 **축소 화면**을 뒀다. 플로우 노드 · 연결선 · 캔버스 · 대시보드 패널 · 목록 표 · 단추 · 상태 표시가 한자리에 모여 있어, 색을 고치면 실제 화면이 어떻게 보일지 그 자리에서 확인된다.
- 미리보기는 **지금 화면에 적용된 테마가 아니라 편집 중인 테마**를 그린다. 종전에는 낮에 앉아 야간 팔레트를 고치면 아무것도 바뀌지 않아, 눈을 감고 칠하는 것과 같았다.
- 표의 색 항목에 커서를 올리면 미리보기에서 그 색이 쓰이는 자리가 강조되고, 미리보기의 조각을 누르면 그 조각이 쓰는 색 항목으로 표가 옮겨 간다. 이름만 보고 "이 색이 어디에 쓰이는가" 를 아는 사람은 없다.
- 색 항목이 21개에서 **24개**로 늘었다 — 캔버스 점격자 · 연결선 · 영역 상자.
