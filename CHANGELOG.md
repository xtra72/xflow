...
### 추가 — 자리를 비우면 자동으로 로그아웃한다

- **설정한 시간 동안 아무 입력이 없으면 자동으로 로그아웃합니다.** 기본은 **10분**이고 켜져 있습니다. 설정 → 시스템 탭에서 시간과 사용 여부를 바꿉니다 — **서버 설정이라 모든 사용자에게 적용됩니다.**
- **잠기기 1분 전에 안내가 뜨고 "계속 사용" 을 누르면 연장됩니다.** 플로우·대시보드·캔버스 편집기가 있어 경고 없이 끊으면 저장하지 않은 편집이 사라집니다. 경고 시간은 설정한 시간 **안에** 들어 있습니다(10분 설정이면 9분에 안내, 10분에 로그아웃). 시간을 아주 짧게 잡으면 안내도 그만큼 앞당겨집니다(1분 설정 → 30초 전).
- **여러 탭을 열어 두어도 한 곳에서 일하면 다른 탭이 잠기지 않습니다.** 마지막 활동을 탭끼리 나눠 봅니다.
- 마우스 움직임·키 입력·스크롤·터치는 모두 사용으로 칩니다. 다만 **탭을 다시 열어 보는 것만으로는 연장되지 않습니다** — 두 시간 만에 돌아온 탭이 돌아왔다는 이유로 연장되면 잠그는 뜻이 없어집니다.
- 로그아웃은 화면만 잠그는 것이 아니라 **서버의 토큰도 함께 폐기**합니다.

### 수정 — 세션이 만료돼도 로그인 화면으로 가지 않던 문제

- **탭을 열어 둔 채 세션이 만료되면 화면이 에러만 뿌린 채 그대로 있었습니다.** 이제 로그인 화면으로 갑니다.
- **자동 토큰 갱신이 한 번도 동작한 적이 없었습니다.** 갱신 장치는 처음부터 있었지만, 응답을 가로채는 두 장치의 **등록 순서** 때문에 갱신 쪽이 401 을 아예 보지 못했습니다. 그래서 24시간이 지나면 그걸로 끝이었고, 7일짜리 갱신 토큰은 쓰이지 않아 **매일 다시 로그인**해야 했습니다. 이제 조용히 갱신되어 세션이 갱신 토큰 수명만큼 유지됩니다.
- **실시간 연결이 끊겼을 때도 만료를 알아챕니다.** 다만 끊긴 까닭이 만료인지 네트워크 문제인지 확인한 뒤에 판단합니다 — 잠깐 끊긴 사용자를 로그인 화면으로 내쫓지 않기 위해서입니다.

### 추가 — 원격 관리 로그와, 수신·접속을 나눈 두 칸

- **노드에 일어난 일을 시간순으로 봅니다.** 원격 관리 → 로그 페이지(모든 노드)와 노드 화면의 로그 탭(그 노드만)입니다. 등록 요청 · 연결 · 끊어짐 · 원격 접속 · 승인/거부/폐기 · 명령 · 이미지 업데이트 · 버전 변경이 한 표에 쌓입니다.
- **목록은 노드·사건·수행자로 거르고, 칸을 눌러 정렬하고, 쪽당 줄 수를 고릅니다.** 정렬과 필터는 **전체 기준**입니다 — 보이는 쪽 안에서만 정렬하면 그 결과가 전체를 대표하지 못합니다. 노드는 이름으로 보이며 단추로 ID 와 오갑니다.
- **"마지막 수신" 이 이제 진짜 수신 시각입니다.** 종전에는 마지막으로 연결된 시각이 그 칸에 들어 있어서, 30초마다 살아 있다고 알려 오는 멀쩡한 노드가 "3일째 무소식" 으로 보였습니다.
- **"마지막 접속" 칸이 생겼습니다** — 관리자가 그 노드를 마지막으로 들여다본 시각과 사람입니다. 노드가 보내는 신호와 사람이 한 일은 다른 질문이라 칸을 나눴습니다.
- 로그는 **30일** 지나면 지웁니다(`remote_management.audit_retention`). 0 으로 두면 지우지 않습니다.

### 추가 — 재연결 간격을 설정한다

- **관리 서버와 끊어진 노드가 다시 붙는 간격을 정할 수 있습니다.** 설정 → 원격 관리 탭의 재연결 최소/최대 간격(기본 1초/60초, 재시작 후 적용)입니다.
- 동작은 종전과 같습니다 — 끊어진 직후에는 최소 간격으로 빨리 붙어 보고, 실패가 이어지면 2배씩 늘려 최대 간격에서 멈추며, **시도를 멈추지 않습니다.**
- **붙자마자 끊기는 상태에서 최소 간격으로 두드리던 것을 고쳤습니다.** 서버가 아직 준비되지 않아 연결을 곧장 닫는 경우가 그런 자리인데, 종전에는 "연결됐다 끊김" 으로 세어 간격을 되돌렸습니다.

### 수정 — 노드 목록이 열리지 않고, 없는 타입을 말해 주지 않던 문제

- **플로우의 노드 목록(`/flows/{id}/nodes`)이 500 으로 실패했습니다.** 데몬이 노드에 넣어 준 내부 연결 고리가 응답에 섞여 나가려다 변환에 실패한 것으로, 노드 하나가 아니라 **목록 전체**가 죽었습니다. 이제 내부 값은 빼고 내보냅니다.
- **"node: type not found" 가 어느 타입인지 말하지 않았습니다.** 오류에는 노드 **이름**만 있어서 저장된 플로우를 열어 봐야 했습니다. 이제 없는 타입 이름이 함께 나옵니다.
- **없는 타입이 여럿이면 한꺼번에 알려 줍니다.** 종전에는 첫 번째에서 멈춰, 하나 고치고 다시 시작하면 또 멈추기를 되풀이해야 했습니다.

### 수정 — 접속됐다는 로그는 찍히는데 관리 화면에는 없던 노드

- **관리 서버 로그에는 `관리 노드 online` 이 찍히고 인벤토리까지 올라오는데, 원격 관리 화면의 노드 목록은 비어 있었다.** 노드가 보낸 접속 인사(hello)를 서버가 **등록 여부를 묻지 않고** 받아들였기 때문이다. 화면의 목록은 등록 대장을 읽으므로, 대장에 없는 노드는 로그에만 살아 있는 유령이 됐다 — 그 노드의 플로우·에이전트·디바이스는 서버에 쌓이는데 정작 노드는 어디에도 보이지 않는다.
- 어쩌다 그 상태가 되는가. 노드 토큰은 24시간짜리이고, 서명 키를 설정에 고정해 두지 않으면 **서버가 재시작할 때마다 새로 뽑힌다.** 그래서 토큰이 무효해지는 일은 드문 사고가 아니라 일상이다. 그런데 노드는 토큰을 **가지고 있다는 이유만으로** hello 를 보냈고, 무효해진 토큰을 버리는 자리가 없어 등록 경로로 돌아가지 못했다 — 스스로 풀리지 않는 상태였다.
- 이제 서버는 등록되지 않았거나 승인되지 않은 hello 를 거부하고, **왜 거부했는지를 노드에 돌려준다.** 노드는 그 신호를 받으면 쥐고 있던 토큰을 버리고 다시 등록을 요청한다. 승인된 노드였다면 새 토큰이 그 자리에서 내려오고, 대장에서 지워진 노드였다면 **승인 대기 목록에 나타난다** — 관리자가 승인하면 끝이다. 노드 장비에 들어가 토큰 파일을 지울 일이 없다.
- 거부하면서 사유를 돌려주는 것이 요점이다. 조용히 끊으면 노드는 같은 토큰으로 재접속만 되풀이한다.
- **운영 권고: `server.basic_auth.jwt_secret` 을 설정에 고정하라.** 비워 두면 재시작마다 키가 바뀌어 모든 노드가 한 번씩 재등록을 지나간다. 위 복구가 자동이라 장애로 보이지는 않지만, 재시작마다 치를 비용은 아니다.
- 승인된 노드가 토큰 없이 보낸 hello 는 이번에도 받아들인다. 그 자리까지 막으려면 승인 노드의 접속 방식을 함께 바꿔야 해서 이번 범위에서 뺐다.

### 수정 — 연결된 적 없는 노드가 online 으로 남아 있던 문제

- **붙어 있지도 않은 노드가 online 으로 표시됐다.** 서버가 살아 있는 동안만 기억하는 연결 목록에 항목이 없으면, 화면은 **DB 에 마지막으로 적힌 값**을 그대로 내보냈다. 항목은 노드가 붙을 때만 생기므로 "항목 없음" 은 "연결 없음" 이 아니라 **"이번 부팅 이후 붙은 적 없음"** 이다 — 서버가 죽던 순간 online 이던 노드는 그 뒤로 영원히 online 으로 남았다.
- 거기에 하트비트가 끊긴 노드를 내리는 청소기가 그 결과를 DB 에 적지 않아, 다음 재시작에 거짓이 되살아났다.
- 이제 **하트비트가 최근에 왔는가**가 판정한다. 재시작 경계를 넘어 살아남는 신호는 그것뿐이다. 청소기가 내린 값도 함께 적는다.
- 같은 판정의 사본이 노드 상세 화면에도 한 벌 더 있었다 — 목록만 고쳤다면 상세는 계속 online 이라고 말했을 것이다.

### 변경 — 빈 자리를 그냥 끌면 이제 화면이 움직인다

- **종전에는 빈 자리를 끌면 고무줄 상자가 쳐지며 그 안의 것들이 골라졌다. 이제 같은 동작이 보는 자리를 옮긴다.** 손에 익은 사람은 반드시 한 번 놀란다 — 고르려고 끌었는데 그림이 통째로 따라온다. 범위로 고르려면 이제 `Ctrl`(`⌘`) 이나 `Shift` 를 함께 누른다.
- 바꾼 까닭은 둘이다. 화면을 옮기는 일은 자주 하고 범위로 고르는 일은 가끔 하는데 자주 하는 쪽이 더 어려운 손짓을 쓰고 있었다. 그리고 배율을 높이면 보이는 자리가 가운데에 박혀 있어서, 출력 영역 바깥에 놓아 둔 것에는 **마우스로는 아예 손이 닿지 않았다.**
- **`Ctrl`(`⌘`) + 끌기는 뜻이 하나 바뀌었다** — 종전에는 이미 고른 것에 **더했는데** 이제는 고른 것을 **갈아치운다.** 더하려면 `Shift` 를 쓴다(종전 그대로).
- **예전 손짓으로 되돌리는 설정은 없다.** 어느 손짓이 무엇을 하는지는 **누른 조합만으로** 정해지고, 그 배정을 바꿀 자리를 두지 않았다 — `Ctrl`(`⌘`) · `Shift` · `Space` 셋이 이 손짓 말의 전부다. 놀란 사람이 가장 먼저 하는 일이 그 설정을 찾는 일이므로, 찾을 것이 없다는 것을 여기 적어 둔다.
- `Space` 를 누른 채 끄는 것도 여전히 화면을 옮기고, 빈 자리를 그냥 누르면 여전히 선택이 비워진다 — 다만 누르는 순간이 아니라 **손을 뗄 때** 비워진다. 그 순간을 미루지 않으면 화면을 옮기려고 누른 것과 선택을 지우려고 누른 것을 가를 수 없다.
- 마우스를 쓰지 않는 사람을 위해 `Space` 와 방향키를 함께 눌러도 같은 일이 일어난다.
- **도형 위에서는 한 가지도 달라지지 않았다.** 그냥 끌면 그 도형이 옮겨지고, `Ctrl` · `Shift` 로 누르면 여럿을 고른다.
- 지금 어느 쪽인지는 **커서 모양**이 알린다 — 빈 자리 위에서는 펼친 손, 끄는 동안은 쥔 손이다. 손짓의 뜻이 바뀐 것을 화면에서 알릴 수 있는 자리가 거기뿐이다.

### 변경 — 캔버스 도구를 왼쪽 줄에서 위쪽 띠로 옮긴다

- **왼쪽 줄에 세로로 쌓여 있던 열 묶음이 셋으로 줄었다** — 도형 · 가져오기 · 서랍. 나머지 일곱(작업 영역 배율 · 격자 · 정렬 · 순서 · 그룹 · 앵커 · 연결선)은 **패널 스타일 미리보기** 제목 바로 아래, 폭을 가로지르는 새 도구 띠로 나갔다.
- 가른 기준은 그 도구가 무엇을 하느냐다. 남은 셋은 모두 캔버스에 **무엇인가를 올려놓는** 일이고 고를 것을 펼쳐 보여야 하므로 세로 자리를 원한다. 나간 일곱은 **캔버스 자체나 이미 고른 것에** 손을 대는 일이라, 한 줄로 늘어놓아도 잃는 것이 없다.
- 창이 좁아지면 도구 띠는 **줄을 늘려 접힌다** — 옆으로 밀리지도, 넘침 단추 뒤로 숨지도 않으므로 단추가 화면에서 사라지는 일은 없다. 설정 창은 좌우를 손잡이로 나눌 수 있어 띠가 쓸 폭이 처음부터 정해져 있지 않다.
- 이름 하나가 바뀌었다 — 배율을 정하던 **보기** 가 **작업 영역** 이 됐다. 미리보기 제목 옆에 붙은 자신의 `%` 와 이 배율이 이제 몇 센티미터 안에 나란히 놓이는데, 둘은 **서로 다른 것을 키우고 줄인다.**

### 변경 — 늘 떠 있던 설명을 `?` 안으로 넣는다

- **화면에 눌러앉아 있던 설명 문장 다섯을 각 묶음 제목 옆 `?` 안으로 옮겼다.** 눌러야 열린다. 한 번 읽으면 그만인 문장이 자리를 계속 차지하면, 정작 손댈 것이 그만큼 아래로 밀린다.
- 옮긴 다섯은 이것들이다 — 서랍이 **이 브라우저에만** 저장된다는 것 · 편집하는 동안 캔버스가 **패널 크기를 따라간다**는 것 · 목록에서 **아래에 있을수록 위에 그려진다**는 것(위아래가 곧 앞뒤 순서다) · 서랍에 **넣는 방법**(고른 것을 끌어 넣거나 저장을 누른다) · 요소 목록이 비었을 때 **시작하는 방법**(왼쪽 도형 팔레트에서 고른다). 다섯 다 지운 것이 아니라 접은 것이다.
- 비어 있을 때 뜨던 알림은 **앞 절반을 그대로 뒀다** — "요소가 없습니다." 와 "저장한 묶음이 없습니다." 는 여전히 빈 자리에 보이고, 그 뒤에 붙어 있던 방법 설명만 `?` 로 들어갔다. 비었다는 사실은 설명이 아니라 상태다.
- 제목이 아예 없던 요소 목록에 제목을 붙였다. `?` 를 걸어 둘 자리가 있어야 했다.
- 옮긴 문장은 **팝오버를 열지 않아도 화면 낭독기에는 그대로 읽힌다.** 눈으로 보는 사람에게만 접은 것이다.

### 추가 — 도형과 도형을 선으로 잇는다

- **모든 도형에 선을 붙일 자리가 아홉 군데씩 생겼다.** 네 모서리 · 네 변의 가운데 · 한가운데 — 도형의 종류가 무엇이든 같은 아홉이다. 자리가 도형마다 다르면 손이 도형마다 그 자리를 새로 배워야 한다. 이 아홉은 저장하지 않고 도형의 외곽선에서 그때그때 셈한다. 저장하지 않으므로 **도형을 고친 뒤에 어긋나 있을 수가 없다.**
- **원하는 자리에 앵커를 직접 더할 수 있다.** 앵커 도구를 고르고 네모난 자리를 차지하는 요소(사각형 · 타원 · 카탈로그 도형 · 그룹) 위에서 더블클릭하면 그 자리에 앵커가 서고, 그 앵커를 다시 더블클릭하면 없어진다.
- **손으로 더한 앵커는 도형의 크기를 바꿔도 제 자리를 지킨다.** 별의 뿔 끝에 앵커를 놓고 별을 두 배로 키워도 여전히 뿔 끝이다. 여러 꼭짓점으로 그려진 도형에서는 꼭짓점 가까이에 놓으면 그 꼭짓점에 딱 붙는다.
- 선과 텍스트에는 앵커를 더할 수 없다. 텍스트 상자의 크기는 정해 둔 수가 아니라 **글자를 재어 나온 수**라, 그 위에 놓은 앵커는 값이 바뀔 때마다 미끄러진다.
- **연결선은 네 가지다** — 직선 · 꺾은 선 · 곡선 · 자유선. 앵커에서 앵커로 끌면 이어지고, 빈 자리에서 손을 놓으면 그 끝은 그냥 그 자리에 선다.
- **이어 둔 도형을 옮기거나 크기를 바꾸면 선의 끝이 따라간다.** 선이 좌표를 베껴 두지 않고 어느 도형의 어느 앵커인지만 적어 두기 때문이다.
- 선을 고르면 양 끝과 꺾임점마다 손잡이가 붙는다. 끝 손잡이를 다른 앵커로 끌면 붙는 자리가 바뀌고, 꺾임점을 끌면 그 점이 옮겨진다. 선 위 아무 데나 더블클릭하면 **누른 자리에 맞는 차례로** 꺾임점이 하나 들어가고, 꺾임점을 더블클릭하면 없어진다. 곡선에서는 꺾임점이 곡선을 제 쪽으로 당긴다.
- 자유선은 손이 그린 길을 그대로 적되 점을 솎아 낸다. 솎지 않으면 선 하나가 대시보드가 저장할 수 있는 양을 통째로 먹는다.
- **갈 곳을 잃은 선을 조용히 버리지 않는다.** 가리키던 도형이나 앵커 자리가 없어지면 그 선은 그려지지도 눌리지도 않되, 목록의 그 줄이 끊어졌다고 알린다 — 끝점을 다른 앵커에 다시 놓으면 살아난다.
- 요소를 지우면 거기 붙어 있던 연결선도 함께 지워진다. 캔버스에서 사라지면서 목록에서도 사라진다. **캔버스에는 되돌리기가 없으므로 이 지움도 되돌릴 수 없다** — 도형 하나를 지우면 거기 모이던 선을 전부 다시 그어야 한다.
- **연결선의 겉모습은 아직 어느 방법으로도 바꿀 수 없다.** 색도 굵기도 점선 여부도 정하는 자리가 없어, 그은 선은 처음 나온 그 파란 선 그대로다. 값에 따라 색이 바뀌게 하는 것도 같은 이유로 되지 않는다 — 값과 규칙을 붙이는 자리가 저장되기는 하지만 그것을 읽어 겉모습을 정하는 쪽이 아직 없다. 경보가 뜨면 빨개져야 할 선이 빨개지지 않는다. 목록에서 연결선 줄을 펼쳐도 고칠 칸이 하나도 나오지 않는 것은 그래서다 — 눌러도 아무 일도 하지 않는 칸을 보이느니 내놓지 않기로 했다.
- 연결선은 맨 바깥의 도형과 그룹에만 붙고 **그룹 안의 부품에는 붙지 않는다.** 부품은 그룹이 정한 자리 위에서 살고, 그룹을 풀거나 부품을 빼내는 순간 그 부품은 새로 매겨진다 — 부품에 선을 붙이려면 그 두 경우에 선이 어떻게 되어야 하는지를 먼저 정해야 한다. 빠뜨린 것이 아니라 아직 손대지 않은 몫이다.
- **도형을 옮겨도 꺾임점은 제자리에 남는다.** 따라가는 것은 양 끝뿐이다. 손으로 찍은 꺾임이 남의 사정으로 미끄러지지 않게 한 것인데, 그래서 크게 재배치한 뒤에는 긴 선의 꺾임을 손으로 다시 밀어 줘야 한다.
- **직선을 한 번 꺾으면 꺾은 선이 되고, 꺾임을 도로 지워도 직선으로 돌아오지 않는다.** 꺾임이 없는 꺾은 선은 직선과 한 획도 다르지 않게 그려지므로, 이 차이는 목록에 적힌 이름에서만 보인다.

### 변경 — 기본 도형 네 가지를 카탈로그 도형과 같은 줄에 세운다

- 사각형 · 타원 · 선 · 텍스트가 **제 그룹에서 나와 `기본` 그룹의 맨 앞으로** 들어갔다. 도형 서랍의 그룹이 넷에서 셋이 됐다.
- 네 가지를 **카탈로그의 도형 30종과 같은 그리개로** 그린다. 종전에는 같은 격자 안에서 네 가지만 윤곽선 아이콘이고 나머지는 칠해진 파란 도형이라, 한 목록에 두 종류의 그림이 섞여 있었다.
- `기본` 그룹은 펼친 채로 열린다. 서랍을 여는 순간 네 가지가 보인다.

### 추가 — LG 시스템 에어컨을 제조사가 낸 문으로 제어한다

- **LG 가 공식으로 파는 게이트웨이(PMBUSB00A)를 붙일 수 있다.** 종전의 LG 에어컨 연결은 실내기와 실외기가 주고받는 신호를 옆에서 엿듣고 해독한 것이었다. 이제는 LG 가 표준 Modbus 로 내보내는 창구에 정면으로 접속한다. 에이전트 타입 `lg_hvacr03`, 실내기 최대 16대(게이트웨이 4개를 엮으면 64대).
- **엿듣기로는 보이지 않던 것들이 보인다** — 에러 코드, 알람, 필터 청소 알림, 그리고 리모컨 잠금 다섯 종류. 종전 경로에서 "미확인"으로 남아 있던 항목들이다.
- **제어가 흉내내기에서 정식 쓰기로 바뀌었다.** 종전에는 컨트롤러가 실내기 상태를 주기적으로 덮어쓰기 때문에, 실내기에 직접 명령을 보내도 곧 지워졌다 — 그래서 실내기인 척 가장해 컨트롤러에 "설정이 바뀌었다"고 보고하는 우회로를 썼다. 공식 경로에는 그 트릭이 필요 없다. 시퀀스 번호 추적도, 자기 신호 걸러내기도, 버스가 조용해지기를 기다리는 일도 전부 사라졌다.
- **제어가 먹지 않으면 왜 먹지 않았는지 알려준다.** 리모컨 잠금이 걸린 항목은 게이트웨이가 정상 응답을 돌려주면서 값만 조용히 버린다. 그래서 명령을 보낸 뒤 다시 읽어 확인하고, 어긋났는데 잠금이 걸려 있으면 어느 잠금인지 함께 알려준다.
- **모드만 바꾸면 온도가 따라 바뀌는 함정을 구조적으로 막았다.** 에어컨은 모드마다 마지막 온도와 풍량을 기억한다 — 냉방에서 난방으로 바꾸면 난방에서 마지막에 쓰던 온도로 돌아간다. 모드·풍량·온도를 함께 지정할 때는 세 값을 **한 번의 전송으로** 쓰므로 중간 상태가 생기지 않는다.
- 플로우 노드 `lg_hvacr03_status` · `lg_hvacr03_control` · `lg_hvacr03` 을 쓸 수 있다. 상태 메시지 형식을 종전 에이전트와 같게 맞췄기 때문에 **노드 구현을 그대로 공유한다** — 새로 쓴 노드 코드는 없다.
- **LG 매뉴얼이 스스로 "직접 재 보라"고 적어 둔 세 값은 설정으로 열어 뒀다** — 온도 배율, 실내기 번호 시작값, 풍량의 "자동" 코드. 기종이나 펌웨어에 따라 다를 수 있는 값들이라, 현장에서 다르게 나오면 설정 한 줄만 고치면 된다. 다시 빌드할 필요가 없다.
- 이 경로는 압축기 주파수나 제상 상태를 내주지 않는다. 그 정보가 필요하면 종전의 엿듣기 경로(`lg_hvacr02`)를 함께 쓴다 — 서로 다른 버스를 보므로 둘을 동시에 붙일 수 있다.
- 환기(ERV)와 하이드로킷(THERMA V) 명령은 매뉴얼대로 구현했으나 **장비가 없어 실제로 확인하지 못했다.** 에어컨에는 이 명령들이 보이지 않도록 막아 두었다.

### 수정 — 실내기를 하나만 골라 보려 하면 아무것도 보이지 않던 문제

- 상태 노드에는 실내기 하나만 골라 받는 설정이 있는데, **그 값을 넣으면 오히려 전부 걸러졌다.** 주소를 두 자리까지만 읽을 수 있는 자리에 여덟 자리 주소를 넣어 비교했기 때문에, 비교 자체가 매번 실패했다. 이제 주소를 있는 그대로 맞춰 본 뒤 실패했을 때만 예전 방식으로 되돌아간다.
- 같은 자리에서 새 게이트웨이의 열 번째 이후 실내기도 어긋났다. 10 을 십육진수로 읽어 16번 실내기를 찾고 있었다.

### 수정 — 다크 모드에서 글자가 배경에 묻히던 문제

- **앱이 고른 테마와 OS 설정이 서로 다르면 화면이 반씩 갈렸다.** 배경 같은 팔레트 색은 앱의 선택을 따르는데 글자 같은 색은 **OS 설정**을 따라, 밝은 OS + 야간 테마에서 어두운 카드 위에 검은 글자가 남았다. 플로우 편집기의 노드 제목이 배경에 묻혀 읽히지 않던 것이 이 자리다. 이제 1,209개 색 규칙이 전부 앱이 고른 테마를 따른다.
- **플로우 편집기가 팔레트를 무시하던 자리 111곳을 열었다.** 노드 · 연결선 · 핸들 · 툴바 · 포트 패널 · 영역 상자 · 컨텍스트 메뉴 · 캔버스 점격자 · 미니맵. 종전에는 색을 바꿔도 편집기만 그대로였다.
- **노드 부제(노드 타입 글자)는 어느 테마에서도 같은 회색이었다.** 밝은 테마에서 카드 대비가 **2.56**으로, 웹 접근성 기준(4.5)의 절반을 조금 넘는 값이었다. 이제 **4.71**이다.
- 야간의 캔버스 점무늬가 밝은 회색으로 남아 있던 것도 함께 고쳤다 — 그 값 하나가 주간용으로 박혀 있었다.
- 터미널(디버그 출력 패널)은 그대로 둔다. 콘솔이 테마를 따라 밝아지는 것은 고침이 아니라 다른 제품이다.

### 추가 — 테마를 고칠 때 결과를 미리 본다

- 설정 → 테마 탭에 **축소 화면**을 뒀다. 표 옆 **절반**을 차지하며, 표를 훑어 내려가도 화면 밖으로 밀려나지 않는다.
- 화면이 **넷**이고 탭으로 고른다 — **대시보드**(통계·차트 패널) · **플로우**(캔버스·노드·연결선·영역) · **에이전트 · 디바이스**(목록 머리글·행·상태 배지) · **스케줄**(이름·활성 배지·계획). 앱 셸과 단추·상태 줄은 어느 탭에서나 보인다. 색은 한 화면에서만 쓰이지 않으므로 하나만 보여 주면 "대시보드에서는 어떻게 보이나" 에 답할 수 없다.
- 미리보기는 **지금 화면에 적용된 테마가 아니라 편집 중인 테마**를 그린다. 종전에는 낮에 앉아 야간 팔레트를 고치면 아무것도 바뀌지 않아, 눈을 감고 칠하는 것과 같았다.
- **표의 색 항목을 누르면** 미리보기에서 그 색이 쓰이는 자리가 강조되고, 그 자리가 다른 화면에 있으면 그 화면으로 옮겨 간다. **미리보기의 조각을 누르면** 그 조각이 쓰는 색 항목들이 표에 표시되고 첫 항목으로 스크롤된다. 이름만 보고 "이 색이 어디에 쓰이는가" 를 아는 사람은 없다.
- 두 방향 다 **누르는 일**이다. 커서가 지나가는 것만으로 바뀌면 표를 훑어보는 동안 미리보기가 쉬지 않고 움직인다. 커서 따라가기를 원하면 미리보기 위의 **「커서 따라가기」** 를 켠다(기본 끔).
- 고른 조각이 없는 화면으로 옮기면 화면이 바뀌고 **선택이 비워진다.** 없는 조각을 고른 채로 두면 그 선택은 아무 데도 보이지 않으면서 표의 항목만 계속 물들여, 화면과 표가 서로 다른 이야기를 한다.
- 색 항목이 21개에서 **29개**로 늘었다 — 캔버스 점격자 · 연결선 · 영역 상자, 그리고 **글자용 상태색 다섯**.

### 추가 — 에이전트 · 디바이스 목록과 스케줄의 상태 배지가 테마를 따른다

- 표의 구조(머리글 · 행 · 구분선 · 이름)는 이미 팔레트를 따르고 있었으나 **상태 배지는 따라오지 않았다.** 세 화면에 박힌 색 81개를 전부 옮겼다 — 이제 테마에서 오류색을 바꾸면 목록의 오류 배지도 바뀐다.
- 옮기면서 **글자용 상태색 다섯**을 새로 뒀다(`실행 글자` · `정지 글자` · `오류 글자` · `경고 글자` · `정보 글자`). 기존 다섯은 **점·막대를 칠하라고** 고른 값이라 글자로 쓰면 밝은 테마에서 웹 접근성 기준(4.5)을 넘지 못한다 — 실행이 3.21, 주의가 2.86이었다. 점의 대비 기준과 글자의 대비 기준은 다르므로 한 값으로 둘을 다 하려 하면 어느 한쪽이 진다.
- 새 값은 배지의 옅은 칠 위에서도, 카드 면 위에서도 기준을 넘도록 **계산해** 골랐다. 밝은 테마 최저 4.99, 어두운 테마 최저 5.42.

### 수정 — 소스에 박힌 NUL 바이트

- 세 파일이 문자열 구분자로 NUL 을 날바이트로 담고 있었다. 동작에는 문제가 없었으나 `grep` 이 그 파일을 이진으로 판정해 **통째로 건너뛰었고**, 그 침묵 때문에 이번 작업의 첫 재고 조사가 파일 하나를 통째로 놓쳤다.

### 변경 — 색을 고르는 자리를 하나로 모은다

- 배경색 · 도형 채움 · 선 색 · 글자 색 — **색을 고르는 자리 39곳이 같은 판을 연다.** 종전에는 셋이었다: 브라우저 기본 색 대화상자(22곳), 프리셋 열 칸만 있는 스와치(17곳), 그리고 그 둘을 합친 패널 색 칸. 어느 화면을 열었느냐에 따라 고를 수 있는 색도, 고르는 방법도 달랐다.
- 새 고르개는 채도 판 · 색상 슬라이더 · 불투명도 슬라이더 · 16진 칸 · 28색 팔레트 · **최근 쓴 색 12칸** · 스포이트 · 비우기를 한 판에 담는다. 스포이트는 **없는 브라우저에서는 단추를 그리지 않는다** — 눌러도 아무 일이 없는 칸은 설명이 필요해진다.
- 갈라져 있던 **프리셋 여섯 벌을 한 벌로 접었다.** 여섯은 서로 다른 팔레트가 아니라 같은 팔레트의 사본이었고 그중 둘이 한 칸씩 어긋나 있었다 — 사용자가 겪은 것은 "같은 색인 줄 알았는데 화면마다 목록이 다르다" 였다.
- **투명도(알파)는 자리마다 갈라 켰다.** 여덟 자리 색(`#rrggbbaa`)을 저장할 수 있는 자리는 그 값을 읽는 모든 코드가 여덟 자리를 견디는 자리뿐이다. 글자색 일가 · 타일 배경 · 테마 토큰은 켜지 않았다 — 그 경로에는 여섯 자리만 통과시키는 관문이 있어서, 켜면 사용자가 정한 색이 **예외도 경고도 없이 사라진다**. 어느 자리가 견디는지는 짐작이 아니라 32개 저장 키를 전수 조사해 정했다.
- 색 뒤에 알파 두 자리를 문자열로 이어붙이던 12곳을 걷어냈다. 여섯 자리에 두 자리를 붙이면 여덟 자리가 되지만 **여덟 자리에 붙이면 열 자리**가 되고, 브라우저는 그 선언만 조용히 버린다 — 테두리가 사라지는 것으로만 관측되고 콘솔에도 시험에도 아무것도 뜨지 않는 종류다.
- 색 값의 자릿수를 세는 코드를 한 곳(`colorFormat`)에 모았다. 갈라짐이 조용히 돌아오지 못하도록 **CI 가드 넷**을 세웠다 — 네이티브 색 입력 · 철거한 옛 부품 이름 · 프리셋 사본 · 알파 이어붙이기. 넷 다 타입 검사도 린트도 잡지 못하는 종류라, 실제로 무는지 일부러 어겨 확인했다.

### 수정 — draw.io 로 그린 도면이 캔버스에 제대로 들어온다

- draw.io 는 SVG `<text>` 를 **한 번도 내보내지 않는다.** 이름표마다 `<switch>` 를 내고 그 안에 XHTML `<foreignObject>` 와 래스터 대안 `<image>` 를 담는다. `<text>` 만 찾던 파서에게 그 문서에는 글자가 하나도 없었고, 사용자는 도형만 들어오고 **글자는 전부 사라진** 그림을 받았다. 이제 그 이름표를 문구 요소로 세운다. 글상자는 `<image>` 형제가 준다 — 바깥 `<div>` 가 조각에 따라 더 정확하지만 draw.io 의 세로 맞춤 세 갈래 중 하나에서만 참이고, 한 갈래에서만 참인 산술은 나머지 둘에서 말없이 틀린다.
- 들어가지 않기로 한 그릇이 **그 안의 도형과 글자를 통째로 삼키고도 아무 말을 하지 않았다.** 내려가지 않는 판단 자체는 그대로 둔다 — `<switch>` 는 조건이 참인 첫 자식 **하나만** 그리므로 전부 내려가면 대안을 겹쳐 그린다. 바뀐 것은 잃은 것이 있을 때 보고에 올린다는 하나뿐이며, 그래서 잃음이 없는 그릇(잉크스케이프가 거의 모든 파일에 내보내는 `<sodipodi:namedview>` 따위)은 여전히 조용하다. "글자가 없다" 와 "그릇 하나를 건너뛰었다" 를 나란히 놓아야 그 둘이 같은 사건임이 보인다.
- 지금의 draw.io 는 도형마다 표현 속성과 `style` 을 함께 내보내고 `style` 쪽에 `light-dark(...)` 를 적는다. `style` 이 이기는 것은 CSS 대로지만 이긴 값을 읽지 못해 **손에 쥔 `fill="#f5f5f5"` 를 버리고 씨앗 색을 칠했다** — 가져온 도형이 전부 같은 파랑이던 증상의 원인이다. 이제 `light-dark(A, B)` 는 밝은 쪽을, `var(--x, F)` 는 적어 둔 대체값을 고르고, 그러고도 읽지 못한 칠은 같은 요소의 표현 속성으로 되돌린다. 대체값 없는 `var(--x)` 는 짐작하지 않고 "읽지 못했다" 로 보고에 올린다.
- 같은 해독을 `<style>` 규칙표에도 적용한다. 한쪽만 풀면 같은 파일 안에서 인라인으로 적힌 색은 살아나고 규칙표로 적힌 색은 씨앗 색으로 떨어져, 보는 사람에게는 "어떤 것은 되고 어떤 것은 안 되는" 설명 없는 어긋남이 된다.
- draw.io 가 글자마다 함께 내보내는 **투명한 잡이 사각형**(`fill="none" stroke="none"`)을 요소로 세우지 않는다. 원본에서 보이지 않던 것이 빈 네모로 서고, 상한을 이름표 하나가 둘씩 먹었다. 판정은 표현 속성 · `style` · 규칙표 · 상속을 모두 지난 **뒤**의 칠을 본다 — `<g stroke="#666"><rect fill="none"/></g>` 를 잘못 버리지 않기 위해서다. 이 버림은 보고하지 않는다: draw.io 파일마다 이름표 수만큼 울리면 보고 전체가 침묵과 같아진다.

### 추가 — 들일 수 있는 도면의 크기를 서버 설정으로 정한다

- 가져오기 상한을 **요소 64 → 1024, 명령 640 → 10240** 으로 올렸다. 64 는 실무 도면을 한 장도 받지 못했다 — draw.io 는 이름표 하나를 요소 하나로 내므로 화면 한 장짜리 설정 도면도 60을 넘고, 넘으면 앞부분만 들어오는 것이 아니라 **통째로 거절**이다. 명령 상한을 함께 올린 이유는 실파일이 도형당 약 7.3 명령을 쓰기 때문이다 — 요소 상한만 올리면 2.7배 큰 도면이 요소 1024 에 한참 못 미친 채 명령 상한에서 거절된다.
- 요소 상한을 이제 **운영자가 정한다** — `dashboard.max_canvas_elements`(기본 1024). 기본값이 프론트엔드가 그동안 컴파일 상수로 들고 있던 그 수이므로, 설정을 쓰지 않는 기존 설치는 **동작이 달라지지 않는다.** 명령 상한은 설정 키를 따로 두지 않고 `요소 수 × 10` 으로 따라간다 — 요소 수만 올려 봤자 명령 상한이 새 죔쇠가 되면 올린 뜻이 사라진다.
- **관계를 뒤집었다.** 예전에는 고정 예산(256 KB)이 있고 가져오기 상한이 그 아래에 머물러야 했다. 이제 운영자가 몇 개짜리 도면을 들일지 정하고 서버가 그 수에 필요한 자리(요소당 1 KiB)를 확보한다 — 기본 1024 개에서 1 MiB 다. 예산을 따로 설정하게 두지 않는 것은, 두 수를 각각 두면 "요소 2048 · 예산 256 KB" 같은 어긋난 조합이 설정 파일에서 만들어지고 그 어긋남이 **가져오기는 통과하되 저장이 413 으로 실패하는** — 원인이 보이지 않는 — 실패가 되기 때문이다.
- 바닥 256 KB 와 천장 8 MiB 를 함께 세웠다. 바닥은 요소 수를 줄인 설정이 **이미 저장되던 대시보드를 깨뜨리지 못하게** 한다(요소 수를 줄인 운영자가 원한 것은 가져오기를 죄는 것이지 기존 대시보드를 저장 못 하게 하는 것이 아니다). 천장은 페이로드 상한이 DoS 방어물이라는 성질을 지킨다 — 설정값은 오타 하나로 10 억이 될 수 있고, 그 수가 곧 요청 하나가 먹을 수 있는 메모리다. 요소 천장(8192)은 따로 고르지 않고 예산 천장에서 유도해 둘이 어긋날 수 없게 했다.
- 편집기가 서버와 같은 수를 쓰도록 `GET /api/v1/dashboard-limits` 를 냈다. 권한은 `dashboard.read` 이고 admin 게이트를 두지 않는다 — 이 값은 비밀이 아니라 편집기가 동작하려면 알아야 하는 수이고, admin 으로 죄면 admin 이 아닌 편집자만 컴파일 기본값으로 떨어져 운영자가 상한을 올려도 **그들에게만** 가져오기가 거절된다.
- **못 읽으면 컴파일 기본값으로 서고 가져오기는 계속된다** — 오프라인 · 구형 서버(404) · 권한 없음(403) · 응답 형상 변경 전부. 상한이 낡는 것보다 가져오기를 아예 못 하게 되는 것이 나쁘기 때문이다.

### 수정 — 413 응답이 실제로 적용된 상한과 다른 수를 말하던 문제

- `errPayloadTooLargeAPI` 가 인자 없이 대시보드 상수를 메시지에 박고 있어서, **12 MiB 에서 잘린 자산 업로드가 "262144 바이트를 넘었다" 고 답했다** — 48배 틀린 수다. 상한을 인자로 받게 바꿔 각 경로가 자기 상한을 말한다.

### 추가 — 패널 요소를 화면에서 직접 끌어 배치한다

- **통계 패널의 세 요소**(본값 · 변화량 · 구간 통계)를 각각 **독립적으로** 끌어 옮기고, 모서리 손잡이로 글자 크기를 바꾸고, 더블클릭으로 글꼴 · 크기 · 색 · 굵기를 정한다. 설정 다이얼로그를 열지 않고 보이는 자리에서 고친다.
- 크기 축은 **절대 px 하나로 통일**했다. 기존 배율(`value_scale` · `sub_value_scale`)은 px 를 직접 지정하지 않았을 때의 폴백으로만 남는다 — 두 축이 곱해지면 "배율을 2로 했는데 왜 이 크기냐" 를 설명할 수 없다.
- 오프셋은 패널 상자 대비 백분율이고 `left`/`top` 으로 편다. `transform: translate(%)` 은 백분율의 기준이 **요소 자신의 크기**라, 요소가 패널 폭의 절반이면 마우스의 정확히 절반 속도로 따라오는 증상이 있었다.
- **편집 중 보조 3종**: 뒤에 10% 격자와 중심 `+` 표식이 깔리고, 패널 아래에 정렬 툴바가 뜬다. 툴바는 가로 3종 · 세로 3종 정렬과 격자 맞춤(자석) 토글, 배치 초기화를 한 줄에 담는다. 격자 맞춤은 `Alt` 를 누르는 동안 잠시 꺼져 정밀 조정을 막지 않는다.
- **선택 모델**: 요소를 눌러 고르고 `Shift` · `Ctrl` · `Cmd` 로 여럿을 고른다. 고른 것은 진한 실선, 아닌 것은 흐린 점선이며 크기 손잡이는 고른 것에만 붙는다. 무리를 끌 때 한 요소가 상한에 닿으면 **전체가 함께 멈춘다** — 각자 죄면 맞춰 둔 대형이 무너진다.
- 정렬은 고른 요소들이 이루는 **바깥 상자** 기준이다(디자인 도구의 정렬과 같다). 2개 이상 골랐으면 고른 것끼리, 아무것도 고르지 않았으면 그려진 전부를 맞춘다. 세로 정렬이 세로로 쌓인 요소를 겹치게 만드는 것은 그 조작의 뜻이므로 막지 않되, 되돌릴 수단(배치 초기화)을 같은 줄에 뒀다.
- 편집 입구는 **3겹**이다. 읽기 전용(설정 콜백 없음)에는 아예 없고, 설정 미리보기는 항상 켜져 있으며, 대시보드는 편집모드에 들어간 뒤 패널의 토글을 눌러야 켜진다. 편집 상태는 config 에 저장하지 않는다 — 저장하면 다음에 열 때도 켜져 있고 그것을 끄는 방법이 화면에 없다.
- 끌어 고치는 값은 미리보기 디바운스(200ms)를 건너뛴다. 이 목록에 새 값을 넣는 것을 잊어 "드래그가 뚝뚝 끊긴다" 는 보고가 반복됐던 자리다.

### 추가 — 바·파이 차트도 같은 편집 장치를 쓴다

- 통계용으로 만든 편집 장치(격자 · 중심 표식 · 정렬 툴바 · 선택 모델 · 크기 손잡이)를 **바와 파이로 넓혔다.** 모듈을 복사하지 않고 공용 모듈로 끌어올려 세 패널이 하나를 함께 쓴다 — 이 기능은 이미 겉으로 드러나지 않는 계산 실수로 세 번 고쳐졌고(절반 속도 · 스냅 기준 · 무리 이동), 사본이 셋이면 다음 실수는 세 곳에서 각각 발견된다.
- **바 차트에 범례를 신설했다.** 바는 끌 대상이 그림 하나뿐이라 라인 · 파이와 구성이 어긋나 있었다. 설정 키와 표현은 파이와 **같게** 뒀다 — 두 패널의 데이터 모양(카테고리 · 값 · 색)이 같은데 설정이 다르면 같은 것을 두 번 배워야 한다.
- 바 범례의 기본은 **꺼짐**이다. 파이는 원래 범례가 있었으므로 기본 켜짐이 맞지만, 바에 기본을 켜면 저장된 모든 바 차트에 갑자기 범례가 나타나 그림을 덮는다.
- 바 범례는 비중을 적지 않는다 — 막대는 합계 대비 비중을 읽는 그림이 아니다.
- 바의 그림 상자 키(`plot_offset_x` · `plot_offset_y` · `plot_size`)는 **라인 차트와 공유한다.** 같은 뜻의 값에 패널마다 다른 이름을 두면 패널 유형을 바꿀 때 설정이 사라진다.
- 파이의 크기 손잡이는 그림 비율(`pie_size`, %)을, 범례의 손잡이는 글자 크기(px)를 바꾼다. 그림은 글자가 아니므로 px 이 아니라 백분율이 맞다.

### 변경 — 테마를 프리셋 2종 + 프리셋별 덧칠로 바꾼다

- 테마 모드는 **시스템 · 주간 · 야간** 셋이고 `custom` 모드는 사라졌다. 커스터마이즈는 이제 모드가 아니라 **모드의 속성**이다 — 주간과 야간의 색을 각각 따로 덧칠하고, 모드를 오가도 각자의 덧칠이 살아 있는다. 종전에는 커스텀으로 들어가는 순간 주/야 구분 자체를 잃었다.
- **진입점은 설정 → 테마 탭 하나다.** 헤더와 대시보드 헤더에 있던 테마 드롭다운을 걷어냈다. 같은 값을 고치는 자리가 셋이면 어느 것이 이기는지 화면이 설명하지 못한다.
- 팔레트 편집기는 5갈래 21개 토큰(배경 5 · 글자 4 · 테두리 3 · 상태 5 · 상호작용 4)을 색 선택기와 hex 칸으로 편집한다. 토큰 하나만 되돌리기, 팔레트 통째로 되돌리기, JSON 내보내기 · 가져오기를 제공한다. 저장 단계가 따로 없고 고치는 즉시 화면 전체에 반영된다 — 되돌리는 길이 초기화이므로 확인 단계가 필요 없다.
- 저장은 **기본값과 다른 값만** 남긴다. 프리셋과 같은 값까지 저장하면 나중에 기본 팔레트를 손질했을 때 그 변경이 사용자에게 도달하지 못한다.
- 저장 형식이 바뀌어 **v5 → v6 이관**을 넣었다. 실제로 `custom` 모드를 쓰던 사용자의 팔레트만 주간 덧칠로 옮기고, 쓰지 않던 사용자의 옛 팔레트는 버린다 — 되살리면 v0.7.0 에서 조정한 밝기가 통째로 되돌아간다.
- **주간 팔레트 값을 다시 골랐다.** 배경 5단(가라앉음 → 기본 → 보조 → 표면 → 떠오름)이 서로 다른 값이면서 밝기가 단조 증가하도록 맞췄고, 상태색과 상호작용색을 한 단 진하게 옮겨 흰 배경에서 읽히게 했다. 야간은 보조 배경과 흐린 상호작용색 둘만 손봤다.
- 앱 전반의 하드코딩 색을 `--color-*` 토큰으로 옮겼다. 의미를 갖는 강조색은 그대로 뒀다 — 예를 들어 메타데이터 칩에서 중립인 `string` 만 토큰으로 바꾸고 `int` · `float` · `boolean` 은 자기 색을 지킨다.
- 적용은 `<html>` 의 `data-theme` 속성이 정본이고, `.dark` 클래스는 아직 남아 있는 Tailwind `dark:` 변형과의 호환을 위해 함께 붙인다. 사용자 덧칠은 그 위에 인라인 CSS 변수로 얹힌다.
- `index.css` 의 첫 화면용 팔레트와 `tokens.ts` 의 팔레트가 갈라지는 것을 막는 **대조 시험**을 넣었다. 두 벌이 실제로 어긋난 적이 있다(v0.6.0 의 기본 배경 `#f1f5f9` 대 `#f9fafb`).

### 추가 — 고정 설치 디바이스가 재시작 직후 목록에 돌아온다

- 종전 복원 경로는 **이미 발견된 디바이스만** 다시 등록할 수 있어 순환이었다. LoRaWAN 센서는 업링크가 와야 발견되므로, 서버를 재시작하면 고정 설치로 표시해 둔 디바이스가 **첫 업링크가 올 때까지 목록에서 사라졌다.** 보고 주기가 긴 센서에서는 그 공백이 길다.
- 디바이스 메타데이터에 소유 정보 두 가지(`agent_name` · `local_id`)를 기록한다. 어느 에이전트의 어떤 주소인지를 알면 디바이스가 살아 있지 않아도 복원할 수 있다.
- 소유 정보는 **서버가 레지스트리에서 읽어 덮어쓴다.** 요청 본문에 실려 온 값은 버린다 — 소유권을 요청으로 위조할 수 있으면 남의 에이전트에 디바이스를 심을 수 있다.
- 복원은 3단 폴백이다. (1) 저장된 소유 정보 → (2) `device_ids` 저장소의 역인덱스 → (3) 지금 살아 있는 디바이스. 어디에서도 찾지 못하면 아무것도 지어내지 않는다. (2)나 (3)으로 찾았으면 그 값을 메타데이터에 되써 다음 부팅부터는 (1)로 끝난다 — 이 필드가 생기기 전에 고정해 둔 기록도 사용자가 다시 고정하지 않아도 스스로 낫는다.
- ChirpStack 에이전트는 복원된 디바이스를 **오프라인 자리표시자**로 넣는다. 마지막 수신 시각은 0이고 측정값은 비어 있다 — 값을 지어내면 화면의 숫자가 실제 관측인지 복원 흔적인지 구분되지 않는다. 첫 업링크가 오면 실제 데이터가 자리표시자를 대체한다. 그때까지 이름은 DevEUI 다.
- `device_ids` 키를 콜론으로 쪼개지 않고 **접두사로** 가른다. 에이전트 참조와 로컬 ID 양쪽에 콜론이 들어갈 수 있다(MAC 형식 주소).

### 추가 — 값이 낡았는지 항목별로 표시한다

- 디바이스 메타데이터에 `stale_after_sec` 를 뒀다. 에이전트의 오프라인 임계값과는 **다른 축**이다 — 저것은 "이 디바이스가 살아 있는가" 를 답하고, 이것은 "지금 보이는 값을 지금 값으로 믿어도 되는가" 를 답한다. 보고 주기가 제각각인 디바이스를 한 화면에 놓으면 하나의 임계값으로는 둘 다 말할 수 없다.
- 미설정이 기본이며 아무것도 표시하지 않는다. `0` · 음수 · 숫자가 아닌 입력은 미설정으로 접는다 — `0` 으로 두면 방금 도착한 값도 낡은 것이 된다. 비우는 것이 제한을 푸는 유일한 길이다.
- 디바이스 상태 패널에서 낡은 값은 **흐려지고 배지가 붙는다.** 흐리게만 하면 왜 흐린지(값이 낡았는지, 항목이 꺼졌는지, 아직 안 왔는지) 화면이 말하지 않는다.
- 서버는 이 값을 저장만 하고 판정하지 않는다. 낡음은 보는 시점에 정해지므로 그리는 쪽이 계산하는 편이 맞다.

### 변경 — 디바이스 속성 카드의 표시 항목을 정리한다

- 게이트웨이 수신 정보와 사용자 메타데이터를 `gw.` · `meta.` 접두사로 갈랐다. 접두사가 없으면 `rssi` 를 실제로 보고하는 디바이스가 게이트웨이 카드와 같은 자리를 다툰다.
- 설정의 **표시 항목 목록과 패널이 그리는 카드가 이제 같은 함수에서 나온다.** 종전에는 목록이 프로토콜 라벨표에서 나와 디바이스가 보고하지 않는 항목까지 고를 수 있었고, `measurements` 컨테이너와 그 자식이 함께 떴으며, 카드가 될 수 없는 `gateways` 도 목록에 있었다.
- 게이트웨이 카드는 **첫 링크**(백엔드가 gateway_id 로 정렬한 순서)를 쓴다. 가장 센 링크를 쓰면 업링크마다 카드가 다른 게이트웨이로 튄다. 링크가 둘 이상일 때만 개수를 덧붙이고, 링크가 하나도 없어도 항목 자체는 고를 수 있게 남긴다.
- 단위를 **라벨이 아니라 값에 붙인다.** `RSSI (dBm)` 이 `RSSI` 와 `-89 dBm` 으로 갈렸다. 사용자가 지정한 단위가 키에서 추측한 단위를 이기므로 `26.4°C ppm` 같은 겹침이 생기지 않고, 기호 단위(`%` · `°C`)는 붙여 쓰고 낱말 단위(`ppm` · `V`)는 띄어 쓴다. 값이 없거나 숫자가 아니면 단위를 붙이지 않는다.
- 아직 값이 오지 않은 항목 중 **통신으로 채워지는 것만** "수신 대기" 를 낸다. 메타데이터 항목은 자리만 지키고 아무 말도 하지 않는다 — 오지 않을 것에 기다리라고 하면 화면이 거짓말을 한다.
- 라벨 조회가 대소문자를 가리지 않는다. `battery` 와 `Battery` 가 같은 이름으로 읽힌다. 배터리 · CO2 · 압력 · 조도와 LoRaWAN 식별자(DevEUI · AppEUI)의 기본 이름을 추가했다.

### 추가 — 목록·상태 패널의 타일과 글자를 설정으로 정한다

- **에이전트 현황**의 요약 배지 3종(전체 · 활성 · 비활성)을 골라 순서를 정하고, 배지마다 글꼴과 배경을 준다. 아무것도 정하지 않으면 셋 다 종전 순서 그대로 나온다.
- **에이전트 상태**는 통계 타일 5종(수신 · 송신 · 오류 · 가동시간 · 폐기)과 메시지 상세 6값(외부 · 내부 × 수신 · 송신 · 오류)을 고를 수 있고, 격자 위에 끌어 배치하고 크기를 바꾼다. 격자를 줄여도 타일이 지워지지 않는다 — 화면 밖으로 밀려날 뿐 설정은 남는다.
- **디바이스 상태**는 그룹(기본 · 상태 · 게이트웨이)마다 **제 격자**를 갖는다. 그래서 열 수 입력 칸이 사라졌다 — 하나의 열 수로 세 그룹을 함께 정할 수 없다. 카드 안의 라벨 · 값 · 시각 자리도 각각 끌어 배치하며, 겹치면 라벨 → 값 → 시각 순으로 빈칸에 밀어 넣는다.
- 타일 설정은 **공통 → 타일별 2단**으로 합쳐진다. 글꼴은 항목 단위로 섞여서(공통에서 색, 타일에서 크기) 각자 살아남고, 값 색 규칙은 통째로 교체된다 — 규칙을 끼워 넣으면 어느 것이 이기는지 읽을 수 없다.
- 값 색 규칙은 **값에만** 걸린다. 값에 따라 변하는 것은 값이므로 라벨은 건드리지 않는다. 비교는 양쪽이 수로 읽히면 수로 하고, 먼저 걸리는 규칙이 이긴다.
- 목록형 패널 4종(플로우 · 에이전트 현황 · 디바이스 · 에이전트 상태)이 표 머리글 · 표 칸 · 배지 글꼴을 **하나의 규칙**으로 공유한다. 종전에는 제목 색을 정하는 자리가 둘이었다.
- 저장된 설정은 그대로 산다. 옛 `accentElements` 의 표 · 배지 색과 `panelColor` 는 폴백으로 계속 읽히고, 옛 카드 배치(`cardSlots` 1~9, `stack`/`inline`/`value`)도 새 좌표로 옮겨진다.
- 제목 옆의 종류 · 프로토콜 글자를 없앴다 — 배지가 같은 것을 이미 말한다.

### 변경 — 게이지의 현재값을 도형 밖 겹침 층으로 꺼낸다

- 값 글자가 도형과 같은 SVG 안에 있어서, 도형을 줄이거나 옮기면 **값도 함께 끌려가 작아졌고** 캔버스 가장자리에서 잘렸다. 이제 값은 형제 겹침 층이라 도형의 크기 · 오프셋이 값을 끌고 가지 않고, 패널 어디로든 옮길 수 있으며 잘리지 않는다. 기본 자리는 종전과 같다.
- 값 글자에 **크기 손잡이**가 붙었다. 고른 상태에서만 나오고, 글자 아래 가운데에 놓인다 — 자릿수에 따라 폭이 변하는 가운데 정렬 글자는 모서리를 잡아 둘 수 없다.
- **주의**: 값 자리의 저장 단위가 viewBox 에서 패널 대비 백분율로 바뀌었고 이관 코드가 없다. 값 글자를 끌어 옮겨 둔 대시보드는 그 자리를 잃고 기본 위치로 돌아간다. 설정의 초기화 버튼은 옛 키도 함께 지운다.

### 추가 — 세로바 게이지의 바 폭과 높이를 따로 정한다

- 세로바 게이지만 폭과 높이를 **각각** 20~100% 로 정한다. 두 축이 갈리는 것은 세로바뿐이다 — 원형 · 반원 · 바늘형은 각도가 곧 읽는 규약이라 비율을 잠가야 한다.
- 배율은 CSS `scale()` 이 아니라 **SVG 도형 자체를 다시 그린다.** `scale(x, y)` 로 하면 눈금 글자와 값 글자까지 함께 눌린다(보고된 결함).
- 바의 가로 중심과 아래 변은 고정이다. 위와 옆에서 줄어들어 읽는 기준선이 움직이지 않는다.
- 설정의 슬라이더 두 개로도, 게이지 몸통의 손잡이로도 바꾼다. 손잡이는 가로 이동을 폭에, 세로 이동을 높이에 대응시킨다 — 다른 게이지 유형은 종전대로 대각선 하나로 잰다.
- 지정하지 않았거나 범위를 벗어난 값은 100% 로 접는다. 저장된 대시보드의 모양은 그대로다.
- 임계값 범례에도 크기 손잡이가 붙어 글자 크기를 끌어서 바꾼다.

### 변경 — 패널 데이터 소스에서 채널을 걷어낸다

- 그래프·통계·바·파이·표·히트맵·게이지의 데이터 소스 목록에서 **채널이 사라졌다.** 소스 종류는 `Store` · `TSDB` · `시스템 지표` 셋이다. 플로우와 서버측 채널 인프라는 그대로 두었다 — 없앤 것은 패널이 채널을 **읽는 길**이지 채널 자체가 아니다.
- 채널 모드로 저장된 패널은 설정을 여는 순간 **Store 로 옮겨진다.** 옮긴 직후에는 고른 시리즈가 없어 비활성이므로 종전과 같은 값을 계속 그리고, 시리즈를 고르는 순간 Store 로 넘어간다.
- 소스를 정할 수 없는 config 의 폴백도 채널에서 **Store 로** 옮겼다. 신규 패널이 이미 Store 로 태어나고 구 패널도 Store 로 이관되므로, 폴백과 이관 대상이 같아야 두 경로가 갈리지 않는다.
- 지운 모듈 9개: `useChartChannel` · `useChartChannels` · `services/api/charts` · `services/ws/remoteChartChannel` 과 각 테스트.

### 추가 — 한 패널에 데이터 소스를 여러 개 둔다

- 같은 종류를 **둘 이상** 붙일 수 있다(종류당 최대 4개). Store 둘을 나란히 두거나 Store 와 TSDB 를 섞어 한 그래프에 그린다.
- 종류 버튼은 **더하기**다. 토글로 두면 같은 버튼을 다시 눌렀을 때 그 소스의 설정이 통째로 사라진다 — 지우는 길은 인스턴스 머리의 휴지통 하나뿐이다.
- 첫 인스턴스는 기존 설정을 물려받고 둘째부터는 빈 상태로 시작한다. 설정을 복제하면 "같은 것을 두 번 그리는" 상태가 기본값이 된다.
- 인스턴스마다 **접을 수 있다.** 접힘은 화면 상태라 저장하지 않는다 — 패널을 다시 열면 모두 펼쳐진다.
- 계열 이름이 소스 간에 겹칠 때만 접미사를 붙인다. 겹치지 않으면 이름을 건드리지 않는다.

### 변경 — 가져올 데이터의 구간을 패널이 소유한다

- 구간(기간·최근·포인트 수)은 데이터 소스가 아니라 **패널 옵션**이 정한다. 소스마다 구간을 따로 두면 한 그래프 안에서 축이 갈라진다.
- 시스템 지표 소스에서도 구간 옵션이 나온다 — 종전에는 Store/TSDB 에서만 보였다.
- `막대 개수`(`max_points`)는 구간 재정의로 **읽지 않는다.** 바·파이·통계에서 그 값은 "몇 개를 보일까" 이지 "몇 개를 가져올까" 가 아닌데, 12막대 패널이 12포인트만 조회하고 있었다.

### 수정 — 시리즈를 골라도 미리보기에 반영되지 않던 문제

- 미리보기 게이트가 **소스 목록을 보지 않고** 최상위 블록만 보고 있었다. 시리즈 선택은 목록 쪽(`sources[]`)에 쓰이므로 고른 뒤에도 게이트가 닫힌 채였고, 미리보기는 합성 사인파에 머물렀다.
- 활성 판정을 목록 전체 기준으로 바꿨다(`isPanelSeriesActive`). 같은 판정을 쓰던 라인·바·파이·통계 패널도 함께 고쳤다 — 대시보드에서도 같은 함정이 있었다.
- 인스턴스 패치가 소스와 무관한 값까지 인스턴스 안에 묻고 있었다. 히트맵 센서 좌표가 `sources[0]` 밑으로 들어가 패널이 읽지 못했다.

### 추가 — 차트 패널 표시 개편

- **타이틀·범례 디자인**: 폰트·크기·색을 팝오버에서 정한다. 타이틀 설정은 모든 패널이 공유한다.
- **범례·그래프 자유 배치**: 둘 다 끌어 옮길 수 있고, 그래프는 크기도 조절된다. 미리보기의 리셋 버튼이 둘을 한 번에 기본값으로 되돌린다.
- **툴팁 정렬**: 레이블은 왼쪽, 값은 오른쪽에 붙는다. 값은 자릿수 고정 폭이라 세로로 읽힌다.
- **Y축 짤림**: 축 범위가 자동일 때 눈금 폭을 표본 없이 계산해 큰 값이 잘렸다. 데이터 범위에서 표본을 뽑아 폭을 정한다.
- **그래프 스타일 샘플**: 스타일을 바꾸면 샘플에도 즉시 적용된다.
- 끌어 옮기는 값은 미리보기 디바운스(200ms)를 **건너뛴다.** 목록을 한 모듈(`previewLiveKeys.ts`)이 소유하고 대상별 시험을 붙였다 — 새 값을 만들 때 목록에 넣는 것을 잊어 드래그가 끊기는 일이 반복됐다.

### 추가 — Figma 설정 화면 추출 도구

- `design/figma/panel-settings/` — 패널 설정 화면을 Figma 문서로 옮기는 플러그인. 실제 DOM 을 읽어 프레임을 만든다.


### 수정 — 실시간 시스템 지표에서 축 단위 라벨이 안 보이던 문제

- 자동 환산 단위는 축의 최대값으로 배율(B·KB·MB)을 정하는데, **데이터가 아직 없으면 기준값이 없어 라벨을 통째로 지우고 있었다.** 실시간 모드는 시작 직후가 늘 그 상태다 — 누적 카운터는 첫 표본에서 증가량을 구할 기준점이 없어 그릴 점이 없다. 그래서 "단위 설정이 안 먹는" 것처럼 보였다.
- 기준값을 모르면 **접기 이전의 밑단위**(`B` · `B/s`)로 떨어진다. 빈 축은 어차피 0 이므로 0 의 배율이 맞고, 값이 들어오면 그때 배율을 다시 정한다.

### 수정 — 실시간 누적 훅이 같은 표본을 되풀이해 쌓아 렌더 루프에 빠질 수 있던 문제

- 스냅샷 객체의 참조가 렌더마다 새로 만들어지면 "effect → setState → 리렌더 → 새 참조 → effect" 가 끝없이 돌아 화면이 죽는다. **표본 시각으로 빗장을 걸어** 한 표본은 한 번만 쌓는다.
- 쌓을 것이 없으면 계열 Map 의 **같은 참조를 돌려준다**. 새 Map 을 만들면 값이 그대로인데도 리렌더가 한 번 더 돈다 — 첫 표본처럼 전부 환산 불가인 구간에서 매 폴링마다 일어난다.

### 수정 — 라인 차트에서 Y축 눈금과 값이 1000배 어긋나 보이던 문제

- 자동 데이터 량 단위(`auto:bytes`)에서 **축은 1024 로 접어 KB 로 그리는데 툴팁·범례는 원값(바이트)을 그대로 찍고 있었다.** 같은 점이 축에서는 `137`, 범례에서는 `137355.20` 으로 읽혀 "축 범위와 그래프 값이 안 맞는" 것으로 보였다.
- 툴팁·범례도 축과 **같은 배율**로 접는다(`formatTickValue`). 접미사는 여전히 붙이지 않는다 — 단위는 축 라벨이 한 번만 말한다는 기존 규칙을 그대로 따른다.
- **축 라벨이 저장값을 그대로 노출하고 있었다.** `auto:bytes` 는 접미사가 아니라 규칙인데 라벨에 `(auto:bytes)` 로 붙어, 눈금은 KB 인데 라벨은 규칙 이름을 말했다. 이제 축이 실제로 접은 배율(`(KB)`)을 말한다 — 배율은 눈금마다 다시 정하지 않고 축 전체가 하나를 쓰므로 라벨 한 줄로 말할 수 있다.
- 배율 기준값은 도메인 상한이며, 자동 축이라 상한을 모를 때는 데이터 최대값을 찾는다. 스캔은 자동 환산 단위일 때만 한다.
- 테스트 스텁이 고정 표본(12.3456)만 노출해 배율 어긋남을 잡을 수 없었다. 접기 밑(1024)을 넘는 표본을 함께 노출하도록 넓혔다.

### 추가 — 시스템 지표 데이터 소스에 실시간 조회 방식

- 시스템 지표 소스에 **조회 방식**을 둔다. 무엇을 그릴지가 아니라 **언제 것을 그릴지**를 고르는 축이다([useSysmetricsLiveSeries.ts](web/src/pages/dashboard/panels/charts/useSysmetricsLiveSeries.ts)).
  - **이력**(기본): 에이전트가 보관한 구간을 질의한다. 열자마자 과거가 그려지고 닫았다 열어도 선이 남지만, 값은 버킷·집계로 접힌 대표값이다.
  - **실시간**: 에이전트 스냅샷을 폴링해 오는 대로 이어 붙인다. 접지 않아 표본이 그대로 보이는 대신 패널을 연 뒤 구간만 남고 닫으면 사라진다.
- 미지정은 이력이다 — 이 축이 생기기 전에 저장된 패널의 동작이 곧 기본값이다(하위호환).
- 누적 카운터(네트워크·디스크 I/O)의 환산 단위는 **초 고정**이다. 에이전트가 이력에 초당 증가량을 저장하므로, 실시간이 다른 단위로 환산하면 같은 시리즈가 모드에 따라 다른 값으로 보인다. 모드는 "언제 것을 보느냐" 이지 "무엇을 보느냐" 가 아니어야 한다.
- 실시간에서는 **창·인터벌·집계·빈 버킷 칸을 내린다.** 실시간에는 버킷이 없고 표시 창은 오래된 점을 버리는 기준일 뿐이라, 같은 칸을 두면 뜻이 갈리고 "고쳤는데 아무 일도 안 일어나는" 칸이 된다.
- 두 훅을 합치지 않은 이유도 같다 — 이력은 "구간을 질의해 버킷으로 접는" 모델이고 실시간은 "표본을 쌓는" 모델이라, 한 코드에 넣으면 어느 축이 어느 모드에서 의미 있는지가 조건문 속으로 숨는다. 반환 형상은 같으므로 패널 렌더 코드는 소스도 모드도 모른 채 그대로다.

### 수정 — 시스템 지표 소스의 안내 문구가 현재 동작과 어긋나 있었다

- "sysmetrics 에이전트는 이력을 보관하지 않습니다" 라고 안내하고 있었는데, 에이전트가 이력을 보관하도록 바뀐 뒤에도 문구가 그대로 남아 있었다. 이력 모드에는 보관 기간 안내를, 실시간 모드에는 마운트 이후 구간만 그려진다는 안내를 각각 붙인다.

### 변경 — 패널 추가 목록에서 시스템 카테고리를 걷어낸다

- **호스트 지표 3종(시스템 지표·네트워크 지표·스토리지 지표)**을 패널 추가 목록에서 내린다. 앞서 모니터링 패널 4종을 내리고 시스템 로그를 '기타' 로 옮긴 데 이어, 시스템 카테고리에 남아 있던 마지막 항목들이다.
- 항목이 하나도 남지 않아 **'시스템' 카테고리 탭 자체를 내렸다.** 빈 탭은 눌러 본 뒤에야 비어 있다는 것을 알게 되므로 없는 편이 낫다.
- 함께 죽은 경로를 걷어낸다: sysmetrics 에이전트 선택 스텝(도달 지점이 사라졌다), `needsSysMetricsAgent` 옵션 필드, 카탈로그 전용 i18n 키(설명 3종 · 카테고리 이름 · 에이전트 선택 제목).
- **타입과 렌더 경로는 그대로 살려 둔다** — 이미 배치된 패널은 계속 동작하고 설정도 열린다(앞선 모니터링 패널과 같은 방식).

### 변경 — 패널 추가 목록에서 모니터링 패널을 내리고 시스템 로그를 기타로 옮긴다

- **시스템 통계·실시간 메트릭·네트워크·시스템 이벤트**를 패널 추가 목록에서 내린다. 넷 다 xflowd 런타임 자신을 보는 패널이라 모니터링 페이지(사이드 메뉴)와 보는 것이 같고, 대시보드에 다시 늘어놓을 이유가 없다([AddPanelDialog.tsx](web/src/pages/dashboard/AddPanelDialog.tsx)).
- **시스템 로그는 '기타' 로 옮긴다.** 나머지 넷과 달리 런타임 지표가 아니라 로그를 읽는 뷰어라, 호스트 지표만 남은 시스템 카테고리에 두면 성격이 어긋난다.
- **타입과 렌더 경로는 그대로 살려 둔다** — 이미 배치된 패널은 계속 동작하고 설정도 열린다. 목록에서 내리는 것과 기능을 지우는 것은 다르다(선행 패널 `resource`/`logs` 를 내릴 때, `line-chart` → `graph-chart` 때와 같은 방식).
- 시스템 카테고리에 그룹이 하나(호스트 지표)만 남아 그룹 제목을 없앴다. 둘로 나눴던 이유가 "CPU 항목이 두 군데 있는 이유를 설명할 자리" 였는데 그 이유 자체가 사라졌고, 다른 단일 그룹 카테고리도 제목을 쓰지 않는다.
- 카탈로그에서만 쓰이던 i18n 키(설명 4종 + 그룹 제목 2종)를 함께 지운다. 패널 **타입 이름**은 남긴다 — 살아 있는 타입의 표시 이름이라 설정 화면이 계속 쓴다.

### 수정 — SVG 도면에서 "늘려서 채우기" 가 먹지 않고 종횡비도 틀어지던 문제

- **SVG 는 자기 문서 안에 `preserveAspectRatio`(기본 `xMidYMid meet`)를 갖고 있고, 그 규칙이 CSS 를 이긴다.** `<img>` 상자에 `object-fit: fill` 을 걸어 상자를 늘려도 그림은 비율을 지키며 상자 안에서 다시 레터박스된다 — 늘려서 채우기를 골라도 도면이 가운데 작게 남던 이유다.
- 같은 이유로 크기를 안 적은 SVG 는 `naturalWidth/naturalHeight` 가 원본 크기가 아니라 대체 요소 기본값(브라우저에 따라 0 또는 300×400)이라, 스테이지 종횡비를 아예 못 구하거나 엉뚱하게 잡았다. 스테이지가 도면에 안 맞으면 센서가 도면이 아니라 패널을 따라간다.
- **SVG 문서 자체를 고쳐서 쓴다**([svgAsset.ts](web/src/pages/dashboard/panels/heatmap/svgAsset.ts)). 종횡비는 `viewBox`(없으면 width/height 속성)에서 직접 읽고, 늘려서 채우기에서는 루트 `<svg>` 의 `preserveAspectRatio` 를 `none` 으로 바꾼 data-URL 로 갈아 끼운다.
- 인라인(`dangerouslySetInnerHTML`)이 아니라 **data-URL 재작성**인 것이 요점이다. 계속 `<img>` 로 그리므로 사용자가 올린 SVG 안의 스크립트가 실행되지 않는다 — 인라인하면 그 격리가 사라진다.
- 래스터(PNG/JPG)는 손대지 않는다. `object-fit: fill` 만으로 늘어나므로 재작성할 것이 없다.
