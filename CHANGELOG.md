...
- **브라우저가 이미 그린 그 이미지**에서 받아오는 경로를 마지막 보루로 더한다([FloorPlanBackground.tsx](web/src/pages/dashboard/panels/heatmap/FloorPlanBackground.tsx)). 기준 레이어의 `onLoad` 에서 원본 크기를 읽으므로 두 번째 요청도, 인증·CORS 문제도 없다. 도면이 보이는 한 스테이지는 반드시 도면에 맞춰진다.
