...
- 미리보기에서 **끌어 옮긴다**. 잡은 대상으로 갈린다 — 값 글자를 잡으면 값만(게이지 안에서의 자리), 게이지 상자를 잡으면 게이지 전체가(패널 안에서의 자리) 움직인다. 두 축은 좌표계도 다르다: 값은 SVG viewBox, 게이지는 패널 대비 백분율.
...
- **위치**를 설정 미리보기에서 끌어 옮긴다. 오프셋은 픽셀이 아니라 차트 영역 대비 **백분율**로 저장한다 — 픽셀로 두면 패널 크기를 바꿀 때 상대 위치가 어긋난다(범례는 HTML 요소라 픽셀이 자연스러워 그대로 둔다). 중심이 가장자리에 붙어 파이가 절반 넘게 잘리지 않도록 ±40%p 로 죈다.
...
- **설정 미리보기에서 끌어 옮겨** 자리를 미세 조정한다([PieLegendDragLayer.tsx](web/src/pages/dashboard/PieLegendDragLayer.tsx)). 게이지 값 드래그와 같은 규칙이다 — 범례 위에서만 시작되고(패널 크기 조절·휠 확대와 부딪히지 않는다), 이동은 프레임당 한 번만 반영하며, 패널 본문 절반을 넘겨 밀리지 않는다(다시 잡을 수 있어야 한다). 되돌리는 버튼을 설정에 두었다 — 드래그는 미리보기에서만 되므로 그것 말고는 출구가 없다.
