...
| `.moai/project/structure.md` | "web/ › 주요 재사용 컴포넌트" 절에 facility 대시보드 신규 파일 블록 추가 (facilityAggregation.ts / useXsfmControl.ts / Facility*Panel.tsx / facilityShared.tsx + 와이어링 4지점) |
...
지하철 역사 시설물 관리 비전의 첫 디바이스(설비)를 대상으로, 운영자가 라인(호선)→역사(station)→기기(device) 계층으로 시설물을 조망·통계·제어하는 3종 대시보드 패널. 신규 17개 + 수정 8개 프론트엔드 파일. **신규 백엔드 코드 0** — SPEC-XSFM-001의 exec 표면(`list_devices`/`list_stations`/제어 셀렉터)을 소비.
...
- **셀렉터 제어 훅**(`web/src/hooks/useXsfmControl.ts`): `set_power`/`set_fan_speed`/`set_multiple`을 device_id/station/line 셀렉터로 execAgent(`fanOutResponse` 타입) + `useFacilityRoster`(refresh 폴링).
...
| # | 항목 | SPEC 계획 | 실제 구현 | 판정 |
|---|------|-----------|-----------|------|
| 1 | **클라이언트 사이드 집계** (A-2, REQ-05-01) | 집계 표면 | 백엔드 집계 엔드포인트 없이 xsfm `list_devices`/`list_stations` exec를 웹에서 클라이언트 집계 | 채택 — 백엔드 집계 엔드포인트는 대규모 fleet 성능을 위해 **OI-1로 이연** |
...
| 4 | **기기 제어 경로** | (제어 진입점) | 기기 패널이 `deviceService.executeCommand`가 아닌 `useXsfmControl`(device_id 셀렉터 execAgent) 사용 | 기능적 동등(둘 다 에이전트 `Process` 도달) — 구현 선택으로 명시 |
| 5 | **런타임 의존** | — | 대시보드는 xsfm exec 표면(develop 머지 완료)을 소비 → xsfm 백엔드 실행 필요 | 런타임 의존성 기록 |
...
| 셀렉터 제어 훅(useXsfmControl) | 구현 + 테스트 (green) |
