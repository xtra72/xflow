...
| 버전    | 날짜         | 작성자  | 변경 내용                                                                                                                                                                                                                                              |
| ----- | ---------- | ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 0.3.1 | 2026-09-16 | xtra | **도구가 서는 자리를 출시 형상에 맞춘다.** M3' 7 과 M8 1 이 앵커·연결선 도구를 "도크에" 세운다고 적고 있었으나, 도크 열 절 가운데 일곱이 미리보기 제목 아래 **도구 띠**로 옮겨 갔다. 두 자리를 고치고 **무엇이 옛 문장을 거짓으로 만들었는지** 함께 적는다. 하는 일은 그대로이므로 작업 목록은 늘지도 줄지도 않았다 |
...
7. 전용 **앵커 도구**를 세운다. 켜진 동안에만 도형 위 더블클릭이 앵커를 더하고, 앵커 위
   더블클릭이 뺀다. 그룹 도구가 세운 형상을 따른다(두 표면이 같은 컴포넌트). 그 자리는
   미리보기 제목 아래 **도구 띠**(`CanvasEditToolbar.tsx`)다 — 0.3.0 까지 "도크에 세운다"
   였고, 도크 열 절 가운데 일곱을 띠로 옮긴 이사가 그 문장을 거짓으로 만들었다
   (`spec.md` §범위 밖 · 함께 실려 온 넷). **자리만 옮겼고 하는 일은 그대로다.**
...
1. 연결선 도구 넷(직선·꺾은 선·곡선·자유선)을 **도구 띠**(`CanvasEditToolbar.tsx`)에
   둔다. **그룹 도구가 세운 형상**을 따른다 — 두 표면이 같은 컴포넌트를 그린다
   (004 불변식 I24). 0.3.0 까지 "도크에 둔다" 였다. 도구 절 일곱의 띠 이사가 자리를
   바꿨을 뿐, 한 번 짓고 두 표면이 받는다는 I24 는 그대로다
   (`spec.md` §범위 밖 · 함께 실려 온 넷).
