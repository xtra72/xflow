...
### M4 — 템플릿 정의/적용 + config-기본/payload-오버라이드

- **Ubiquitous**: 템플릿은 `(area, address offset)`만 정의한 부분 규칙이며, 노드 config 내부에 op-list(`command_set` 패턴)로 저장된다.
- **Event-driven**: WHEN 템플릿에 `(device_id, start address, count)`를 적용 THEN 구체 규칙 `{source area = 템플릿 area, source address = start, count, target unit_id = device_id, target area = source area(별도 지정 없으면), target address = start + offset}`으로 인스턴스화한다.
- **State-driven**: IF 페이로드에 `rules`/`templates` 오버라이드가 존재하면 THEN config 기본값 대신 페이로드 값을 사용한다(RLock으로 config 기본 읽기 → payload 존재 시 대체).
- **Unwanted**: 시스템은 재사용 named/전역 템플릿 스토리지나 별도 API를 도입하지 않아야 한다(노드 config 내부 저장으로 한정).
