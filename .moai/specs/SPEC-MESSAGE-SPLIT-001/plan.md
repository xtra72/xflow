...
- **Metadata** — `pkg/message/metadata.go`: 값 계약 = string(flat) 또는 map[string]string(group)(line 29-38). `Set(k,v string)`(line 94), `All() map[string]string`(line 107), `Raw() map[string]any`(line 147), `SetGroup`(line 118). `MetaKeyCorrelationID = "_correlationID"`(line 14).
- **JSONPath** — `pkg/message/path.go`: `evaluatePath`(line 11), `$.` 접두사 필수(line 13), `$.items[*]` 와일드카드는 배열 반환(line 116-135), `$.items[0]` 인덱스 지원. 평면 키 `"items"` 는 JSONPath 가 아니므로 `Payload().Get("items")` 로 해석해야 함.
...
1. **배열 추출**: `path` 가 `$.` 접두면 `msg.Payload().GetPath(path)`, 아니면 `msg.Payload().Get(path)`. 결과를 `toAnySlice` 로 슬라이스 변환.
...
4. **요소별 팬아웃**: `results := make([]message.Message, 0, len(arr))`; 각 요소 `elem`(인덱스 `i`)에 대해:
   ...
   - **correlation id** (REQ-17, SHOULD): `out.Metadata().Set(message.MetaKeyCorrelationID, parent.ID()+"#"+strconv.Itoa(i))`.
...
- **Tertiary Goal (M3) — Priority Medium**: 엣지 케이스(on_missing/on_empty passthrough·error·emit_none) + correlation id + 순서 보존(REQ-17,18,19,03).
...
| 평면 키 vs JSONPath 혼동(`"items"` 는 JSONPath 아님) | `$.` 접두 판정으로 `Get` vs `GetPath` 분기(구현 스케치 1단계). |
| `$.items[*]` 와일드카드가 요소 배열이 아닌 투영 결과 반환 가능성 | path.go:124 — 남은 토큰 없으면 배열 그대로 반환. `$.items` 와 `$.items[*]` 모두 배열 반환 확인됨. 테스트로 고정. |
