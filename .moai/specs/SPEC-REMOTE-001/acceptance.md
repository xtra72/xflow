...
### AC-71: 노드 해상도 보고 + 저장·노출 (REQ-M01, M02)

- Given: 노드 config 에 장비 모니터 해상도(`display.resolution="1920x1080"` 또는 `display.width=1920`/`display.height=1080`)가 선언되어 있다.
- When: 노드가 `register`(이후 `heartbeat`)로 접속하고, 관리자가 노드 상세 API 를 조회한다.
- Then: 노드가 `display_width=1920`/`display_height=1080`(px)을 시스템 정보 페이로드로 보고하고(자원 메트릭 CPU/메모리/디스크는 보고하지 않음), 서버가 `managed_nodes` 에 저장하여 노드 상세(NodeDetail)로 노출한다. 관리자 뷰가 이 값을 고정 캔버스 크기로 소비한다.

### AC-72: 노드 해상도 하위 호환·폴백 (REQ-M03, N03, K09)

- Given: (a) 해상도를 보고하지 않는 구버전/미설정 노드, (b) 잘못된 형식(0/음수/비정수)을 보고하는 노드가 있다.
- When: 각 노드가 접속하고 관리자가 관리자 뷰에서 그 노드의 대시보드를 연다.
- Then: 해상도 필드는 빈값/0 으로 처리되어 등록·관리가 정상 동작하며(회귀 0), 관리자 뷰는 **합리적 기본 해상도**(예: 1920×1080) 또는 **가용 컨테이너 크기**로 폴백하여 대시보드를 안전하게 렌더한다. 잘못된 형식도 폴백으로 처리된다.

### AC-73: 관리자 뷰 — 상단 바 관리 크롬 + 전체 너비 노드 화면 (REQ-M04)

- Given: server 모드 + admin 관리자가 관리자 뷰에서 노드를 선택했다.
- When: 관리자 뷰가 렌더된다.
- Then: 관리 크롬(노드 피커·서브탭 네비·관리 액션·나가기)이 **상단 수평 바**에 배치되고, 노드 화면이 그 아래 **남은 전체 너비/높이**를 점유한다. 좌측 디렉토리/사이드바가 노드 화면 영역에서 제거되어 노드 화면의 좌우 폭이 축소(왜곡)되지 않는다. (M9 의 좌 디렉토리 20rem + 우 대시보드 master/detail 배치가 상단 바 + 풀폭으로 진화된다.)

### AC-74: 노드 피커 — M9 그룹 구조 보존 (REQ-M05, K01~K05)

- Given: 노드가 여러 그룹("공장A"/"공장B")과 미지정("전체")에 분산되어 있다.
- When: 관리자가 상단 바 노드 피커를 열어 다른 그룹의 노드로 전환한다.
- Then: 노드 피커가 좌측 디렉토리를 대체하되 **그룹별로 묶여**("전체" 기본 버킷 포함) 노드를 선택 가능하게 하고(단일 레벨 그룹 보존), 노드 전환 시 상단 바 컨텍스트(노드 이름·online/status)와 하단 노드 화면이 즉시 전환된다. 그룹 배정/해제 affordance(REQ-K02)가 보존된다(피커 메뉴/관리 액션 위치).

### AC-75: 상단 서브탭 네비 + 전역 사이드바 숨김·복원 (REQ-M06)

- Given: 관리자가 관리자 뷰에 진입한다.
- When: 서브탭(개요/플로우/에이전트/디바이스/대시보드)을 전환하고, 이후 관리자 뷰를 나간다.
- Then: M9 노드 대시보드 서브탭이 **상단 바**에 배치되어 전환되고(딥링크 `?node=`/`?tab=` 보존), 관리자 뷰에서는 앱 전역 좌측 `Sidebar` 가 숨김/접힘되어 노드 화면이 가로막히지 않으며, 나가기 시 전역 사이드바가 이전 상태로 복원된다(다른 화면 회귀 0).

### AC-76: 대시보드 고정 캔버스 — 노드 해상도 (REQ-M07)

- Given: 노드가 해상도 1920×1080 을 보고했고, 관리자 뷰 대시보드 서브탭이 열려 있다.
- When: 원격 대시보드(RemoteDashboardView)가 렌더된다.
- Then: 대시보드가 노드 해상도와 **동일한 고정 픽셀 크기 캔버스**(1920×1080)에 렌더되고, 내부 그리드/패널은 **반응형 리플로우 없이** 고정 컬럼·고정 레이아웃으로 노드 장비에서의 배치를 그대로 유지한다(패널 re-flow 없음). 해상도 미보고 시 폴백 크기(REQ-M03)가 캔버스 크기로 사용된다.

### AC-77: 레터박스 스케일-투-핏 — 종횡비 보존 (REQ-M08, A21, A16)

- Given: 고정 캔버스(예: 1920×1080)가 관리자 뷰의 가용 콘텐츠 영역(다른 종횡비)에 표시된다.
- When: 가용 영역이 캔버스와 다른 종횡비이거나 크기가 변한다.
- Then: 캔버스가 `scale=min(availW/W, availH/H)` 로 종횡비를 보존한 채 스케일-투-핏되고, 남는 축에 레터박스/필러박스 여백이 생기며, 패널이 늘어나거나(stretch) 잘리지(crop) 않는다. 패널 내부 폰트/요소가 비례 축소·확대되고, 패널 컴포넌트 코드는 분기되지 않는다(스케일은 바깥 래퍼 — target 미지정 시 로컬 렌더 바이트 동일, A16). 가용 영역 변경 시 재스케일된다.

### AC-78: 고정 캔버스 내부 패널 데이터·게이팅·READ-ONLY 불변 (REQ-M09, L04~L12, J03/J12)

- Given: 고정 캔버스 안에 그룹 L 패널(flows/agents/devices/single-device·resource·logs·chart·control)이 렌더된다.
- When: 패널이 데이터를 로드하고 관리자가 control 패널 명령·자원 제어를 시도한다.
- Then: 패널 데이터는 그룹 L 경로(대시보드 config `dashboard.get_shared`/`get_mine`·`monitor.metrics` query·`monitor.logs`/`chart` 스트림·control 그룹 D)로, 게이팅(승인∧온라인∧노출)·실패 의미(503/504/502/404)·redaction·**read-only-by-default**(원격 config 편집 비활성)가 그룹 L 그대로 적용된다. 충실 재현은 렌더 레이아웃(고정 캔버스+스케일)에만 관여하며, 신규 데이터/변경 경로를 신설하지 않는다(프록시 경유 변경 금지).

### AC-79: 충실 재현 범위 = 대시보드만 (REQ-M07, OQ-M2)

- Given: 관리자 뷰에서 대시보드 서브탭과 플로우/에이전트/디바이스 서브탭이 모두 도달 가능하다.
- When: 관리자가 각 서브탭을 연다.
- Then: 대시보드 서브탭만 노드 해상도 고정 캔버스(레터박스)로 충실 재현되고, 플로우/에이전트/디바이스(M8 통합 제어) 서브탭은 상단 바 아래 **전체 너비 반응형**으로 표시된다(관리 도구 화면 — 노드 장비 화면 복제 대상 아님, 고정 캔버스 스케일 미적용).

### AC-80: M9 레이아웃 대체 — 기능 보존·딥링크 보존·등록 관리 불변 (REQ-M10, K11~K16)

- Given: M9 좌측 디렉토리 `NodeManagementPage` 가 상단 바 관리자 뷰로 대체된다.
...
| 관리자 뷰(그룹 M, 백엔드) | `go test` + testify | 노드 해상도 보고(config 선언→register/heartbeat→저장→NodeDetail 노출)·미보고 0 하위 호환·잘못된 형식 폴백·자원 메트릭 비보고 |
| 관리자 뷰(그룹 M, 추후) | Vitest + Testing Library | 상단 바 관리 크롬+풀폭 노드 화면(좌측 크롬 제거)·노드 피커(그룹 묶음·M9 그룹 보존)·서브탭 호이스팅·전역 사이드바 숨김/복원·딥링크 보존·M9 기능 보존·등록 관리 불변 |
| 충실 재현(그룹 M, 추후) | Vitest + Testing Library | 고정 캔버스(노드 해상도·리플로우 없음)·레터박스 스케일(종횡비 보존·stretch/crop 금지·재스케일·미보고 폴백)·패널 데이터/게이팅/READ-ONLY 불변(그룹 L 경로)·충실 재현 범위=대시보드만 |
