...
# SPEC-XSFM-001 구현 계획: 지하철 역사 설비 관리 에이전트 (MQTT)
...
| 순서 | 작업 | 파일 | 우선도 |
|------|------|------|--------|
| 1 | 센티널 에러 정의 (12종, `ErrInvalidTransportMode` 포함) | `internal/agent/xsfm/errors.go` | High |
| 2 | XSFMConfig 파싱·검증 (**transport_mode**/broker/tls/lwt/토픽템플릿/페이로드매핑, 모드별 검증 분기 REQ-01-13) | `internal/agent/xsfm/config.go` | High |
| 3 | I/O 경계 추상화: `CommandSink`(명령 출력) + state ingress(상태 입력) 인터페이스 + direct(Paho 래핑, thingplus 패턴) / port(포트) 두 구현 | `internal/agent/xsfm/transport.go` | High |
| 4 | Device / 로스터 타입 + 관측 플래그 | `internal/agent/xsfm/device.go` | High |
| 5 | XSFMAgent 코어 (Agent + MessageReceiver, Init/Start/Stop/Pause/Resume, transport_mode 배선 분기) | `internal/agent/xsfm/agent.go` | High |
| 6 | 토픽 렌더링(direct) + 페이로드 매핑 헬퍼(양 모드 공통 인코딩/디코딩) | `internal/agent/xsfm/mapping.go` | High |

### Secondary Goal (M2): 개별 제어

| 순서 | 작업 | 파일 | 우선도 |
|------|------|------|--------|
| 7 | Process 명령 표면 (set_power / set_fan_speed / set_multiple) | `internal/agent/xsfm/agent.go` | High |
| 8 | 풍량 1/2/3 검증 + power-ON 게이트 정책 | `internal/agent/xsfm/control.go` | High |
| 9 | 제어 명령 → 명령 토픽 발행 | `internal/agent/xsfm/control.go` | High |

### M3: 그룹 제어 (NET-NEW)

| 순서 | 작업 | 파일 | 우선도 |
|------|------|------|--------|
| 10 | GroupMembers 도출 (로스터 group_id 속성) | `internal/agent/xsfm/device.go` | High |
| 11 | 그룹 fan-out (멤버별 개별 발행) + 집계 응답 | `internal/agent/xsfm/group.go` | High |
| 12 | 부분 실패 best-effort + 빈 그룹 처리 | `internal/agent/xsfm/group.go` | High |

### M4: 상태 모니터링 + 오프라인 감지

| 순서 | 작업 | 파일 | 우선도 |
|------|------|------|--------|
| 13 | state 토픽 구독 → 페이로드 디코딩 → 로스터 갱신 | `internal/agent/xsfm/agent.go` | High |
| 14 | 관측 기반 emit (observed 플래그, power=off 시 fan 생략) | `internal/agent/xsfm/device.go` | High |
| 15 | LWT + offline_timeout 오프라인 감지 루프 | `internal/agent/xsfm/monitor.go` | High |
| 16 | request_state 온디맨드 캐시 응답 | `internal/agent/xsfm/agent.go` | Medium |

### M5: 로깅 (상태 시계열 + 제어 감사)

| 순서 | 작업 | 파일 | 우선도 |
|------|------|------|--------|
| 17 | 상태 변경 메시지 JSON (epoch ms timestamp) → msgCh | `internal/agent/xsfm/agent.go` | High |
| 18 | 제어 감사 기록 (개별/그룹, remote_audit_repository) | `internal/agent/xsfm/control.go` | High |

### M6: 플로우 노드 + 디바이스 어댑터

| 순서 | 작업 | 파일 | 우선도 |
|------|------|------|--------|
| 19 | xsfm_status 노드 (상태 emit → influxdb_write 연동 + **port 모드 상태 입력 포트**) | `internal/node/xsfm_status.go` | High |
| 20 | xsfm_control 노드 (제어 + **port 모드 제어 출력 포트**, 상태 입력 포트와 분리) | `internal/node/xsfm_control.go` | High |
| 21 | CommandSpec (set_power bool, set_fan_speed enum) + Executor | `internal/device/adapter/xsfm.go` | High |

### M7: 로스터 영속화 + add/remove/set

| 순서 | 작업 | 파일 | 우선도 |
|------|------|------|--------|
| 22 | add_device / remove_device / set_device / list_devices | `internal/agent/xsfm/agent.go` | High |
| 23 | 로스터 영속화 (GetPersistableDevices + device_id repo 라운드트립) | `internal/agent/xsfm/persist.go` | High |

### M8: 등록/배선

| 순서 | 작업 | 파일 | 우선도 |
|------|------|------|--------|
| 24 | RegisterXSFMTypes 타입 등록 | `internal/agent/xsfm/register.go` | High |
| 25 | main.go 배선 (RegisterXSFMTypes 호출) | `cmd/xflowd/main.go` | High |
...
| 순서 | 작업 | 파일 | 우선도 |
|------|------|------|--------|
| 27 | XSFM_FIELDS + 타입 엔트리 | `web/src/config/agentSchemas.ts` | High |
...
| 순서 | 작업 | 파일 | 우선도 |
|------|------|------|--------|
| 29 | 단위 테스트 (설정/제어/그룹 fan-out/상태 디코딩/오프라인/영속화) | `internal/agent/xsfm/*_test.go` | High |
| 30 | 예제 에이전트 YAML (broker + 토픽 템플릿 + 페이로드 매핑) | `examples/agents/xsfm-mqtt.yaml` | Medium |
| 31 | 예제 플로우 YAML (상태 → influxdb_write) | `examples/flows/xsfm-monitoring.yaml` | Low |
...
| 순서 | 작업 | 파일 | 우선도 |
|------|------|------|--------|
| 32 | Device 구조체에 `Station`/`Place`/`Index` 선택 필드 추가 (하위호환) | `internal/agent/xsfm/device.go` | High |
| 33 | add_device/set_device가 station/place/index 수용·갱신 | `internal/agent/xsfm/agent.go` | High |
| 34 | 위치 속성 영속화 라운드트립 (기존 필드 없는 파일 하위호환 로드) | `internal/agent/xsfm/persist.go` | High |

### M11: 역사 레지스트리 (station → line SSOT)

| 순서 | 작업 | 파일 | 우선도 |
|------|------|------|--------|
| 35 | StationEntry 타입 + StationRegistry(인메모리 캐시 + station→line/StationsByLine) | `internal/agent/xsfm/station_registry.go` | High |
| 36 | 역사 레지스트리 파일 저장소 (device_metadata 패턴, atomic write) | `internal/storage/station_registry_repository.go` | High |
| 37 | 설정 시드(`station_registry`) 로드 + `station_registry_path` 배선 | `internal/agent/xsfm/config.go` | High |
| 38 | add_station/remove_station/list_stations 런타임 명령 | `internal/agent/xsfm/agent.go` | Medium |

### M12: 제어 응답 대기 (pending-command 레지스트리)

| 순서 | 작업 | 파일 | 우선도 |
|------|------|------|--------|
| 39 | `control_response_timeout` 설정 파싱 (0=비활성) | `internal/agent/xsfm/config.go` | High |
| 40 | pending-command 레지스트리 (device_id+명령 키잉, 명령별 타임아웃, 동시성) | `internal/agent/xsfm/pending.go` | High |
| 41 | 제어 발행 경로에 pending 등록 + 에코 대기/타임아웃(`ErrControlTimeout`) 통합 | `internal/agent/xsfm/control.go` | High |
| 42 | 상태 유입 경로에서 매칭 에코로 pending resolve (양 모드 동일 상관) | `internal/agent/xsfm/agent.go` | High |

### M13: line/station 셀렉터 일괄 제어 (M4 fan-out 확장)

| 순서 | 작업 | 파일 | 우선도 |
|------|------|------|--------|
| 43 | `DevicesByStation` / `DevicesByLine`(레지스트리 경유) 대상 도출 | `internal/agent/xsfm/group.go` | High |
| 44 | 셀렉터 우선순위(device_id>station>line>group_id) + 멤버별 응답 대기 집계(ok/error/timeout) | `internal/agent/xsfm/group.go` | High |

### v0.3.0 테스트 (추가)

| 순서 | 작업 | 파일 | 우선도 |
|------|------|------|--------|
| 45 | 위치 속성 라운드트립 / 역사 레지스트리 station→line / 응답 대기 성공·타임아웃 / port 에코 상관 / 동시 명령 / line·station fan-out 집계 단위 테스트 | `internal/agent/xsfm/*_test.go`, `internal/storage/station_registry_repository_test.go` | High |
...
- RegisterXSFMTypes를 main.go(:376-400 부근)에 배선 (프로젝트 컨벤션: 정의만으로는 부족).
- API 어댑터 + 프론트엔드 XSFM_FIELDS + i18n.
