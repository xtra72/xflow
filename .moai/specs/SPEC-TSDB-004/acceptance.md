...
## AC-11 — 그룹 수 상한과 절단 신호

**Given** 그룹 수가 상한(OQ2 확정값)을 넘는 질의이고
**When** 응답을 받으면
**Then** 반환 그룹 수가 상한 이하이고 `truncated` 가 `true` 다.

**Given** 상한 이하이면
**Then** `truncated` 가 `false` 다.

## AC-12 — 절단을 조용히 수행하지 않는다

**Given** `truncated: true` 응답을 받고
**When** 패널을 렌더하면
**Then** 절단 배너가 표시된다.
**And** 배너가 **좁히는 방법**(태그 사전 필터 추가 또는 그룹 키 축소)을 함께 안내한다.
...
| 그룹 수가 정확히 상한과 같음 | `truncated: false` (초과일 때만 true) |
