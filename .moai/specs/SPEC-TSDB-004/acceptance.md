...
**Given** 그룹 키의 카디널리티가 큰 질의이고 `group_filter` 를 지정하지 않았을 때
**When** 응답을 받으면
**Then** 모든 그룹이 반환된다 — 잘리지 않는다.
**And** `truncated` 가 group by 를 이유로 `true` 가 되지 않는다.

## AC-12 — 시리즈축 페이지네이션

**Given** `group_by: ["host"]` 이고 host 값이 a·b·c·d 4종일 때
**When** `group_filter: [{"host":"a"},{"host":"b"}]` 로 질의하면
**Then** 응답에 `host` 가 a·b 인 그룹만 포함된다.
**And** `group_filter: [{"host":"c"},{"host":"d"}]` 로 질의하면 c·d 만 포함된다.
**And** **두 페이지의 버킷 시작 시각 집합이 동일**하다 — 시리즈축을 나누는 것이지 시간축을 나누는 것이 아니다(UB1-10).

```
페이지1 → hosts [a,b] · buckets [14:11, 14:12, 14:13]
페이지2 → hosts [c,d] · buckets [14:11, 14:12, 14:13]
```

**And** 다중 키에서 `group_filter: [{"host":"a","rack":"r1"}]` 는 그 **조합**만 선택하며, `host=a,rack=r2` 는 선택되지 않는다(UB1-11).
**And** 키가 하나도 없는 `group_filter` 항목은 `400` 이다 — 빈 조합은 "모든 그룹" 이 되어 페이지 선택을 조용히 무효화한다.
**And** `group_filter` 가 비면 생성 쿼리가 본 축 도입 이전과 바이트 단위로 같다.

**Given** 전체 그룹 수가 한 페이지를 넘고
**When** 설정 UI 를 보면
**Then** 페이지 이동 수단과 **전체 그룹 수**가 표시된다.
**And** D5 열거가 `truncated: true` 를 돌려주면 좁히는 방법(태그 사전 필터 · 그룹 키 축소 · 탐색 창 축소)을 안내한다.
