...
### 2.7 [U7] (Ubiquitous) 그룹 수에 상한을 두지 않고 목록을 페이지네이션한다

시스템은 한 요청이 만드는 **그룹 수에 상한을 두지 않는다.** 질의 결과는 절단되지 않으며 `truncated` 는 group by 때문에 `true` 가 되지 않는다.

대신 **결과 시리즈를 열거하는 UI 표면**이 페이지네이션한다.

| 표면 | 처리 |
|------|------|
| 설정 UI 의 그룹 목록 | 페이지네이션. 전체 그룹 수를 함께 표시한다 |
| 차트 범례 | 페이지네이션 또는 스크롤 |
| 질의 API 응답 | **페이지네이션하지 않는다** — 전량 반환 |

**질의 API 를 페이지네이션하지 않는 이유.** 차트는 폴링으로 갱신되며 매 폴링이 완결된 시간창을 요구한다. 응답을 페이지로 쪼개면 한 폴링 안에서 페이지 간 버킷 경계가 어긋나거나, 페이지를 다 받기 전에 다음 폴링이 시작된다. 이는 [SPEC-TSDB-002](../SPEC-TSDB-002/spec.md) §2.8 의 "경계는 서버가 소유한다" 계약과 충돌한다.

**시스템은 그럼에도 사용자에게 규모를 알려야 한다.** 그룹 수가 많아 차트가 읽기 어려워지는 것은 사용자가 판단할 문제이며, 판단하려면 숫자가 보여야 한다. 설정 UI 는 마지막 질의의 그룹 수를 표시하고(§2.11 S1), 그 수가 실용 한계를 넘으면 **좁히는 방법**을 함께 안내한다 — 태그 사전 필터를 걸거나 그룹 키를 줄이는 것.

> **잔여 위험(수용됨).** 상한이 없으므로 고카디널리티 태그(예: `request_id`)로 group by 하면 폴링마다 수천 시리즈가 계산·전송되고 차트가 그만큼의 라인을 그린다. 페이지네이션은 **열거 표면**의 가독성을 풀 뿐 질의 부하 · 페이로드 크기 · 차트 렌더 비용은 줄이지 않는다. 이는 "상한을 두지 않는다"는 결정의 직접적 귀결이며 §4.4 가 근거를 기록한다.
...
### 4.4 그룹 수 상한을 두지 않고 열거 표면을 페이지네이션한다

[SPEC-TSDB-003](../SPEC-TSDB-003/spec.md) §4.5 는 **열거**(시리즈 선택 표)에 대해 "페이지네이션 대신 상한 + 절단 신호" 를 택했다. 본 SPEC 은 **질의**에 대해 반대 방향을 택한다. 두 판단이 모순이 아닌 이유는 대상이 다르기 때문이다.

| 축 | 열거(TSDB-003) | 질의(본 SPEC) |
|----|----------------|---------------|
| 사용자가 원하는 것 | 고를 후보 몇 개 | 지정한 축의 **전부** |
| 일부만 주면 | 나머지를 좁혀서 다시 찾으면 된다 | **그래프가 틀린 그림이 된다** |
| 절단의 의미 | "더 좁혀라" 라는 안내 | 없는 데이터를 있는 것처럼 보이게 함 |

`group_by: ['host']` 는 "host 별로 나눠서 **전부** 보여달라" 는 요청이다. 여기서 상한을 걸어 일부만 돌려주면 사용자는 그것이 전부인 줄 알고 잘못된 결론을 내린다. 절단 배너로 알린다 해도, 차트는 이미 부분 집합을 완전한 그림처럼 그리고 있다.

따라서 **질의는 전량 반환하고, 많아서 읽기 어려운 문제는 열거 표면의 페이지네이션으로 푼다.** 규모 판단은 사용자에게 맡기되 판단에 필요한 숫자(그룹 수)를 항상 보여 준다.

**이 선택의 비용은 §2.7 의 잔여 위험 문단이 기록한다** — 고카디널리티 태그에서 질의 부하 · 페이로드 · 렌더 비용이 그대로 사용자에게 간다. 상한은 그 비용을 막는 대신 정확성을 희생하는 거래였고, 여기서는 정확성을 택했다.
