...
| 날짜 | 버전 | 변경 내용 | 작성자 | 상태 |
|------|------|----------|--------|------|
| 2026-05-22 | 0.14.2 | **BREAKING (continued) — LGCNP/LGCP/LGAP 의 `*_temp` 잔존 정리 + 명령명 통일**. v0.17.0 의 sed 가 `,omitempty` 가 붙은 JSON tag 를 놓쳐서 LGCNP ODU/IDU + LGCP/LGAP 의 다수 필드가 누락됨. 일괄 갱신: `set_temp` → `set_temperature`, `room_temp` → `room_temperature`, `outdoor_temp` → `outdoor_temperature`, `avg_temp` → `avg_temperature`, `pipe_in_temp` → `pipe_in_temperature`, `pipe_out_temp` → `pipe_out_temperature`, `pipe_temp1_c` → `pipe_temperature1_c`, `pipe_temp2_c` → `pipe_temperature2_c`, `comp_*_temp` / `condenser_temp_*` 추가 정리. Century 의 `set_temp` 명령명도 `set_temperature` 로 통일. deduplicate compare_fields 예제 + 테스트 fixture 도 갱신. | xtra | Implemented |
| 2026-05-22 | 0.14.1 | **BREAKING (continued) — `target_temp` → `target_temperature`**. v0.14.0 의 필드명 정리 후속 — `target_temp` 누락분 처리. 5 HVAC agent JSON tag (Century/Samsung/LG.{LGCNP/LGCP/LGAP}), 노드 테스트, web src (deviceLabels / panels / nodeTypeMeta) 전반에서 일괄 변경. 제어 명령 payload 키도 동일 적용 (`set_temperature` 명령의 `target_temp` 파라미터 → `target_temperature`). | xtra | Implemented |
