# SPEC-LG-HVACR-002-001: Implementation Plan (v2.0.0 - Clean Transport Abstraction)
...
2. **LG HVACR-02 에이전트 내부 구조를 변경하지 않는다.** `captureLoop()`, `transportReader`, LG ICP-02 FrameParser, `Process()` 는 그대로 유지.
3. **기존 `lgapSerialTransport` 를 수정하지 않는다.** 검증된 코드는 건드리지 않는다.
4. **`NewHvacr02Agent()` 에 Transport Factory만 추가한다.** 설정에 따라 올바른 transport 구현체를 생성.
...
**TCP Server 연결 관리**: LG ICP-02 는 point-to-point 프로토콜이므로 TCP Server 모드에서 단일 활성 연결만 유지한다. 새 연결이 들어오면 기존 연결을 닫고 새 연결로 교체한다.
...
**Config 확장**: `Hvacr02Config` 에 `TransportType`, `TCPHost`, `TCPPort`, `TCPReadTimeout`, `TCPWriteTimeout`, `TCPConnectTimeout` 필드를 추가하고, `parseHvacr02Config()` 에서 `transport_type` 에 따라 필수 필드를 조건부로 검증한다.
...
| TCP Server 다중 연결 | 최신 연결 교체 (단일 활성) | LG ICP-02 point-to-point 프로토콜 특성 |
| 재연결 로직 | 기존 reconnectLoop 재사용 | transport.Open()/Available() 만 구현하면 자동 동작 |
| 파일 구조 | transport_tcp.go 신규 생성 | 기존 transport.go 는 수정하지 않음 |
| Config 확장 | 기존 Hvacr02Config 에 필드 추가 | 별도 struct 불필요, 단순성 유지 |
...
**산출물**:
- `lg_hvacr02_agent_test.go`: 기존 serial 모드 동작에 대한 characterization 테스트 보강
...
- `lg_hvacr02_config_test.go`: 기존 설정 파싱에 대한 characterization 테스트 보강
...
**방법론**: TDD (신규 필드) + DDD (기존 parseHvacr02Config 수정)
...
**목표**: NewHvacr02Agent() 에 Transport Factory 추가, reconnectLoop 미세 조정

**산출물**:
- `lg_hvacr02_agent.go` 수정:
  - `NewHvacr02Agent()` 에 `transport_type` 에 따른 transport 생성 로직 추가
...
**방법론**: Hybrid (TDD: Factory 로직, DDD: 기존 NewHvacr02Agent 수정)
...
**산출물**:
- `lg_hvacr02_agent_test.go` 추가:
  - TCP Client transport 로 captureLoop 동작 테스트:
    - TCP 서버에서 LG ICP-02 프레임 전송 → captureLoop 파싱 → msgCh 출력
  - TCP Server transport 로 captureLoop 동작 테스트:
    - TCP 클라이언트에서 LG ICP-02 프레임 전송 → captureLoop 파싱 → msgCh 출력
...
| 파일 | 변경 유형 | 설명 |
|------|-----------|------|
| `internal/agent/lg/lg_hvacr02_config.go` | 수정 | TransportType, TCPHost, TCPPort, TCP 타임아웃 필드 추가, serial_port 조건부 필수 |
| `internal/agent/lg/lg_hvacr02_agent.go` | 수정 | NewHvacr02Agent() 에 Transport Factory 추가, reconnectLoop 미세 조정 |
...
| `internal/agent/lg/lg_hvacr02_config_test.go` | 수정 | TCP 설정 파싱 테스트 추가 |
| `internal/agent/lg/lg_hvacr02_agent_test.go` | 수정 | Transport Factory 및 TCP 통합 테스트 추가 |
...
| M5: Transport Factory | Hybrid (TDD + DDD) | 신규 Factory + 기존 NewHvacr02Agent 수정 |
