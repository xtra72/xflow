...
# SPEC-AIRPURIFIER-001 인수 기준: 지하철 역사 공기청정기 관리 에이전트 (MQTT)
...
```gherkin
Given transport_mode 미지정(기본 direct)이고 AirPurifierConfig에 broker "tcp://broker:1883", state_topic_template "airpurifier/{device_id}/state", command_topic_template "airpurifier/{device_id}/cmd", 유효한 payload_mapping이 설정된 경우
When AirPurifierAgent.Init(config)이 호출되면
...
```gherkin
Given state_topic_template이 "airpurifier/state" (device_id placeholder 없음)인 경우
When parseAirPurifierConfig(opts)가 호출되면
...
```gherkin
Given AirPurifierAgent 인스턴스가 생성된 경우
When agent.Agent 및 agent.MessageReceiver 인터페이스로 캐스팅하면
Then 컴파일 타임에 성공해야 한다 (var _ agent.Agent = (*AirPurifierAgent)(nil))
```

### Scenario 1.6: MQTT 재연결 시 구독 복원 (direct 모드)

```gherkin
Given transport_mode가 "direct"이고 AirPurifierAgent가 Running 상태이며 디바이스 3대의 state 토픽을 구독 중인 경우
...
```gherkin
Given AirPurifierConfig에 transport_mode가 지정되지 않은 경우
When parseAirPurifierConfig(opts)가 호출되면
...
```gherkin
Given AirPurifierAgent가 Running 상태인 경우
...
```gherkin
Given airpurifier_status 노드가 influxdb_write 노드에 연결된 플로우인 경우
...
```gherkin
Given RegisterAirPurifierTypes(registry)가 호출된 경우
When registry.CreateAgent("airpurifier", config)가 호출되면
Then AirPurifierAgent 인스턴스가 생성되어 반환되어야 한다
```
...
Then RegisterAirPurifierTypes가 호출되어 "airpurifier" 타입이 사용 가능해야 한다
```

### Scenario 8.1: 디바이스 어댑터 CommandSpec

```gherkin
Given internal/device/adapter/airpurifier.go의 CommandSpec인 경우
...
Then { value: 'airpurifier', label: 'Air Purifier' } 엔트리와 AIRPURIFIER_FIELDS(broker/tls/토픽템플릿/페이로드매핑/qos/offline_timeout)가 존재해야 한다
