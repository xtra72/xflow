...
And VerifyLGCPCRC(frame) == true이다
```
...
When 기존 VerifyLGCPCRC() 함수로 CRC를 검증한다
...
```gherkin
Given LGCPSequenceManager가 초기화되어 있다
...
```gherkin
Given LGCP 에이전트가 제어 프레임 [56 12 ...] 을 전송했다
...
```gherkin
Given LGCP 에이전트가 제어 프레임을 전송한 지 에코 윈도우가 경과했다
...
```gherkin
Given LGCP 에이전트에 indoor 타입 디바이스 "44550065"가 등록되어 있다
...
```gherkin
Given LGCP 에이전트에 controller 타입 디바이스 "44550000"가 등록되어 있다
When LGCPDeviceProvider.Devices()를 호출한다
...
Then LGCPAgent.Process()가 올바른 JSON으로 호출된다
...
```gherkin
Given LGCP 에이전트가 실행 중이고 captureLoop가 프레임을 수신 중이다
...
```gherkin
Given control_enabled=false인 LGCP 에이전트가 실행 중이다 (기본값)
When Process({"command":"set_power", "address":"44550065", "params":{"power":true}})를 호출한다
...
```gherkin
Given control_enabled=false인 LGCP 에이전트가 실행 중이다
...
```gherkin
Given control_enabled=false인 LGCP 에이전트가 있다
When capabilities를 조회한다
Then ["passive-monitor"]가 반환된다

Given control_enabled=true인 LGCP 에이전트가 있다
When capabilities를 조회한다
...
```gherkin
Given LGCP 에이전트에 "44550099" 디바이스가 등록되어 있지 않다
