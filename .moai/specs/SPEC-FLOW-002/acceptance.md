...
Scenario: 누락 에이전트와 동일 타입의 기존 에이전트가 존재
  Given 서버에 다음 에이전트가 등록되어 있을 때:
    ...
    | lgcp-sensor-3 | lgcp  |
    | mqtt-broker-1 | mqtt  |
  And 가져오기 플로우에 "lgcp-sensor-1" (type: lgcp)이 참조되어 있을 때
  When 플로우 파일을 가져오기하면
  Then "lgcp-sensor-1"은 누락 에이전트로 표시되어야 한다
  And 동일 타입 에이전트 목록에 "lgcp-sensor-2", "lgcp-sensor-3"이 표시되어야 한다
...
Scenario: 동일 타입 에이전트가 있을 때 드롭다운 옵션
  Given 누락 에이전트 "lgcp-sensor-1" (type: lgcp)이 표시되어 있을 때
  And 서버에 "lgcp-sensor-2", "lgcp-sensor-3" (type: lgcp)이 존재할 때
  When 드롭다운을 클릭하면
...
Scenario: 드롭다운에서 기존 에이전트 선택
  Given 누락 에이전트 "lgcp-sensor-1"의 드롭다운이 표시되어 있을 때
  When "lgcp-sensor-2"를 선택하면
  Then 해당 에이전트의 해결 방법이 "substitute"로 변경되어야 한다
  And 선택된 대체 에이전트가 "lgcp-sensor-2"로 기록되어야 한다
  And UI에 "lgcp-sensor-1 -> lgcp-sensor-2" 매핑이 표시되어야 한다
```
...
Scenario: 하나의 노드에서 에이전트 이름 치환
  Given 플로우 정의에 다음 노드가 있을 때:
    ...
    | lgcp-bridge  | lgcp-sensor-1        |
  And remapTable이 { "lgcp-sensor-1": "lgcp-sensor-2" }일 때
  When remapAgentNames(definition, remapTable)을 호출하면
  Then 결과의 nodes[0].agent_ref.agent_name이 "lgcp-sensor-2"여야 한다
...
Scenario: 여러 노드가 같은 에이전트를 참조할 때 일괄 치환
  Given 플로우 정의에 다음 노드가 있을 때:
    | node_name     | agent_ref.agent_name |
    | lgcp-input    | lgcp-sensor-1        |
    | lgcp-output   | lgcp-sensor-1        |
    | mqtt-bridge   | mqtt-broker-1        |
  And remapTable이 { "lgcp-sensor-1": "lgcp-sensor-2" }일 때
  When remapAgentNames(definition, remapTable)을 호출하면
  Then nodes[0].agent_ref.agent_name이 "lgcp-sensor-2"여야 한다
  And nodes[1].agent_ref.agent_name이 "lgcp-sensor-2"여야 한다
...
Scenario: 리매핑이 원본 데이터를 변경하지 않음
  Given 원본 플로우 정의에 agent_ref.agent_name이 "lgcp-sensor-1"인 노드가 있을 때
  And remapTable이 { "lgcp-sensor-1": "lgcp-sensor-2" }일 때
  When remapAgentNames(definition, remapTable)을 호출하면
  Then 원본 definition의 nodes[0].agent_ref.agent_name은 여전히 "lgcp-sensor-1"이어야 한다
...
Scenario: agent_ref가 없는 노드는 건너뜀
  Given 플로우 정의에 다음 노드가 있을 때:
    ...
    | lgcp-bridge | bridge | { agent_name: "lgcp-sensor-1" } |
  And remapTable이 { "lgcp-sensor-1": "lgcp-sensor-2" }일 때
  When remapAgentNames(definition, remapTable)을 호출하면
...
Scenario: "새로 생성"으로 해결된 에이전트 처리
  Given 누락 에이전트 "lgcp-sensor-1" (type: lgcp)이 "새로 생성"으로 선택되었을 때
  When 가져오기를 실행하면
  Then createAgent API가 { name: "lgcp-sensor-1", type: "lgcp" }로 호출되어야 한다
...
Scenario: 대체 에이전트로 해결된 에이전트 처리
  Given 누락 에이전트 "lgcp-sensor-1"이 "lgcp-sensor-2"로 대체 선택되었을 때
  When 가져오기를 실행하면
  Then createAgent API가 호출되지 않아야 한다 (해당 에이전트에 대해)
  And createFlow API가 리매핑된 definition으로 호출되어야 한다
  And 리매핑된 definition 내 "lgcp-sensor-1" 참조가 "lgcp-sensor-2"로 변경되어야 한다
```
...
Scenario: "건너뛰기"로 해결된 에이전트 처리
  Given 누락 에이전트 "lgcp-sensor-1"이 "건너뛰기"로 선택되었을 때
  When 가져오기를 실행하면
  Then createAgent API가 호출되지 않아야 한다 (해당 에이전트에 대해)
...
Scenario: 여러 해결 방법이 혼합된 가져오기
  Given 다음 누락 에이전트 해결 상태가 있을 때:
    | name          | 해결 방법                  |
    | lgcp-sensor-1 | lgcp-sensor-2로 대체       |
    | mqtt-pub-1    | 새로 생성                  |
    | serial-dev-1  | 건너뛰기                   |
  When 가져오기를 실행하면
  Then createAgent API가 { name: "mqtt-pub-1", type: "mqtt" }로 1회 호출되어야 한다
  And createFlow API가 호출되어야 한다
  And 전송된 definition에서 "lgcp-sensor-1" 참조는 "lgcp-sensor-2"로 변경되어야 한다
...
Scenario: 에이전트 파일 드래그 앤 드롭으로 누락 에이전트 보완
  Given 누락 에이전트 "lgcp-sensor-1"이 표시되어 있을 때
  When "lgcp-sensor-1" 에이전트 정의 파일을 드래그 앤 드롭하면
