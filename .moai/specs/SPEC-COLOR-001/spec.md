...
### OQ1 — 참조의 "자동" 드롭다운을 어떤 개념으로 받을 것인가 — **닫힘**

**사용자가 답했다: "자동 - 테마에 맞는 적절한 색 추천".**

곧 (가)도 (나)도 아닌 **셋째**다. "자동" 인 자리는 그 자리의 **역할**에 따라 **현재 테마
팔레트에서** 색을 푼다. 그리고 **테마를 따라 움직인다** — day↔night 를 바꾸면 그리는 색이
바뀌고, 사용자가 팔레트를 손보면 그것도 따라온다.

> **0.1.0 의 문장(폐기 · 남긴다)**: "본 SPEC 은 그 자리를 `inheritedColor`(상속 색
> 미리보기)로 잇는다 … **정하기 전까지는 (가)로 구현한다**". (가)는 **틀린 것이 아니라
> 좁다** — `AccentGroupPicker` 의 상속은 그대로 살고, 답이 요구하는 것은 그 위에 얹는
> 갈래 하나다. 그러나 "묻는 것" 이 내민 두 갈래가 답을 담지 못했으므로 그 이분법은 폐기한다.

#### 이 답이 왜 무게가 다른가

이번 작업 묶음의 결함 가족이 전부 **팔레트를 따라가기를 멈춘 고정색**이었다 —
캔버스는 움직이는데 `dark:bg-zinc-900` 으로 박혀 버린 노드 카드, 선언된 적 없는 토큰을
가리키던 참조들. "자동" 은 사용자가 **그 반대 성질을 기본값으로** 달라고 한 것이다.

그래서 두 가지가 따라 나온다.

**(1) 푼 값을 저장하지 않는다.** 리터럴을 저장하는 순간 얼어붙고, 사용자가 벗어나려는
그 결함을 그대로 재생산한다. "자동" 은 **표지로 저장되고 그리는 시각에 풀린다.**

**(2) 이미 있는 표지를 먼저 쓴다 — 그러나 그것이 성립하는 자리에서만.**
"값이 없음(`undefined`)" 이 이미 "기본을 쓴다" 를 뜻하고 렌더러가 이미 그렇게 다루는
자리라면, 새 저장 표지도 소비자 일제 수정도 필요 없다. **감사해 보니 그것이 저장소
전체에서 성립하지는 않는다.** 성립하는 자리와 성립하지 않는 자리를 아래에 적는다.

#### 역할 → 토큰 (`lib/theme/tokens.ts` 의 label·hint 로 정했다 — 이름으로 고르지 않았다)

| 역할 | 토큰 | 그 토큰의 label · hint | day | night |
|------|------|------------------------|-----|-------|
| 글자 | `--color-text-primary` | 기본 텍스트 · **본문 · 제목** | `#111827` | `#f3f4f6` |
| 칠·배경 | `--color-bg-surface` | 표면 배경 · **카드 · 패널** | `#fbfcfe` | `#1f2937` |
| 선·테두리 | `--color-border-default` | 기본 테두리 · **카드 · 입력창** | `#dfe3e9` | `#374151` |

hint 로 고른 것은 이 색 자리들이 전부 **패널 안의 카드·표면·구분선**에 걸리기 때문이다.
그리고 이 셋은 지어낸 대응이 아니다 — **렌더러가 이미 폴백으로 쓰고 있는 바로 그 토큰들**
이다(`text-(--color-text-primary)` · `bg-(--color-bg-surface)` · `--color-border-default`).
값은 `resolveTokens('day'|'night')` 를 실제로 불러 얻었다.

`status` · `interactive` 두 무리는 **어떤 사용자 편집 색 필드의 폴백에도 쓰이지 않는다**
(실측). 임계값·델타처럼 상태를 뜻하는 자리에 "자동" 을 주려면 그것은 **새 대응을 만드는
일**이지 있는 것을 재사용하는 일이 아니다 — 이 SPEC 은 하지 않는다.

#### "없음 = 테마 기본" 이 성립하는 자리와 아닌 자리 (실측)

**성립한다** — 렌더러가 값이 없으면 토큰으로 떨어진다. `*_font.color` 일가(`panelChromeContext.ts:69`
가 키 자체를 만들지 않아 패널의 `text-(--color-text-*)` 클래스가 산다) · stat 값/통계
`font_color` · 차트 범례 `font_color` · 게이지 `needle_color`(`stroke-(--color-text-primary)`) ·
`tileBg`/`override.bg`(`bg-(--color-bg-surface)`) · sysmetrics `valueColor`·`labelColor` ·
히트맵 범례 `font_color` · 캔버스 `config.background`(투명 → 패널 표면이 비친다) ·
stat 델타 `flat_color`. **마지막 것이 이 설계의 선례다** — `DEFAULT_DELTA_COLORS.flat` 이
이미 **`'var(--color-text-muted)'` 라는 토큰 문자열 그대로** 저장·전달되어 `style={{color}}`
까지 살아서 간다(실행으로 확인).

**성립하지 않는다 — 네 갈래이며 갈래마다 이유가 다르다.**

| 갈래 | 자리 | "없음" 의 실제 뜻 | 그래서 |
|------|------|-------------------|--------|
| ① 부모 상속 | `accentElements[<그룹>]` | **`panelColor` 를 물려받는다**(`acColor` 여섯 사본 전부 `return panelColor`) | 토큰으로 바꾸면 그 상속 사슬이 끊긴다. 여기서는 (가)의 `inheritedColor` 가 **여전히 옳다** |
| ② 안 그림 | `panelColor` · 캔버스 `style.{fill,stroke,textColor}` | **아예 그리지 않는다**(악센트 테두리 없음 · 칠 없음) | "자동" 을 주면 없던 것이 생긴다 — 거동 변화이지 색 바꾸기가 아니다 |
| ③ 얼어붙은 리터럴 | 게이지 `base_color`(`#E2E8F0`) · 등고선 `line.color`(`#334155`) · 델타 `up`/`down` · 로그 레벨 배지(`dark:bg-blue-900`) | 없으면 **코드에 박힌 색**으로 떨어진다 | 표지는 성립하나 **기본값이 테마를 안 따른다.** 얼어붙은 자리가 config 에서 렌더러로 옮겨갔을 뿐이다 |
| ④ 캔버스는 `var()` 자체를 못 쓴다 | 캔버스 2D | `ctx.fillStyle = 'var(--x)'` 는 무효 | 그리는 시각에 `getComputedStyle` 로 풀고 테마가 바뀌면 다시 그려야 한다. **가장 큰 구멍이다** |

여기에 하나 더, **`undefined` 와 빈 문자열이 갈라지는 자리**가 있다. 게이지
`readBaseColor` 는 `undefined` → `#E2E8F0`(회색 채움), `''` → 채우지 않음으로 **일부러**
가른다(실행으로 확인). 비우기가 `undefined` 를 내는 고르개는 사용자가 끈 "채움 없음" 을
**조용히 회색 채움으로 되돌린다.** 오늘 그 자리가 체크상자로 `''` 를 쓰는 이유다.

#### 본 SPEC 이 하는 것과 하지 않는 것

**이 SPEC 은 "자동" 을 구현하지 않는다.** 위 표가 보이듯 ①~④ 는 저장 표지 하나로 덮이지
않고, 갈래마다 다른 작업이다. 본 SPEC 의 `inheritedColor`(표시 전용 · 저장 안 함)는
**갈래 ①에 대한 답으로서 그대로 유효하며**, 나머지 셋은 후속 SPEC 이다.

후속 SPEC 이 받아야 할 것: (a) 위 역할→토큰 대응, (b) 갈래 ③의 리터럴들을 토큰으로
재-출처화, (c) 갈래 ④의 캔버스 해석-후-재그리기, (d) 갈래 ②는 "자동" 을 주지 않는다는
결정(또는 주려면 거동 변화를 명시), (e) `undefined`/`''` 가 갈라지는 자리의 비우기 규약.
