...
| **R10** | ~~**OQ1 의 답이 (나)로 밝혀진다**~~ → **닫혔다. 답은 (가)도 (나)도 아니었다** | 사용자의 답은 **"테마에 맞는 적절한 색 추천"** — 역할별로 **테마 팔레트에서 푸는 것**이고 **테마를 따라 움직여야** 한다 | 완화가 예상대로 들어맞았다 — `inheritedColor` 는 **되돌릴 것이 없고** 갈래 ①(부모 상속)의 답으로 그대로 산다. 다만 예상보다 **넓다**: spec.md §OQ1 의 갈래표가 ①~④ 를 가르며, ②(없음=안 그림) · ③(없음=박힌 리터럴) · ④(캔버스는 `var()` 불가)는 저장 표지 하나로 덮이지 않아 **후속 SPEC** 이다. 본 SPEC 의 범위는 R10 때문에 커지지 않는다 |
