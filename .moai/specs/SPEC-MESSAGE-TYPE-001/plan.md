...
- **T2-2**: `internal/node/nasa.go`, `lg_hvacr01.go`, `lgcp.go`, `lgap.go`, `century.go` — 각 노드의 입력 메시지 분류 분기를 `msg.Type()` 기반으로 갱신.
...
| 사이트 | 사유 |
|--------|------|
| HVAC 5 에이전트 emit (lg_hvacr01_agent / lgcp_agent / LGAP agent.go / Samsung agent.go / century message.go) | 메시지 빈도 가장 높음, 분류 식별이 다운스트림 분기에 직접 영향 |
...
| HVAC 노드 (nasa.go / lg_hvacr01.go / lgcp.go / lgap.go / century.go) | 입력 메시지 분류 분기 갱신 |
