...
| 날짜         | 버전    | 변경 내용                                                                                                                                                                                                                                                                       |
| ---------- | ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-08-04 | 1.22.0 | **현재 습도(%) 모니터링 (additive, NASA 프로토콜 V1.1)**. NASA RS485 프로토콜 V1.1 이 실내기 지표 세트(Message Set) #10 — Index `0x4038`, 1바이트 값 = 현재 습도 percent (예: `0x3E`=62) — 를 추가했다. 기존 현재 온도 필드(`0x4203`, `MsgCurrentTemp`)를 미러링해 `MsgCurrentHumidity uint16 = 0x4038` 상수, `NasaDeviceState.CurrentHumidity uint8`, `UpdateFromMessageSets` 의 1바이트 추출 case, `StateForJSON` 의 관측 기반 emit(`observedHumidity`, `observedCore` 를 `uint8`→`uint16` 확장하여 9번째 bit 추가), `State()` 요약/진단 로그, `stateChanged`/`nonTempFieldsChangedHvacr01`(이산 필드 — 온도 임계값 게이트 대상 아님), device provider 및 어댑터 DTO(`SamsungNasaDeviceInfo.CurrentHumidity *uint8`) 를 추가했다. 습도는 plain uint8 percent 로 온도 2바이트 BE 디코드/음수 처리 없이 그대로 저장한다. 파서는 index 니블 0 → 1바이트로 자동 사이징하므로 파서 변경 없음. 관련: `internal/agent/samsung/message.go`, `device.go`, `agent.go`, `provider.go`, `internal/device/adapter/samsung_nasa.go`, `references/protocols/samsung_nasa_protocol.md`. |
...
| `MsgCurrentHumidity`  | `0x4038` | 1 byte    | 현재 습도(%)  | zz = 현재 습도 percent, uint8 (0~100, `0x3E`=62)              |
