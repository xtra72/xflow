...
//   2) 정규화 좌표는 스테이지 크기에 곱해질 뿐 **0..1 로 잘리지 않는다**(스테이지 밖 배치 보존).
//   3) 0 크기 스테이지는 NaN 이 아니라 유한한 0 을 낸다.
...
const ZERO_STAGE: StageSize = { width: 0, height: 0 };
...
describe('projectBox / projectLine / projectPoint — 800x600 스테이지', () => {
  it('정규화 좌표를 스테이지 px 로 투영한다', () => {
    expect(projectBox({ x: 0.25, y: 0.5, w: 0.5, h: 0.25 }, STAGE)).toEqual({
      x: 200,
      y: 300,
      w: 400,
      h: 150,
    });
    expect(projectLine({ x1: 0, y1: 0, x2: 1, y2: 1 }, STAGE)).toEqual({
      x1: 0,
      y1: 0,
      x2: 800,
      y2: 600,
    });
    expect(projectPoint({ x: 0.5, y: 0.5 }, STAGE)).toEqual({ x: 400, y: 300 });
  });

  it('0..1 밖 좌표를 clamp 하지 않는다 — 스테이지 밖으로 걸치는 배치는 정당한 저술이다', () => {
    expect(projectBox({ x: -0.25, y: 1.5, w: 2, h: 0.1 }, STAGE)).toEqual({
      x: -200,
      y: 900,
      w: 1600,
      h: 60,
    });
    expect(projectLine({ x1: -0.5, y1: 0, x2: 1.5, y2: 2 }, STAGE)).toEqual({
      x1: -400,
      y1: 0,
      x2: 1200,
      y2: 1200,
    });
    expect(projectPoint({ x: 1.25, y: -0.5 }, STAGE)).toEqual({ x: 1000, y: -300 });
  });

  it('0 크기 스테이지는 NaN 이 아니라 유한한 0 을 낸다', () => {
    expect(projectBox({ x: 0.5, y: 0.5, w: 0.5, h: 0.5 }, ZERO_STAGE)).toEqual({
      x: 0,
      y: 0,
      w: 0,
      h: 0,
    });
    expect(projectLine({ x1: 0.1, y1: 0.2, x2: 0.9, y2: 0.8 }, ZERO_STAGE)).toEqual({
      x1: 0,
      y1: 0,
      x2: 0,
      y2: 0,
    });
    expect(projectPoint({ x: 0.5, y: 0.5 }, ZERO_STAGE)).toEqual({ x: 0, y: 0 });
  });

  it('손상된 스테이지 크기(음수·비유한)도 0 으로 다룬다', () => {
    expect(projectPoint({ x: 0.5, y: 0.5 }, { width: -10, height: NaN })).toEqual({ x: 0, y: 0 });
  });

  it('손상된 좌표는 0 으로 보정해 NaN 전파를 막는다', () => {
    expect(projectPoint({ x: NaN, y: 0.5 }, STAGE)).toEqual({ x: 0, y: 300 });
    expect(projectBox({ x: 0.5, y: Infinity, w: NaN, h: 0.5 }, STAGE)).toEqual({
      x: 400,
      y: 0,
      w: 0,
      h: 300,
    });
    expect(projectLine({ x1: NaN, y1: 0.5, x2: 0.5, y2: Infinity }, STAGE)).toEqual({
      x1: 0,
      y1: 300,
      x2: 400,
      y2: 0,
...
describe('labelAnchor', () => {
  it('rect 는 박스 중심이다', () => {
    ...
    expect(labelAnchor(el, STAGE)).toEqual({ x: 400, y: 300 });
  });
  ...
  it('line 은 두 끝점의 중점이다', () => {
    const el: CanvasElement = {
      id: 'l',
      kind: 'line',
      geometry: { x1: 0, y1: 0, x2: 1, y2: 0.5 },
      style: {},
    };
    expect(labelAnchor(el, STAGE)).toEqual({ x: 400, y: 150 });
  });
