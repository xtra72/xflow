...
//
// 배율이 "얼마나 물러나 보는가" 라면 팬은 **"물러난 그 시야로 어디를 보는가"** 다. 배율이
// 그러했듯 팬도 새 기구가 아니라 **이미 있는 산술의 입력**이며, 들어가는 자리는 편집
// 갈래의 `origin` **그 한 줄**이다.
//
//     origin = floor((잰 상자 - 출력 영역) / 2 + 팬)
//              └──── 종전의 가운데 ────┘   └ 더한 것 ┘
//
// 그 한 줄뿐인 근거는 **나머지 셋에 자리가 없다**는 것이다.
//   - `box` 는 잰 상자 그 자체다(불변식 I22). 상자를 키워 팬을 만들면 그 불변식이 깨지고,
//     깨진 자리는 컨테이너의 `overflow-hidden` 이 그대로 잘라 낸다.
//   - `stage` · `cell` 은 `stageLattice` 가 낸다. 거기에 팬을 얹으면 시야가 아니라 **축척**이
//     달라지므로 그것은 팬이 아니라 조용한 배율이고, 격자가 함께 흔들린다.
//   - `derivedCanvasSize` 는 인자가 둘이어야 한다(형상 가드 · 불변식 I20).
// 축척을 건드리지 않고 시야를 옮길 수 있는 자유도가 `origin` 하나뿐이라, 팬은 그 하나다.
//
// **정수성은 그 한 줄이 그대로 진다.** 팬은 `floor` **안쪽**에 들어가므로 원점은 여전히
// 정수이며, 사용자가 세 번 돌려보낸 "격자가 일정하지 않음" 의 네 번째 얼굴이 다시 열리지
// 않는다(위 §왜 비율로 줄이고 절대 px 로 줄이지 않는가 3.).
//
// **좌표 보정도 없다.** `canvas-stage` 는 `origin` 에 실제로 앉은 **진짜 DOM 상자**이므로
// (`CanvasSurface` §상자 둘 다 진짜 DOM 상자다) 오버레이가 제 `getBoundingClientRect()` 로
// 재는 순간 이동량이 이미 들어 있다. 여기에 보정을 더하면 두 번 세는 것이 된다
// (`previewPan.ts` §왜 좌표 보정이 필요 없는가가 같은 말을 한다).
//
// **가운데 표식(`+`)의 뜻이 한 번 옮겨 간다.** `PanelEditGrid` 의 `+` 는 격자 상자, 즉
// **작업 영역**의 한가운데에 선다. 팬이 0 이면 그 자리가 출력 영역의 중심과 정확히 겹치고
// (그것이 종전의 가운데 정렬이다), 팬이 붙으면 둘이 갈린다. 그래서 `+` 는 이제 "출력
// 영역의 중심" 이 아니라 **"작업 영역의 중심"** 이며, 출력 영역의 중심과 겹치는 그 순간이
// 곧 **팬이 쉬고 있다**는 뜻이다 — 그 표식이 제 주석에 적어 둔 쓰임("가운데로 되돌렸다 를
// 눈으로 확인하는 기준")이 새 자유도 위에서 그대로 성립한다. 표식을 옮기지 않은 것은
// 그것이 여섯 패널이 함께 쓰는 컴포넌트이고, 옮기면 이 SPEC 이 캔버스 밖 다섯 패널의
// 그림까지 바꾸기 때문이다.
//
 ...
 *
 * `z ≤ 1` 이면 `reduced ≤ outer` 이고 `stage ≤ reduced ≤ outer` 이므로 **팬이 0 일 때**
 * `origin ≥ 0` 이며 출력 영역이 작업 영역 **안**에 있다. `z > 1` 이면 그 셋이 한꺼번에
 * 깨져 출력 영역 일부가 컨테이너의 `overflow-hidden` 에 잘린다.
 *
 * **상한은 팬이 생긴 뒤에도 그대로다**(사용자 신고 2026-09-16). 옛 근거("잘린 자리를
 * 가져올 팬이 없다" · 가정 A8)는 이제 절반만 참이다 — 가져올 팬은 생겼다. 그럼에도 상한을
 * 내리지 않는 근거는 남는다: `z > 1` 은 **저술 여백을 0 보다 작게** 만들어, 캔버스 밖에
 * 놓은 것에 닿으라고 만든 이 작업 영역이 캔버스 **안**조차 다 담지 못하게 한다. 팬은 여백을
 * 한쪽으로 **모는** 기구이지 없는 여백을 만드는 기구가 아니므로, 확대의 기각은 팬이 답한
 * 물음과 다른 물음이다.
 */
export const MAX_WORKSPACE_ZOOM = 1;

/**
 * 팬이 **쉬는 자리**. `workspaceBox` 의 기본 인자이자 종전의 "가운데" 그 자체다.
 *
 * 기본값이 곧 종전 동작이므로 팬을 넘기지 않은 모든 호출은 0.6.0 과 한 픽셀도 다르지
 * 않다(배율의 `DEFAULT_WORKSPACE_ZOOM` 이 지는 그 몫과 같은 몫이다).
 */
export const NO_WORKSPACE_PAN: StageCell = { x: 0, y: 0 };

/**
 * 팬을 담기는 범위로 죈다 — **출력 영역의 중심은 작업 영역을 벗어나지 않는다.**
 *
 * ## 왜 상자만 보는가 (요소를 보지 않는다)
 *
 * 파서는 요소 좌표를 **죄지 않는다** — 캔버스 밖 저술은 합법이다(`canvasConfig` 가정 A5).
 * 그래서 "가장 멀리 놓인 요소까지" 로 상한을 정하면 상한이 요소 배열을 따라 움직이고,
 * 같은 손짓이 요소 하나를 지운 뒤에 다른 거리를 낸다. 게다가 이 모듈은 상자 두 벌만 받는
 * **DOM 무의존 산술**이라 요소를 들이면 그 성질부터 깨진다. 상한은 화면이 정한다.
 *
 * ## 규칙은 한 문장이다
 *
 * 출력 영역의 중심은 상자 좌표로 `origin + stage/2 = box/2 + pan` 이므로, 그 점이 상자
 * 안이려면 `0 ≤ box/2 + pan ≤ box` 이고 곧 **`|pan| ≤ box / 2`** 다. `stage` 가 식에서
 * 사라지는 것에 뜻이 있다 — 상한이 배율과 무관하므로 **배율을 바꿔도 팬이 범위를 벗어나
 * 튀지 않는다**(`previewPan` 이 줌을 낮출 때마다 옛 이동량을 되죄어야 하는 것과 갈리는
 * 자리다).
 *
 * 그 규칙이 지키는 것과 내주는 것을 함께 적는다.
 *   - **지킨다**: 출력 영역이 통째로 화면을 떠나는 일이 없다. 반쪽은 언제나 남으므로
 *     사용자가 무엇을 보고 있는지 잃지 않는다. "잘린 자리를 가져올 팬이 없다" 는 이유로
 *     확대를 기각한 그 자리(`MAX_WORKSPACE_ZOOM`)와 반대로, 여기서는 **가져올 팬이 곧
 *     이것**이라 반쯤 나간 출력 영역이 막다른 상태가 아니다.
 *   - **내준다**: `origin ≥ 0` 이 더는 무조건 참이 아니다. 팬이 붙으면 출력 영역의 한쪽이
 *     상자 밖으로 나가 잘린다 — 그것이 저술 여백을 한쪽에 몰아 주는 대가이며, 이 기능이
 *     하는 일 그 자체다. 팬 0 에서는 종전대로 `origin ≥ 0` 이다.
 *
 * 죔의 주인이 여기 하나인 것은 배율의 규율 그대로다 — `workspaceBox` 는 죄지 않고 **죈
 * 값만 받는다**(`clampWorkspaceZoom` 과 같은 자리·같은 이유).
 */
export function clampWorkspacePan(pan: StageCell, box: StageSize): StageCell {
  return {
    x: clampPanAxis(pan.x, positiveOrZero(box.width) / 2),
    y: clampPanAxis(pan.y, positiveOrZero(box.height) / 2),
  };
}

/** 한 축을 `±bound` 로 죈다. 잴 수 없는 값(NaN·Infinity)은 0 으로 떨어뜨린다. */
function clampPanAxis(value: number, bound: number): number {
 ...
 * 그 갈래는 배율도 팬도 **보지 않는다** — 둘 다 편집기의 시야이고, 편집이 꺼진 자리에는
 * 시야를 고를 사람이 없다(REQ-05 · 가정 A3). 그래서 편집을 끄면 팬이 화면에서 **저절로
 * 사라지고**(되돌릴 상태를 따로 두지 않는다) 다시 켜면 보던 자리로 돌아온다.
 *
 * `workspace === true`(편집 중)에서는 잰 상자를 **보기 배율** `zoom` 으로 줄인 상자에
 * 격자를 맞추고, 그 결과를 잰 상자 **가운데에서 `pan` 만큼 민 자리**에 앉힌다. 가운데는
 * 이제 기준점이지 고정점이 아니다 — 팬이 0 이면 사방에 고르게 남는 종전의 그 자리이고,
 * 팬이 붙으면 저술 여백이 한쪽으로 몰린다(그것이 캔버스 밖에 놓은 것에 닿는 유일한 길이다).
 *
 * 그 이동이 `PanelEditGrid` 의 중심 표식(`+`)의 뜻을 한 번 옮긴다 — 머리말 §보기 팬의
 * 마지막 문단이 그 새 뜻을 적어 둔다. **조용히 옮기지 않는 것이 요점이다.**
 *
 ...
 *
 * ## 팬이 들어가는 자리는 `origin` 두 줄뿐이다 (사용자 신고 2026-09-16)
 *
 * 배율의 좁음을 그대로 물려받는다. 기본값 `NO_WORKSPACE_PAN` 이 종전 동작이므로 팬을
 * 넘기지 않은 호출은 한 픽셀도 다르지 않고, `box` 는 팬과 무관하게 여전히 `outer` 이며
 * (불변식 I22), `origin` 은 여전히 **같은 `floor` 하나**가 내는 정수다 — 팬은 그 `floor`
 * 바깥이 아니라 **안쪽**에 들어간다. 왜 이 자리뿐인지는 머리말 §보기 팬에 적었다.
 *
 ...
 * `clampWorkspaceZoom` · `clampWorkspacePan` 이 진다 — 죈 값만 이 자리에 들어오고, 그
 * 아래는 종전의 퇴화 처리가 그대로 받친다.
 */
export function workspaceBox(
  ...
  pan: StageCell = NO_WORKSPACE_PAN,
): CanvasWorkspaceBox {
  ...
  return {
    ...
    stage: lat.stage,
    origin: {
      x: Math.floor((outerW - lat.stage.width) / 2 + pan.x),
      y: Math.floor((outerH - lat.stage.height) / 2 + pan.y),
    },
    cell: lat.cell,
  };
}
