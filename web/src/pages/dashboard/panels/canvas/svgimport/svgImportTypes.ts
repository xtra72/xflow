 ...
 * 형상이 바뀌는 날 조용히 틀린다. **원시형 치환이 그 "형상이 바뀌는 날" 이었으므로 다시
 * 잰다**(측정: `svgImportNative.test.ts` §예산 — 그 시험이 이 표의 수를 `toBe` 로 못박는다).
 * ```
 * 경로 64개 × 10 C-명령(명령 640 전부)        =  50,424 B = 19.24%
 * 문구 64개 × 256자 한글                      =  57,400 B = 21.90%
 * 원시형 64개(사각 — 칠과 선을 둘 다 말함)    =   9,528 B =  3.63%
 * 경로 3개(명령 640 전부) + 원시형 61개       =  52,580 B = 20.06%
 ...
 * 자리를 채운다" 는 그 전략의 **채울 것**에 새로 끼지만, 요소 한 건이 256자 한글 문구의
 * 1/6 에 불과해 남은 자리를 원시형으로 채우면 예산이 오히려 줄어든다(위 넷째 줄). 그래서
 * 이 치환은 예산을 **푸는** 방향이며, 상한 셋 가운데 어느 것도 다시 정할 이유가 없다.
 *
 * 표의 수가 SPEC 본문의 옛 수(49,971 · 57,370 · 97,465)와 몇백 바이트씩 어긋나는 것은
 * **고정 입력이 다르기 때문**이지 형상이 바뀌어서가 아니다 — 좌표 자릿수와 `<text>` 의
 * `x` 배치가 다르면 요소마다 몇 바이트씩 달라진다. 위 수는 명명된 시험이 실제로 지나는
 * 문서를 잰 것이므로, 어긋남이 생기면 그 시험이 빨개진다.
 *
 ...
 * 문구만의 최악(64 × 256자 한글)이 **실측 57,400 B**(256KB 예산의 21.90%)로 경로 쪽 최악
 * (50,424 B · 19.24%)과 같은 자리에 선다. **넘으면 자르고 보고한다** — 이름표 하나 때문에
 * 문서 전체를 거절하는 것은 대가가 크다.
 *
 * 둘을 섞은 최악은 그보다 크다(98,208 B · 37.46% — `MAX_IMPORT_COMMANDS` 머리말의 실측표).
