...
//
// M10 이 더하는 다섯째 표(`TOOL_POINT_GESTURE`)는 그 갈라짐의 **나머지 반쪽**이다. 위에서
// 예고한 대로 연결선 도구의 더블클릭은 앵커가 아니라 선 위의 중간점이며, "그러면 무엇인가"
// 에 답하는 자리가 없으면 그 뜻은 오버레이의 `if` 사슬로 흩어진다. 다섯 표가 모두
// `Record<CanvasTool, …>` 이므로 갈래가 하나 늘면 컴파일러가 **다섯 자리를 함께** 가리킨다.
//
// **이 모듈은 DOM 도 React 도 모른다.** 유니온 하나 · 표 다섯 · 순수 함수 하나뿐이다.
...
/**
 * 이 도구에서 **더블클릭이 선 위의 중간점을 뜻하는가** (REQ-05 · REQ-05-b · M10).
 *
 * 위 `TOOL_ANCHOR_GESTURE` 의 **짝**이다. 한 몸짓(두 번째 누름)에 뜻이 둘이므로 어느 쪽인지
 * 답하는 자리가 있어야 하고, 011 은 그 답을 **도구**에서 찾기로 이미 정했다(§표가 넷). 이
 * 표가 다섯째이며 같은 규율이다 — `tool !== 'anchor'` 로 적으면 일곱째 도구가 조용히 참으로
 * 떨어지고, 그 도구의 더블클릭이 무엇을 뜻하는지 아무도 묻지 않은 채 지나간다.
 *
 * ## 값이 위 표의 **정확한 반대**인 것은 우연이다
 *
 * 오늘 여섯 도구에서 두 표는 서로의 부정이다. 그래도 `!TOOL_ANCHOR_GESTURE[tool]` 로 줄여
 * 적지 않는다 — 그 등식은 **여섯의 우연**이지 법칙이 아니다. M3'b 가 `TOOL_SHOWS_ANCHORS`
 * 와 `TOOL_ANCHOR_GESTURE` 에 대해 같은 말을 적었고(§표가 넷) 그 둘은 M8 에서 실제로
 * 갈라졌다. 더블클릭을 **아무 뜻으로도** 쓰지 않는 도구(예: 문구 편집 도구)가 하나 서는
 * 날, 줄여 적은 쪽은 그 도구에 점을 흩뿌리기 시작한다.
 *
 * ## 고르기 도구에서도 참이다 — 그리고 009 와 섞이지 않는다
 *
 * 고르기 도구의 더블클릭에는 이미 뜻이 있다(그룹 진입 — 009 REQ-01). 두 뜻이 부딪히지
 * 않는 것은 **대상이 다르기** 때문이다: 그룹 부품 위면 진입이고 연결선 잉크 위면 점이다.
 * SPEC 이 그 둘을 나란히 적어 두었고(REQ-05-c "두 몸짓은 섞이지 않는다 — 대상이 다르다"),
 * 그래서 이 자리에서 고르기를 빼면 "선을 고른 뒤 도구를 갈아 끼워야 꺾을 수 있다" 가 된다.
 *
 * 앵커 도구만 거짓인 까닭은 그 도구가 더블클릭을 **대상과 무관하게** 가져가기 때문이다
 * (위 표의 머리말). 가져간 몸짓을 둘이 나눠 쓰면 도형 위의 두 번째 누름이 앵커인지 점인지
 * 눌러 봐야 알게 된다.
 */
export const TOOL_POINT_GESTURE: Readonly<Record<CanvasTool, boolean>> = {
  select: true,
  anchor: false,
  straight: true,
  elbow: true,
