...
export interface ConnectorElement {
  ...
  /**
   * 직각 선의 **가운데 구간이 놓인 자리** (SPEC-CANVAS-019). 절대 캔버스 좌표 하나.
   *
   * 축이 가로면 `x`, 세로면 `y` 로 읽는다. **어느 쪽인지 되묻지 않아도 되는 것이 018 의
   * 값이다** — 018 이 축을 앵커에서 읽게 만들어 상자를 옮겨도 뒤집히지 않으므로, 수 하나가
   * 뜻을 잃지 않는다. 017 까지였다면 같은 수가 어떤 날은 `x`, 어떤 날은 `y` 로 읽혔다.
   *
   * **점 둘로 적지 않는 이유**: 그 둘은 `(x, 시작.y)` 와 `(x, 끝.y)` 인데 상자를 옮기면
   * `시작.y` 가 바뀌어 저장된 점이 낡는다. 그러면 첫 구간이 더 이상 수평이 아니고 018 이
