...
describe('고정 앵커가 저장되지 않는다 (AC-14 · A1)', () => {
  ...
  it('내보내는 함수가 여섯뿐이다 — 읽기 하나 · 쓰기 둘 · 집기 셋', () => {
    // M3 에서는 이 목록이 하나였다. M3′ 가 **임의 앵커**(A10 — 저장한다)의 더하기·빼기로
    // 둘을 더했고, M3′b 가 **몸짓의 산술** 둘을 더했다(`nextAnchorId` · `anchorGestureAt`).
    // M8 이 여섯째(`anchorHitAt`)를 더한다 — 잇는 몸짓이 **여러 노드에 걸쳐** 한 점이
    // 가리키는 앵커를 물어야 하기 때문이다(REQ-03).
    //
    // 뒤엣셋이 이 파일에 사는 근거는 하나다: 그 셋이 보는 앵커 자리는 `anchorPoints` 가
    // 낸 **그 지도**여야 한다. 오버레이에 두면 화면에 점을 찍는 함수와 그 점을 집는 함수가
    // 갈라지고, 그 어긋남은 예외도 경고도 없이 화면에서만 보인다(위험 R1). 그리고 그
    // 갈라짐은 011 의 가드 하나가 이미 **세어서** 막고 있다 — `anchorPoints` 를 부르는
    // 제품 파일은 이름으로 적은 둘뿐이고, 이 파일은 그 목록 밖(즉 원본)이다.
    //
    // 목록이 닫혀 있다는 성질은 그대로다: 일곱째가 생기면 빨개진다. 고정 아홉이 저장되지
