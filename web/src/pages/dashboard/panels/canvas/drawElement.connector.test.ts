...
describe('곡선은 **새 명령을 만들지 않는다**', () => {
  ...
  it('AC-49: `DrawContext2D` 에 `quadraticCurveTo` 가 없다', () => {
    ...
    //
    // ## `setLineDash` 를 이 목록에서 뺀다 (SPEC-CANVAS-012 M6 · AC-31)
    //
    // **지우지 않고 한 낱말만 줄인다.** 남은 셋을 거절한 008 의 근거는 오늘도 한 글자도
    // 틀리지 않았고(2차는 3차와 **항등**이다 · `arcTo` 는 모서리를 적는 **두 번째 방법**을
    // 만든다 · `arc` 는 현재 점에서 호 시작점까지 **직선을 몰래 긋는다**), 012 는 그 셋에
    // 손대지 않았다. 목록을 통째로 지우면 그 셋을 지키던 가드가 함께 사라진다.
    //
    // 008 이 `setLineDash` 를 거절한 문장은 **범위 판정**이었다 — "뒤의 셋은 기하 축이
    // 아니라 스타일 축이다 … **008 의 주제가 아니다**". 012 의 주제가 그 스타일 축이므로
    // 그 거절은 012 에 걸리지 않는다.
    //
    // **그리고 이 단언은 애초에 인터페이스가 아니라 스텁을 재고 있었다.** `Object.keys`
    // 는 이 파일의 기록 스텁이 가진 키를 세므로, 선택적 멤버를 스텁이 구현하지 않으면
    // 멤버가 **있어도** 초록으로 남는다. 즉 `setLineDash` 를 남겨 두면 "없다" 고 말하면서
    // 실제로는 아무것도 막지 못하는 단언이 된다 — 그런 단언은 가드가 아니라 거짓말이다.
    // 파선을 실제로 재는 일은 제 스텁이 그 멤버를 갖춘 `drawElement.dash.test.ts` 가 진다.
    const ctx: DrawContext2D = makeRecorder();
    expect('quadraticCurveTo' in ctx).toBe(false);
    for (const absent of ['quadraticCurveTo', 'arc', 'arcTo']) {
