...
//
// 정규화(0..1) 스테이지 좌표 → CSS px 투영, 도형별 경로 수치, 텍스트 정렬 기준점,
...
//
// 좌표 규약은 `heatmap/stage.ts` 와 같다: 정규화 좌표는 스테이지 크기에 곱해 px 가 된다.
// 두 번째 규약을 만들지 않는다. 다만 좌표를 0..1 로 **clamp 하지 않는다** — 스테이지 밖으로
// 일부 걸치는 배치도 뜻이 있는 저술이며(canvasConfig 의 기하 손상 정책과 같은 이유),
// 잘라내면 사용자 의도가 조용히 바뀐다.
//
...
/** 정규화 좌표 한 축을 해당 축 길이에 투영한다. */
function project(coord: number, extent: number): number {
  return finite(coord) * extent;
}
 ...
 * 조용히 사라져 원인을 찾기 어렵다.
 */
export function projectBox(geo: BoxGeometry, stage: StageSize): PxBox {
  const w = positiveOrZero(stage.width);
  const h = positiveOrZero(stage.height);
  return {
    x: project(geo.x, w),
    y: project(geo.y, h),
    w: project(geo.w, w),
...
export function projectLine(geo: LineGeometry, stage: StageSize): PxLine {
  const w = positiveOrZero(stage.width);
  const h = positiveOrZero(stage.height);
  return {
    x1: project(geo.x1, w),
...
export function projectPoint(geo: PointGeometry, stage: StageSize): PxPoint {
  return {
    x: project(geo.x, positiveOrZero(stage.width)),
    y: project(geo.y, positiveOrZero(stage.height)),
  };
}
 ...
 *
 * 공간 불변: 정규화 박스든 투영된 px 박스든 같은 산술이라 어느 쪽에도 쓸 수 있다.
 ...
 */
export function labelAnchor(el: CanvasElement, stage: StageSize): PxPoint {
  switch (el.kind) {
    case 'rect':
    case 'ellipse': {
      const { cx, cy } = ellipseParams(projectBox(el.geometry, stage));
      return { x: cx, y: cy };
    }
    case 'line': {
    ...
    default:
      return projectPoint(el.geometry, stage);
  }
}
