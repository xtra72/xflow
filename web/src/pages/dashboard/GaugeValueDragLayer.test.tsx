// 미리보기에서 현재값을 끌어 옮기는 레이어.
//
// 미리보기에는 패널 크기 조절·휠 확대 같은 다른 조작이 이미 있다. 아무 데나 잡아도
// 끌리면 그것들과 부딪히므로, **값 글자 위에서만** 드래그가 시작되어야 한다.
...
function Fixture({ onChange }: { onChange: (n: { x: number; y: number }) => void }) {
  return (
    <GaugeValueDragLayer offsetX={0} offsetY={0} onChange={onChange}>
...
/** jsdom 은 레이아웃을 하지 않으므로 SVG 크기를 직접 심는다. */
function stubSvgSize(width: number, height: number): void {
  vi.spyOn(screen.getByTestId('svg'), 'getBoundingClientRect').mockReturnValue({
    width,
    height,
    top: 0,
    left: 0,
...
describe('값 글자에서만 시작한다', () => {
  ...
  it('값 글자가 아닌 곳을 잡으면 끌리지 않는다 — 다른 조작과 부딪히지 않는다', async () => {
    const onChange = vi.fn();
    render(<Fixture onChange={onChange} />);
    stubSvgSize(400, 400);
