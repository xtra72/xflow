...
// 게이지 상자(`data-gauge-body`)를 잡으면 게이지 전체가 움직인다. 그 밖을 잡으면 아무
// 일도 없다: 미리보기에는 패널 크기 조절·휠 확대 같은 다른 조작이 이미 있어서, 아무
// 데나 잡아도 끌리면 그것들과 부딪힌다. 파이 드래그 레이어와 같은 규칙이다.
//
// **좌표계가 둘로 갈린다.** 값 글자는 SVG `viewBox` 좌표 안에 있으므로 픽셀을 환산해야
// 하고, 게이지 상자는 CSS transform 이라 패널 대비 백분율을 쓴다(파이와 같은 어휘).
//
...
export function GaugeDragLayer({
  ...
  const stateRef = useRef({ value, body });
  stateRef.current = { value, body };
  ...
  const onPointerDown = (e: React.PointerEvent<HTMLDivElement>): void => {
    ...
    // 값 글자를 먼저 본다 — 글자는 게이지 상자 **안**에 있으므로, 순서를 뒤집으면
    // 글자를 잡아도 게이지 전체가 움직인다.
    const valueEl = target?.closest?.('[data-gauge-value-text]');
    const bodyEl = valueEl ? null : target?.closest?.('[data-gauge-body]');
    const kind: Kind | null = valueEl ? 'value' : bodyEl ? 'body' : null;
    if (!kind) return;

    // 값 글자는 SVG 안의 좌표라 그 SVG 를, 게이지 상자는 자기 자신을 기준 변으로 쓴다.
    const box = kind === 'value' ? valueEl!.closest('svg') : bodyEl!;
    const viewBox =
  ...
  return (
    <div
      ref={hostRef}
      className="contents [&_[data-gauge-value-text]]:cursor-move [&_[data-gauge-body]]:cursor-move"
