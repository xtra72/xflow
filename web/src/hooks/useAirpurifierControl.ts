...
// 실행·응답 대기·집계)은 백엔드가 수행한다(internal/agent/airpurifier/group.go). 이 훅은
...
import { useAirpurifierDevices, useStations } from '@/hooks/useStation';
...
/**
 * airpurifier 제어 뮤테이션 훅. set_power / set_fan_speed / set_multiple 을 노출한다.
 * 각 뮤테이션 변수는 셀렉터(하나) + 제어 값이며, exec 로 전송 후 fan-out/단일 응답을 타입화해
 * 반환한다. 성공 시 로스터 쿼리를 무효화해 UI 를 갱신한다(선택적, best-effort).
 */
export function useAirpurifierControl(agentId: string) {
...
export interface FacilityRoster {
  ...
  isError: boolean;
  refetch: () => void;
  devicesQuery: ReturnType<typeof useAirpurifierDevices>;
  stationsQuery: ReturnType<typeof useStations>;
}

/**
 * Facility 대시보드용 로스터 읽기 훅(REQ-FACDASH-001-05-04). list_devices + list_stations 를
 * 함께 조회하며, refreshMs 지정 시 두 쿼리를 주기 폴링한다(airpurifier 로스터는 WebSocket
 * 푸시가 아닌 exec 폴링). 기존 useAirpurifierDevices / useStations 를 재사용해 DRY 를 유지한다.
 */
export function useFacilityRoster(agentId: string, refreshMs?: number): FacilityRoster {
  const devicesQuery = useAirpurifierDevices(agentId, refreshMs);
