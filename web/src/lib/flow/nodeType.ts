// 노드 타입 정규화 유틸.
//
// 12개 HVAC 노드 + XSFM(설비) status/control 노드 TYPE 식별자가 `_` → `-` 로
// 정규화되었다(canonical). 백엔드는 두 표기를 모두 해석하지만(엔진 alias),
// 프론트엔드의 스키마/메타 조회는 canonical `-` 키만 보유한다. 저장된 플로우가
// 옛 `_` 타입을 들고 있어도 스키마/포트/설명이 정상 해석되도록, 조회 직전에 이
// 함수로 정규화한다.
//
// 매핑 대상이 아닌 타입은 그대로 반환한다. (하이픈 없는 `xsfm` 통합 노드는 별칭 불필요.)

/** 옛 `_` 타입 → canonical `-` 타입 매핑 (HVAC 12종 + XSFM 2종). */
const HVAC_TYPE_ALIASES: Readonly<Record<string, string>> = {
  ...
  // XSFM(설비) status/control `_` 별칭 (백엔드 registry alias 와 정렬).
 ...
 *
 * 옛 `_` 타입(HVAC 12종 + XSFM status/control 2종)은 대응하는 `-` 표기로
 * 변환하고, 그 외 타입은 변경 없이 그대로 반환한다. 빈 문자열/이미 canonical
 * 인 값도 안전하다.
 */
