    ...
    configSchema: {
      fields: [
        ...
        },
        {
          ...
          label: '폴링 명령',
          default: 'get_recent',
          options: ['get_recent', 'get_all', 'get_state'],
          description: 'get_recent: 변경 누적 (lastSeq cursor) / get_all: 모든 디바이스 즉시 / get_state: 단일 디바이스 (device_id 필요)',
        },
        {
          name: 'poll_interval',
          type: 'string',
    ...
    configSchema: {
      fields: [
        ...
        },
        {
          ...
          label: '폴링 명령',
          default: 'get_recent',
          options: ['get_recent', 'get_all', 'get_stats'],
          description: 'get_recent (count=0=drain) / get_all (모든 device 즉시 snapshot) / get_stats (통계)',
        },
        {
          name: 'recent_count',
          type: 'number',
  ...
  'lgcnp-status': {
    ...
    configSchema: {
      fields: [
        ...
        { name: 'poll_command', type: 'select', label: '폴링 명령', options: ['get_recent', 'get_all', 'get_stats'], default: 'get_recent', description: 'get_recent (count=0=drain) / get_all (모든 device 즉시 snapshot) / get_stats (통계)' },
