...
export interface ConfigField {
  name: string;
  type: 'string' | 'number' | 'boolean' | 'select' | 'object' | 'string_list' | 'agent_select' | 'register_map' | 'transform_pipeline' | 'key_value_map' | 'typed_key_value_map' | 'trigger_schedules' | 'compare_fields';
  label: string;
  required?: boolean;
  default?: unknown;
  options?: string[];
