...
| 10–11 | `temp_A` (실내온도 추정) | `0` | `0` | `0x00FC`=25.2℃ | `0x00FC`=25.2℃ | 운전 중에만 채워짐 |
| 12–13 | `op_val_2` (운전 부하 추정) | `0` | `0` | `0x00FC`=252 | `1248` | 운전 부하에 따라 변동 |

> **v0.3 추가**: CAP-4에서 data[8–9]가 `996`, data[12–13]이 `1248`로 새로 채워졌다(CAP-3에선 0 또는 252). 압축기가 정상 부하 운전에 들어가면서 나타나는 값이라 **압축기 주파수 / 소비전력 / 운전 적산값** 등의 운전 부하 지표로 추정되나, 단위·의미는 미확정. data[10–11]은 두 운전 캡처에서 모두 25.2℃로 안정적이어서 실내/리턴에어 온도 가능성이 있다. data[0] `status`는 4캡처 모두 다른 값(`0x63`/`0x4D`/`0x3B`/`0x39`)으로, 모드 외 동적 비트가 섞인 bitmap이다.
...
| Reg 0x02 data[11–16] (온도/습도 추정) | 설정 온도만 단독 변경, 또는 장시간 캡처로 실내온도 변화 추적 |
...
| Reg 0x04 data[10–13] 온도 확정 | 실내온도가 변하는 구간 캡처 |
...
5. 냉방 가동 후 실내가 식는 동안 장시간 캡처 → 실내온도 필드(reg 0x02 tail / reg 0x04 data[10–13]) 식별
...
def decode_reg02(d: bytes) -> dict:
    """Register 0x02 응답 해석 (설정 readback)."""
    if len(d) < 9:
        return {}
    return {
...
01 00 30 00 11 00 00 06 3b 00 04 [3b f6 09 00 00 00 00 2c 00 00 fc 00 fc 00] 75 59
  data[0]=3b → status   data[10..13]=fc 00 fc 00 → 25.2℃ (운전 중 온도)
...
| reg0x02 setpoint | 25.0℃ | 27.0℃ | 25.0℃ | 25.0℃ |
...
| reg0x04 `data[10..11]` temp_A | 0 | 0 | 25.2℃ | 25.2℃ |
...
01 00 30 00 14 00 00 06 3b 00 02 [00 01 11 00 00 00 00 fa 00 00 00 fa 00 1b 38 39 00] 90 8c
  data[1]=01·data[2]=11·setpoint=25.0℃ → CAP-3와 동일 (설정 불변)
...
01 00 30 00 11 00 00 06 3b 00 04 [39 f6 09 00 00 00 00 2c e4 03 fc 00 e0 04] 2c 7c
  data[0]=39 status   data[8..9]=e4 03 → 996   data[10..11]=fc 00 → 25.2℃
