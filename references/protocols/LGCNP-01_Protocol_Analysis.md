...
[07]    1B   RESERVED          0x00
[08]    1B   FAN_PARAM         팬/운전 파라미터 (0x20 또는 0x21)
[09]    1B   SET_TEMP_RAW      설정온도 raw  ← b[29]와 항상 동일
...
[27]    1B   PARAM2            = 0x28
[28]    1B   RESERVED          0x00
[29]    1B   SET_TEMP_RAW2     설정온도 재확인 (b[09]와 항상 동일 ← 이중 기록)
[30]    1B   PARAM3            파라미터
...
| 필드 쌍 | 오프셋 | 검증 규칙 |
|--------|-------|---------|
| 설정온도 | b[09] = b[29] | 항상 동일 → 불일치 시 프레임 폐기 |
...
| IDU | 설정온도 | 실내온도 | 흡입온도 | 토출온도 |
|-----|--------|--------|--------|--------|
| #1 | 22°C | 22.5°C | 26.5°C | 27.5°C |
| #2 | 25°C | 23.5°C | 27.5°C | 27.0°C |
| #3 | 24°C | 22.5°C | 28.0°C | 27.5°C |
| #4 | 21°C | 23.5°C | 27.0°C | 27.0°C |
| #5 | 23°C | 23.5°C | 28.0°C | 27.0°C |
...
```
|현재값 - 이전값| <= 2.0°C   (설정온도 제외, 운전 중 변경 가능)
```
...
function validate(pkt, prev) {
    ...
    // 계층 2: 이중 기록 ← 가장 먼저 체크
    if (pkt[9]  !== pkt[29]) return { ok: false, reason: 'SET_TEMP 이중 기록 불일치' };
...
function tryParse(pkt) {
    ...
    if (pkt[9]  !== pkt[29]) return null;  // 설정온도 이중 기록
...
function decode(pkt, iduNum) {
    ...
    const inletTemp  = (pkt[24] - 0x40) / 2.0;
    const outletTemp = (pkt[25] - 0x40) / 2.0;
    const rangeOk = setTemp >= 16 && setTemp <= 30
    ...
    return {
        ...
        setTemp, opMode: pkt[10], statusFlags: pkt[11],
        iduIndex: pkt[20],
...
func ParseIDU(buf []byte) (*IDUPacket, error) {
    ...
    // 이중 기록 검증 — 핵심 무결성
    if buf[9] != buf[29] {
        return nil, fmt.Errorf("SET_TEMP mismatch: b09=0x%02x b29=0x%02x", buf[9], buf[29])
    }
    if buf[23] != buf[36] {
