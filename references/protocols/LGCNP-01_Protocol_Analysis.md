...
| bit3 | 0x08 | 그룹 B 식별 (냉방 그룹 등) | **실측 확인** |
...
**IDU 그룹 분기 기준:**
| 그룹 | IDU | CMD 쌍 | 캡처 1 설정온도 |
|------|-----|--------|--------------|
| A | #1, #4 | 01↔41 | 23°C |
| B | #2, #3, #5 | 09↔49 | 25°C |

그룹 분기는 운전 모드/설정온도에 기반한 것으로 추정. 동일 설정의 IDU들이 같은 그룹에 배정된다.
...
**b[30]이 풍량 설정 필드임을 실측 확인. bit6으로 미풍/약풍을 구분.**

30,363줄 스트림 분석에서 확정:
- IDU#2만 b[30]=0x14, 나머지 4대는 모두 0x54 (전 구간 고정)
- IDU#2 = 약풍(1대), IDU#1,#3,#4,#5 = 미풍(4대) — 현장 설정과 일치

| b[30] | bit6 | 풍량 | 통일 ID | 관측 수 | 확인 상태 |
|-------|------|------|--------|--------|----------|
| 0x54 | 1 | quiet (미풍) | 1 | 1,098개 | **실측 확인** |
| 0x14 | 0 | low (약풍) | 2 | 1,080개 | **실측 확인** |
| 기타 | - | auto (자동) | 0 | 미관측 | 기본값 |
...
| CMD bit3 (0x08) 그룹 B | 냉방 그룹에 해당하는 것으로 추정. 다른 모드 조합에서의 동작 미확인 |
